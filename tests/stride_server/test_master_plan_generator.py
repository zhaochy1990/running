...
class TestBuildMasterPlan:
    ...
    def test_sub250_fm_gate_does_not_treat_or_inside_normal_as_disjunction(self):
        data = json.loads(_VALID_JSON_STR)
        principle = (
            "PB2:59:22→2:50为5.2%；A=2:50 opens only with HM<=1:24:30 or "
            "10K<=37:45 + max legal 29-32km/22-24kmMP + VO2/HR/RPE normal "
            "and Achilles normal; train by B."
        )
        race_target = (
            "A=2:50 opens only with HM<=1:24:30 or 10K<=37:45 + "
            "max legal 29-32km/22-24kmMP + VO2/HR/RPE normal and Achilles normal; "
            "B=2:52-2:53; C<3h."
        )
        data["plan"]["training_principles"] = [principle]
        data["plan"]["milestones"] = [
