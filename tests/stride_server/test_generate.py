...
def _no_active_master_plan(monkeypatch):
    ...
    fake so the route is exercised without a live LLM. ``today`` is pinned AFTER
    the target weeks so generation is not mid-week and prior weeks count as
    complete (deterministic regardless of wall clock).
    """
...
class TestNewUserDefaultKm:
    ...
    def test_source_echoed(self, app_client):
        ...
        # dropped with the rule-engine template — the executable week is now
