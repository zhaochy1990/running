...
def test_onboarding_background_sync_stops_before_sqlite_write_when_writer_is_busy(
    monkeypatch,
):
    ...
    writes: list[dict] = []
    monkeypatch.setattr(ob_mod, "_read_onboarding", lambda _uuid: {})
    monkeypatch.setattr(
        ob_mod,
        "_write_onboarding",
        lambda _uuid, data: writes.append(dict(data)),
    )
    source = MagicMock()
    ...
    source.sync_user.assert_not_called()
    assert writes[-1]["sync_state"] == "error"
    assert writes[-1]["sync_progress"]["phase"] == "error"
    assert "稍后重试" in writes[-1]["error"]
