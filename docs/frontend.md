...
- `GET /api/declarations` / `GET /api/declarations/{doc_type}`（Go `cmd/api`）—— 合规声明（用户协议 / 隐私政策 / 儿童个人信息保护规则 / 个人信息收集清单 / 第三方信息共享清单）的**当前生效版本**。**完全公开无鉴权**（登录前页面要展示协议）；列表返回 5 种类型的 `title/version/effective_at/updated_at` 元信息（未发布的 `current` 为 `null`），详情额外返回 `content_markdown` 正文。**永不返回草稿**：未发布类型与不存在的类型同样返回 `404`；非法 `doc_type` 返回 `400 invalid_doc_type`。管理端维护接口在 `/api/admin/declarations*`，仅 admin JWT（TierAdmin）。
