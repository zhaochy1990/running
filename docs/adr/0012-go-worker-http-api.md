...
- **Committing the generated `docs` package + a `git diff` drift check**: rejected in favor of generate-at-build to keep generated artifacts out of the tree; the trade-off is weaker staleness detection (see Consequences).
...
- **Swagger codegen is a build prerequisite:** because the generated `docs` package is git-ignored and imported by `gin-swagger`, `go build`/`go test` of `cmd/api` won't compile until `swag init` has run — so `make swagger` must run before local builds, and in the Dockerfile/CI before `go build`. Without a committed baseline, CI can catch annotation *errors* (`swag init` fails) but not *staleness* (no `git diff` to compare against).
