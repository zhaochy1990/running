...
The Python side has one canonical Shanghai helper (`stride_core/timefmt.py`), enforced by a CI grep invariant. The Go side had grown **four independent Shanghai (UTC+8) implementations** — `watchmap.ShanghaiDay` (exported, tested, 2 callers), `handlers/compute.shanghaiToday`/`shanghaiDayOf` (private), `pb.shanghaiDayStr` (private), and `garmin.shanghaiZone` (`time.FixedZone("CST", 8*3600)`) — each re-deriving the same civil-day/zone logic. When the new Go Race Goal handler (ADR 0021) needed a "today in Shanghai" for its future-date guard, adding a fifth copy was the path of least resistance. We instead **create one canonical package and converge the existing four onto it**, mirroring the Python single-source discipline (AGENTS.md "don't reinvent the wheel").
