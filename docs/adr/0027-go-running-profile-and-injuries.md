...
- This intentionally breaks the old Web profile POST contract; no compatibility path for stale clients is required. Go may deploy before Web with an explicitly accepted temporary old-UI outage, but Web route cutover remains blocked until the required Go capability and canonical MySQL season-plan readers are deployed.
