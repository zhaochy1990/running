...
- The Web `/plan` active path reads the authenticated JWT `sub` from its auth state and makes one request to `/api/users/{user_id}/master-plan/current`. A 404 alone enters the existing creation UI; transport failures, malformed content, invariant violations, and storage errors render an explicit read error.
...
- Web routing is declared in `Dockerfile.web` with `STRIDE_ROUTE_GET_USERS_USER_ID_MASTER_PLAN_CURRENT=go`. The BFF matches the dynamic user path and routes it to Go. No deploy-workflow readiness gate is added; release ordering and manifest review are manual operational controls.
