...
每次 push `data/*/logs/*/plan.md` 或 `plan.json` 后，`sync-data.yml` 调 `POST /internal/plan/reparse?user=&folder=`，header `X-Internal-Token: $STRIDE_INTERNAL_TOKEN`。server 优先校验 authored `plan.json` 并只写 `WeeklyPlanStore`（prod `strideweeklyplan`）；缺少有效 JSON 时才重跑 LLM reverse parser。SQLite 不保存新的结构化周计划。
