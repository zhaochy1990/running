...
每次 push `data/*/logs/*/plan.md` 或 `plan.json` 后，`sync-data.yml` 调 `POST /internal/plan/reparse?user=&folder=`，header `X-Internal-Token: $STRIDE_INTERNAL_TOKEN`。server 优先校验 authored `plan.json` 并写入 `WeeklyPlanStore`（prod `strideweeklyplan`），同时刷新 `planned_session` / `planned_nutrition` 手表投影；缺少有效 JSON 时才重跑 LLM reverse parser。
