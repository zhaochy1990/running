...
- COROS API 单位怪点：activity list summary 的 `distance` 已观察为米；detail/lap 的 `summary.distance` / `lap.distance` 已观察为厘米（除 100 得米）；frequencyList 的累计 `distance` 也是厘米，入库前统一除 100 存米；time centiseconds（除 100 得秒），calories cal*1000
