...
This is a client of the STRIDE backend at `https://api.stride-running.cn/api/*`, with auth over the same gateway at `/api/auth/*`. No new backend endpoints required for v1 reads — push notifications (S13a) add a few.
