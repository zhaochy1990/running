...
| `GET /api/{user}/inbody` | inbody.py | W | InBody 列表 |
| `GET /api/{user}/inbody/summary` | inbody.py | W | InBody 概览 |
| `GET /api/{user}/inbody/{scan_date}` | inbody.py | W | InBody 单条 |
| `POST /api/{user}/inbody` | inbody.py | W | 上传 InBody |
...
| `GET /api/{user}/inbody` | InBody 全列表 + 每次全字段（30+ 字段）。手机端只展示趋势 mini，不展示原始报告。 | 加 `?summary=true` 只返回 `{date, weight, body_fat_pct, skeletal_muscle_mass}`。 |
