...
function resolveCredentialsFile() {
  ...
  try {
    ...
    mainWorktreePath = path.join(path.dirname(path.resolve(repoRoot, commonGitDir)), ".credentials.local");
  } catch {
  ...
  const candidates = [
    ...
    mainWorktreePath,
    process.env.HOME ? path.join(process.env.HOME, "workspace", "running", ".credentials.local") : null,
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, "workspace", "running", ".credentials.local") : null,
  ].filter(Boolean);
  const found = candidates.find((c) => fs.existsSync(c));
  if (!found) {
    throw new Error(".credentials.local not found (tried repoRoot, main worktree, HOME/USERPROFILE/workspace/running, STRIDE_CREDENTIALS_FILE)");
  }
  return found;
}
...
function loadConfig({ requireCoros = true } = {}) {
  ...
  const adminPassword =
    flat.auth_password || (sections.admin || {}).password || (sections.stride || {}).password || process.env.STRIDE_SMOKE_ADMIN_PASSWORD || "";
  ...
  const authBase = (env.VITE_AUTH_BASE_URL || env.VITE_DEV_AUTH_PROXY || process.env.VITE_AUTH_BASE_URL || "").replace(/\/$/, "");
...
function loadLocalCredentials() {
  ...
  if (!credentials) {
    throw new Error(
      ".credentials.local must contain email/password (or user_email/user_password), " + "or set STRIDE_SMOKE_USER_EMAIL / STRIDE_SMOKE_USER_PASSWORD",
    );
  }
...
function verifyCredentialStructure() {
  ...
  try {
    ...
    return {
      ...
      sectionNames: [],
      flatKeys: [],
      hasLocalCreds: Boolean(process.env.STRIDE_SMOKE_USER_EMAIL && process.env.STRIDE_SMOKE_USER_PASSWORD),
    };
  }
...
async function mintInviteCode(cfg) {
  ...
  if (mint.status === 401 || mint.status === 403) {
    throw new Error(
      `mint invite code rejected (HTTP ${mint.status}). The admin account lacks ` + `role=admin, or set a single-use invite_code in .credentials.local.`,
    );
  }
...
async function registerThrowaway(cfg) {
  ...
  const res = await postJson(
    `${cfg.authBase}/api/auth/register`,
    {
      email,
      password,
 ...
 */
async function runOnboardingScenario({ name, screenshot, requireCoros = true, captureConsoleErrors = true }, scenario) {
