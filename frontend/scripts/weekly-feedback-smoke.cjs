...
async function main() {
  ...
  try {
    ...
    const bootstrap = await page.evaluate(async ({ weekName }) => {
      ...
      const created = await request(`${path}/feedback`, {
        method: 'PUT', headers, body: JSON.stringify({ content: 'Local browser smoke', generated_by: 'legacy-client' }),
      })
      const forbidden = await fetch(`/api/00000000-0000-4000-8000-000000000000/weeks/${weekName}/feedback`, {
        method: 'PUT', headers, body: JSON.stringify({ content: 'forbidden' }),
      })
    ...
    const cleared = await page.evaluate(async ({ weekName }) => {
      const token = sessionStorage.getItem('access_token')
      const payload = JSON.parse(atob(token.split('.')[1].replace(/-/g, '+').replace(/_/g, '/')))
      const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }
      const path = `/api/${payload.sub}/weeks/${weekName}`
