...
function extractErrorMessage(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    ...
    return extractErrorMessage(data.error) || extractErrorMessage(data.message) || extractErrorMessage(data.detail) || extractErrorMessage(data.msg);
  }
  return "";
}
...
function PasswordRuleList({ id, rules, visible }: { id: string; rules: PasswordRule[]; visible: boolean }) {
  ...
  return (
    <ul id={id} className="mt-2 space-y-1 text-xs" aria-label="密码规则">
      {rules.map((rule) => (
        <li key={rule.id} className={rule.isValid ? "flex items-center gap-1.5 text-accent-green" : "flex items-center gap-1.5 text-red-400"}>
