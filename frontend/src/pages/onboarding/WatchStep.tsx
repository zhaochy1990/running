...
const PROVIDERS: ProviderMeta[] = [
  {
    ...
    display_name: "COROS",
    caption: "高驰",
    accent_classes: "border-accent-green/40 bg-accent-green/5 hover:border-accent-green hover:bg-accent-green/10",
  ...
  },
  {
    ...
    display_name: "GARMIN",
    caption: "佳明",
    accent_classes: "border-sky-500/40 bg-sky-500/5 hover:border-sky-500 hover:bg-sky-500/10",
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        const fallback = provider === "coros" ? "COROS 账号验证失败，请检查账号和密码" : "佳明账号验证失败，请检查邮箱、密码和区域";
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-text-muted mt-1">选择你日常使用的运动手表，用于同步训练数据</p>
        </div>
          ...
          {PROVIDERS.map((p) => (
            ...
            >
              <div className="flex items-baseline gap-3">
                <span className="font-mono text-base font-bold text-text-primary tracking-wide">{p.display_name}</span>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="text-lg font-bold text-text-primary">连接 {meta!.display_name} 账号</h2>
        <p className="text-sm text-text-muted mt-1">输入你的 {meta!.caption} 账号用于同步训练数据</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-mono text-text-muted uppercase tracking-wider mb-1">{meta!.caption} 密码</label>
          <input
            type="password"
            required
            value={password}
        ...
        {meta!.needs_region && (
          <div>
            <label className="block text-xs font-mono text-text-muted uppercase tracking-wider mb-1">账号区域</label>
            ...
            <p className="mt-1 text-xs text-text-muted">如果你常在 connect.garmin.cn 登录，选"中国"；otherwise 选"国际"。</p>
          </div>
        )}
