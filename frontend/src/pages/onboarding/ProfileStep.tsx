...
export default function ProfileStep({ onSuccess }: Props) {
  ...
  const inputCls = (field?: string) =>
    `w-full rounded-lg border px-3 py-2 text-sm text-text-primary bg-bg-base focus:outline-none focus:ring-1 focus:ring-accent-green ${
      field && fieldErrors[field] ? "border-red-500/60 focus:border-red-500" : "border-border-subtle focus:border-accent-green"
    }`;
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <section className="space-y-4">
          ...
          <div>
            <label htmlFor="onboarding-running-age" className="block text-xs font-mono text-text-muted uppercase tracking-wider mb-1">
              跑龄
            </label>
            <select
