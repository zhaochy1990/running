...
import { getPipelineRun, getJobState, postOnboardingComplete, triggerSync, ApiError, type JobState, type PipelineRun } from "../../api";
...
export default function SubmitStep({ userId }: Props) {
  ...
  const clearStartKey = useCallback(
    (forUserId = userId) => {
  ...
  const refreshRun = useCallback(
    async (runId: string, expectedUserId: string, generation: number) => {
      try {
  ...
  const refreshActiveJob = useCallback(
    async (runId: string, jobId: string, expectedUserId: string, generation: number) => {
      try {
        const job = await getJobState(jobId);
        if (
          !mountedRef.current ||
  ...
  const startNewRun = useCallback(
    async (expectedUserId = userId, generation = generationRef.current) => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-text-primary">{done ? "数据准备完成" : failed ? "数据同步遇到问题" : "正在准备你的训练数据"}</h2>
      ...
      <ol className="space-y-3" aria-label="数据准备进度">
        {STAGES.map((stage, index) => {
          ...
          const progress =
            state === "active" && stageForStep(run?.steps[run.current_step]?.name) === index && activeJob?.stepJobId === currentJobId
...
function StageIcon({ state }: { state: StageState }) {
  ...
  return (
    <span aria-hidden="true" className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-xs ${color}`}>
      {icon}
    </span>
  );
}
