...
import { getHealth, getPMC, getHrv, type HealthRecord, type PMCRecord, type PMCSummary, type HRVSnapshot, type HrvDailyRecord, type HrvSummary } from "../api";
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: -5 }}>
                  ...
                  <Area
                    ...
                    fill="url(#gradRHR)"
                    dot={false}
                    activeDot={{ r: 3, fill: "#00a85a", stroke: "#1e1e2e", strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </ChartCard>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 0, left: -5 }}>
                  ...
                  <ReferenceLine
                    ...
                    strokeDasharray="4 4"
                    strokeOpacity={0.4}
                    label={{ value: "恢复", position: "right", fill: "#00a85a", fontSize: 10, fontFamily: "JetBrains Mono" }}
                  />
                  <ReferenceLine
                    y={50}
                    stroke="#e68a00"
                    strokeDasharray="4 4"
                    strokeOpacity={0.4}
                    label={{ value: "疲劳", position: "right", fill: "#e68a00", fontSize: 10, fontFamily: "JetBrains Mono" }}
                  />
                  <ReferenceLine
                    y={60}
                    stroke="#d32f2f"
                    strokeDasharray="4 4"
                    strokeOpacity={0.4}
                    label={{ value: "高疲劳", position: "right", fill: "#d32f2f", fontSize: 10, fontFamily: "JetBrains Mono" }}
                  />
                  <Area
                    type="monotone"
                    dataKey="fatigue"
                    ...
                    dot={false}
                    activeDot={{ r: 4, fill: "#e68a00", stroke: "#ffffff", strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            </ChartCard>
              ...
              <div className="bg-bg-card border border-border-subtle rounded-2xl p-5">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={hrvChartData} margin={{ top: 5, right: 5, bottom: 0, left: -5 }}>
                    ...
                    <Tooltip {...TOOLTIP_STYLE} formatter={(v: unknown) => (v == null ? ["—", "HRV"] : [`${v} ms`, "HRV"])} />
                    {hrvBalancedLow != null && hrvBalancedHigh != null && (
                      <ReferenceArea
                        y1={hrvBalancedLow}
                        y2={hrvBalancedHigh}
          ...
          {pmcSummary && (
            <div className="mb-6">
              <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
                <MetricCard
                  label="体能"
                  sublabel="CTI (Fitness)"
                  value={pmcSummary.current_cti != null ? `${pmcSummary.current_cti}` : "—"}
                  unit=""
                  color="#00a85a"
                  detail="42天慢性负荷"
                  help={
                    <>
                      <strong>慢性训练负荷</strong>（42天加权平均），代表长期累积的体能基线。{"\n\n"}使用方法：{"\n"}• 缓慢上升（每4周 +8~+12）= 健康进步{"\n"}•
                      水平不动 = 维持期{"\n"}• 下降 = 体能流失，需加量{"\n"}• 上升过快（+10+/周）= 过度积累，易受伤
                    </>
                  }
                />
                <MetricCard
                  label="急性负荷"
                  sublabel="ATI (Acute Load)"
                  value={pmcSummary.current_ati != null ? `${pmcSummary.current_ati}` : "—"}
                  unit=""
                  color="#0097a7"
                  detail="7天急性负荷"
                  help={
                    <>
                      <strong>急性训练负荷</strong>（7天加权平均），代表近期训练应激。{"\n\n"}使用方法：{"\n"}• ATI 高于 CTI → 负荷累积期{"\n"}• ATI 低于 CTI →
                      恢复/减量期{"\n"}• 健康训练应呈波浪穿越 CTI{"\n"}• 单日突增 = 比赛或高质量课
                    </>
                  }
                />
                <MetricCard
                  label="竞技状态"
                  sublabel="TSB (Form)"
                  value={pmcSummary.current_tsb != null ? `${pmcSummary.current_tsb > 0 ? "+" : ""}${pmcSummary.current_tsb}` : "—"}
                  unit=""
                  color={tsbColor(pmcSummary.current_tsb)}
                  detail={tsbZoneLabel(pmcSummary.current_tsb_zone)}
                  help={
                    <>
                      <strong>训练应激平衡 = CTI − ATI</strong>。衡量已从近期训练中恢复多少、是否适合比赛。{"\n\n"}使用方法：{"\n"}• +10~+25 比赛就绪甜区{"\n"}•
                ...
                <MetricCard
                  label="CTL周增量"
                  sublabel="Ramp Rate"
                  value={pmcSummary.ctl_ramp != null ? `${pmcSummary.ctl_ramp > 0 ? "+" : ""}${pmcSummary.ctl_ramp}` : "—"}
                  unit="/周"
                  ...
                  help={
                    <>
                      <strong>CTI 过去7天的变化率</strong>，反映体能增长或衰减速度。{"\n\n"}使用方法：{"\n"}• +3~+7/周 健康递进{"\n"}• 高于 +8/周
                      增长过快，易受伤{"\n"}• 0 附近 维持期{"\n"}• 低于 -8/周 流失过快，需加量{"\n"}• 赛后 -10~-15 属正常
                    </>
                  }
                />
                ...
                <tbody>
                  {records.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 text-right">
                        <span style={{ color: ratioColor(r.training_load_ratio) }}>{r.training_load_ratio?.toFixed(2) ?? "—"}</span>
                      </td>
                      <td className="py-2 px-3 text-right">
                        <span
...
function MetricCards({ latest, hrv, rhrBaseline }: { latest: HealthRecord; hrv: HRVSnapshot | null; rhrBaseline: number | null }) {
  ...
  const hrvColor =
    hrvValue != null && hrvLow != null && hrvHigh != null
      ? hrvValue >= hrvLow && hrvValue <= hrvHigh
        ? "#00a85a"
        : hrvValue > hrvHigh
          ? "#0097a7"
          : "#e68a00"
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
      <MetricCard
        ...
        unit="bpm"
        color={
          latest.rhr == null || rhrBaseline == null
            ? "#8888a0"
            : latest.rhr > rhrBaseline + 8
              ? "#d32f2f"
              : latest.rhr > rhrBaseline + 3
                ? "#ffab00"
                : "#00a85a"
        }
      ...
      <MetricCard
        ...
        help={
          <>
            <strong>疲劳评分</strong>（0-100），综合训练与恢复数据。{"\n\n"}使用方法：{"\n"}• 低于 40 已恢复，可上质量课{"\n"}• 40-50 正常训练{"\n"}• 50-60
            疲劳中，减强度{"\n"}• 高于 60 高疲劳，跳过训练或休息
          </>
        }
      />
      <MetricCard
        ...
        detail={
          latest.training_load_ratio != null
            ? latest.training_load_ratio < 0.7
              ? "偏低"
              : latest.training_load_ratio <= 1.0
                ? "最佳"
                : latest.training_load_ratio <= 1.2
                  ? "偏高"
                  : "过高"
            : ""
        }
        help={
          <>
            <strong>急性 / 慢性负荷比</strong>。反映近期应激相对于基线。{"\n\n"}使用方法：{"\n"}• 0.8-1.1 健康训练区{"\n"}• 高于 1.2 过度应激，减量{"\n"}• 高于
            1.5 极高风险，强制休息{"\n"}• 低于 0.7 流失体能，需加量{"\n"}• 4周块均值 ≈ 1.0 为理想周期化
          </>
        }
      />
      <MetricCard
        ...
        help={
          <>
            <strong>负荷分类标签</strong>（Low / Optimal / High / Very High），由手表按近期训练应激综合打分。{"\n\n"}使用方法：{"\n"}• Optimal 持续期 =
