...
export default function CoachWeeklyPlanPage({ initialTab = "schedule" }: CoachWeeklyPlanPageProps) {
  ...
  const { week, weeks, planDays, strength, structuredStatus, canPushRun, canPushStrength, loading, error, saveFeedback, pushSession } = useCoachWeeklyPlan();
  ...
  if (error)
    return (
      <div role="alert" className="mx-auto mt-10 max-w-xl rounded-xl border border-accent-red/30 bg-red-soft p-4 text-sm text-accent-red">
        {error}
      </div>
    );
  if (!week)
