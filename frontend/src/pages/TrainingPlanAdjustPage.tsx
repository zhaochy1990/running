...
type AdjustmentAssessment = NonNullable<Awaited<ReturnType<typeof sendMasterPlanAdjustMessage>>["data"]["assessment"]>;
...
const DIRECTION_SUGGESTIONS = ["把周跑量降低到 45–50 公里", "把周跑量提高到 55–65 公里", "把训练重点改成马拉松配速耐力", "把这个阶段延长两周"];
...
const EXPLICIT_PHASE_TARGET_RE =
  /基础期|基础阶段|专项期|专项阶段|强化期|高峰期|赛前期|减量期|调整期|恢复期|恢复阶段|当前阶段|现阶段|这个阶段|本阶段|下一阶段|下个阶段|后续阶段|第\s*[一二三四五六七八九十0-9]+\s*(?:个)?阶段|phase[-_ ]?[a-z0-9]+/i;
...
export default function TrainingPlanAdjustPage() {
  ...
  const loadPlan = useCallback(() => {
    ...
    let cancelled = false;
    setLoadingPlan(true);
    Promise.all([getCurrentMasterPlan(user).catch(() => null), getMyProfile().catch(() => null)])
  ...
  const loadScan = useCallback(() => {
    ...
    async function run() {
      ...
      const planDaysP = currentWeek ? getPlanDays(user, currentWeek.date_from, currentWeek.date_to).catch(() => null) : Promise.resolve(null);
      const [activities, health, hrv, pmc, zones, planDays] = await Promise.all([activitiesP, healthP, hrvP, pmcP, zonesP, planDaysP]);
      if (cancelled) return;
      setScan(
        buildScanState({
  ...
  const previewPhases = useMemo(() => applyDiffToDisplayPhases(currentPhases, selectedDiff), [currentPhases, selectedDiff]);
  ...
  const requestAdjustDiff = async (nextAnswers: FlowAnswers) => {
    ...
    try {
      const response = await sendMasterPlanAdjustMessage(masterPlan.plan_id, composeAdjustMessage(nextAnswers), []);
  ...
  const applyDiff = async () => {
    ...
    try {
      ...
      const response = await applyMasterPlanAdjustDiff(masterPlan.plan_id, diff, opIds, composeChangeReason(answers));
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 sm:px-8 sm:py-8 animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-[13px] text-text-secondary mt-1.5 max-w-[620px]">
          先说清楚想调整的方向，必要时再确认阶段。澄清后 STRIDE 才会加载训练数据、判断想法是否合理；只有合理时才生成可应用方案。
        </p>
      </div>
...
function PlanReferencePanel({
  ...
  const previewTarget = useMemo(() => applyDiffToTargetProfile(targetProfile, masterPlan, diff), [targetProfile, masterPlan, diff]);
  return (
    <aside className="pa-current" id="pa-leftplan">
      {previewReady && (
        <div className="pa-planswitch">
          <button type="button" data-plan="cur" className={!isNew ? "active" : ""} onClick={() => onModeChange("current")}>
            当前计划
          </button>
          <button type="button" data-plan="new" className={isNew ? "active" : ""} onClick={() => onModeChange("new")}>
            新计划
          </button>
        </div>
      )}
...
function PlanReferenceView({
  ...
  const totalWeeks = isNew ? (phases.at(-1)?.weekEnd ?? planTotalWeeks(masterPlan, phases)) : planTotalWeeks(masterPlan, phases);
  return (
    <div className="pa-planview">
      <div className="pa-chead">
        <div className={`pa-ctag ${isNew ? "newt" : ""}`}>{isNew ? "新计划 · 已生成" : "当前计划"}</div>
        <div className="pa-ctitle">
          {planReferenceTitle(targetProfile, totalWeeks)}
          {isNew ? "（已调整）" : ""}
        </div>
      ...
      <div className="pa-cbody">
        ...
        <RaceCard changed={Boolean(isNew && targetChanged)} target={targetProfile} masterPlan={masterPlan} />
  ...
  return (
    <div className="pa-changes">
      ...
      <div className="pa-chrow">
        ...
        <div className="tx">
          <b>{summary || "调整方案已通过合理性门槛"}</b>
          <span className="why"> — 仅展示后端返回的结构化 diff</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="pa-race">
      <div className="top">
        ...
        <div className="rd">
          {distance}
          {raceDate ? ` · ${formatSlashDate(raceDate)}` : " · 日期暂无数据"}
          {targetTime}
        </div>
      </div>
      <div className="pa-cdown">
        <div className="c">
          ...
          <div className="u">周 后开赛</div>
        </div>
        <div className="c">
          <div className="n">{days ?? "--"}</div>
          <div className="u">天 倒计时</div>
        </div>
      </div>
    </div>
  );
...
function PlanMileageCurve({ phases, totalWeeks, isNew }: { phases: DisplayPhase[]; totalWeeks: number; isNew: boolean }) {
  ...
  return (
    <div>
      <div className="pa-cl">
        全程周量曲线 · {totalWeeks || bars.length || "--"} 周{" "}
        <span className="sp">
          · {isNew ? "预览调整 · " : ""}
          {peak ? `峰值 ${peak.km}km (W${padWeek(peak.week)})` : "暂无周量数据"}
        </span>
      </div>
      <div className="pa-cbars">
        {bars.length > 0 ? (
          bars.map((bar) => (
            <div
              key={bar.week}
              className={`b p${bar.phaseIndex + 1}${bar.changed ? " cut" : ""}${peak?.week === bar.week ? " peak" : ""}`}
      ...
      <div className="pa-caxis">
        <span>{phases[0]?.start ? `W01 · ${formatMonthDay(phases[0].start)}` : "W01"}</span>
        <span>{phases.at(-1)?.end ? `W${padWeek(totalWeeks || bars.length)} · ${formatMonthDay(phases.at(-1)?.end ?? "")}` : "终点暂无"}</span>
      </div>
    </div>
  );
}
    ...
    <div className={`pa-pd ${isCurrent && !changed ? "now" : ""} ${changed ? "chg" : ""}`.trim()}>
      ...
      <div className="pa-pd-b">
        ...
        <div className="pa-pd-key">
          {phaseKeywords(phase).map((keyword) => (
            <span key={keyword} className="k">
              {keyword}
            </span>
          ))}
        </div>
  ...
  return (
    <section className="pa-panel">
      <div className="pa-phead">
        ...
        <div className={`pa-status ${loading ? "" : "done"}`}>
          {loading ? <span className="pa-spin" /> : <CheckMiniIcon />}
          <span>{loading ? "分析中…" : "已完成"}</span>
        </div>
      </div>
      <div className="pa-pbody pa-scan-body">
        ...
        <div className="pa-zones">
          <ZoneTable title="配速区间" anchor="阈值配速" anchorValue={scan.paceZone || "暂无数据"} unit="区间 /km" rows={scan.paceRows} />
          <ZoneTable title="心率区间" anchor="阈值心率" anchorValue={scan.hrZone || "暂无数据"} unit="区间 bpm" rows={scan.hrRows} />
        </div>
        ...
        <div className="pa-summary">
          ...
          <div className="pa-sstats">
            {scan.rows.slice(0, 3).map((row) => (
              <div key={row.id} className="pa-sstat">
                <div className={`sn ${row.tone === "warn" ? "warn" : ""}`}>{row.value}</div>
                <div className="sd">{scanDesignLabel(row.label)}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <section className="pa-convo">
      ...
      <div className="pa-aq">
        {step === "direction" && (
          <>
            <label htmlFor="adjust-direction" className="aq-q block">
              这次具体想怎么调整训练计划？
            </label>
        ...
        {step === "clarification" && (
          <>
            <label htmlFor="adjust-clarification" className="aq-q block">
              请补充调整细节
            </label>
          ...
          <div className="space-y-3">
            ...
            {assessment && (
              <div
                className={`rounded-xl border p-3 ${
                  assessment.verdict === "reasonable" ? "border-accent-green/25 bg-accent-green/5" : "border-accent-amber/30 bg-accent-amber/5"
                }`}
              >
            ...
            {assessment && assessment.verdict !== "reasonable" && (
              <button
                type="button"
                onClick={onRevise}
                className="inline-flex h-8 items-center rounded-md border border-border px-3 text-xs font-semibold text-text-secondary"
              >
                修改我的想法
              </button>
            )}
          ...
          <div className="rounded-xl border border-accent-amber/30 bg-accent-amber/5 p-3">
            ...
            {hasPlanId && (
              <button
                ...
                className="mt-2 text-xs font-semibold text-accent-amber disabled:opacity-50"
              >
                重试评估
              </button>
            )}
        ...
        {previewReady && (
          <div className="space-y-3">
              ...
              <div className="rounded-xl border border-border-subtle bg-bg-primary p-3">
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <span className="font-mono text-[11px] text-text-muted">
                    {acceptedCount}/{diff.ops.length}
                  </span>
                </div>
                {diff.ai_explanation && <p className="text-xs text-text-secondary mb-3">{diff.ai_explanation}</p>}
                <div className="space-y-2">
                  {diff.ops.map((op) => (
                    <DiffOpRow key={op.id} op={op} checked={selectedOpIds.has(op.id)} onToggle={() => onToggleOp(op.id)} />
                  ))}
                </div>
              </div>
            )}
            ...
            {applyError && <div className="rounded-xl border border-accent-red/30 bg-accent-red/5 p-3 text-xs text-accent-red">采用失败：{applyError}</div>}
              ...
              <div className="rounded-xl border border-accent-green/25 bg-accent-green/5 p-3">
                ...
                <div className="space-y-1">
                  {affectedWeeks.map((week) => (
                    <div key={week.folder} className="font-mono text-xs text-accent-green-dim">
                      {week.folder}
                    </div>
                  ))}
                </div>
        ...
        <div className="aq-foot">
          <span className="note">
            {stageIndex < 2 ? "澄清完成前不读取训练数据、不生成方案" : hasPlanId ? "合理性通过后才可应用调整" : "当前没有可调整的激活计划"}
          </span>
        </div>
      </div>
    </section>
...
function DiffOpRow({ op, checked, onToggle }: { op: MasterPlanDiffOp; checked: boolean; onToggle: () => void }) {
  ...
  return (
    <label
      className={`flex items-start gap-2 rounded-lg border border-border-subtle bg-bg-card p-2.5 ${disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
    >
...
function buildScanState({
  ...
  const currentWeekActivities =
    currentWeek && activities
      ...
      : null;
  const plannedSessions = planDays ? planDays.flatMap((day) => day.sessions).filter((session) => session.kind !== "rest" && session.kind !== "note") : null;
...
function applyDiffToDisplayPhases(phases: DisplayPhase[], diff: MasterPlanDiff | null): DisplayPhase[] {
  ...
  return phases.map((phase) => {
    const directPatches = diff.ops.filter((op) => op.phase_id === phase.id).map((op) => ({ ...(op.new_value ?? {}), ...(op.spec_patch ?? {}) }));
    const atomicPatches = diff.ops.flatMap((op) => {
      ...
      return phaseUpdates.filter((update): update is Record<string, unknown> => isRecord(update) && update.phase_id === phase.id);
    });
    const patches = [...directPatches, ...atomicPatches];
    const next = patches.reduce<DisplayPhase>(
      (current, patch) => ({
        ...current,
        ...
        low: typeof patch.weekly_distance_km_low === "number" ? patch.weekly_distance_km_low : current.low,
        high: typeof patch.weekly_distance_km_high === "number" ? patch.weekly_distance_km_high : current.high,
        changed: true,
      }),
      phase,
    );
...
function applyDiffToTargetProfile(target: TargetProfile, masterPlan: MasterPlan | null, diff: MasterPlanDiff | null): TargetProfile {
...
function targetProfileChanged(current: TargetProfile, next: TargetProfile): boolean {
  return (
    current.raceName !== next.raceName || current.distance !== next.distance || current.raceDate !== next.raceDate || current.targetTime !== next.targetTime
  );
}
...
function buildMileageBars(phases: DisplayPhase[], totalWeeks: number): PlanMileageBar[] {
  const raw = phases
    .flatMap((phase, phaseIndex) =>
      Array.from({ length: phase.weekCount }, (_, localIndex) => {
  ...
  return (
    <div className="pa-zcard">
      <div className="zhead">
        ...
        <div className="zanchor">
          {anchor} <b>{anchorValue}</b>
        </div>
      </div>
        ...
        <tbody>
          {displayRows.map((row) => (
            <tr key={row.zone} className={row.threshold ? "thr" : ""}>
              <td className="zn" style={{ color: row.color }}>
