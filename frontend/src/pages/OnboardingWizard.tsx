    ...
    <div className="flex min-h-screen items-start justify-center bg-bg-base px-4 py-12">
      <div className="w-full max-w-lg">
            ...
            {steps.map((label, index) => {
              ...
              return (
                <li key={label} className="flex min-w-0 items-center gap-2 text-xs">
                  ...
                  <span className={index === stepIndex ? "font-medium text-text-primary" : "text-text-muted"}>{label}</span>
        ...
        <div className="bg-bg-card border border-border-subtle rounded-lg p-8">
          ...
          {step === "watch" && <WatchStep onSuccess={() => setStep("injuries")} />}

          {step === "injuries" && <InjuryStep onSuccess={() => setStep("submit")} />}

          {step === "submit" && profileData && userId && <SubmitStep userId={userId} />}
