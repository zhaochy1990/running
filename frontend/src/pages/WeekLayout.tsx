      ...
      {loadingDetail ? (
        ...
        <div className="animate-fade-in">
          {(() => {
            ...
            return (
              <>
                <ViewHead eyebrow={phase ?? undefined} title={title} />
          ...
          <div className="flex gap-1 p-1 bg-bg-secondary rounded-lg w-fit mb-6">
            ...
            <TabButton active={activeTab === "feedback"} onClick={() => setActiveTab("feedback")} color="cyan">
              本周反馈
              {needsFeedback && <span className="ml-1.5 w-1.5 h-1.5 rounded-full bg-accent-amber inline-block align-middle" />}
            </TabButton>
          </div>
          ...
          {activeTab === "plan" && weekDetail.plan && (
            <div className="space-y-3 animate-fade-in">
              <SelectedVariantBar summary={weekDetail.variants_summary} />
          ...
          {activeTab === "activities" && <ActivityList activities={weekDetail.activities} />}
          {activeTab === "feedback" && (
            <FeedbackPanel
              user={user}
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={`text-sm font-mono font-semibold ${accent ? "text-accent-green" : "text-text-primary"}`}>{value}</span>
    </div>
  );
}
...
function CalendarTab({
  ...
  const showReparse = structuredStatus === "parse_failed" || structuredStatus === "stale" || structuredStatus === "none" || structuredStatus === "backfilled";
      ...
      {showReparse && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {reparseError && <span className="text-[11px] font-mono text-accent-red">{reparseError}</span>}
            <button
              type="button"
              onClick={onReparse}
...
function StrengthTab({ data, loading }: { data: StrengthTabResponse | null; loading: boolean }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="w-6 h-6 border-2 border-accent-green/30 border-t-accent-green rounded-full animate-spin" />
  ...
  if (!data || data.sessions.length === 0) {
    return <div className="text-text-muted text-center py-16 text-sm">本周没有力量训练</div>;
  }
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="border border-accent-red/40 bg-accent-red/10 rounded-xl p-3 sm:p-4">
        <p className="text-xs sm:text-sm text-accent-red leading-relaxed">
          <strong>⚠️ 注意：</strong>
          动作清单优先匹配 COROS 内置动作库（推送到手表后有官方动画指导）， 但本页展示的<strong>动作示意图为 AI 生成</strong>
          ，可能存在解剖错误、姿态偏差或方向反转。 请<strong>仔细鉴别</strong>后再依图执行；以动作要点 /
          发力部位文字描述为准，必要时参考手表内置动画或专业视频。
        </p>
      </div>
      {data.sessions.map((sess) => (
        <div key={`${sess.date}-${sess.session_index}`} className="bg-bg-card border border-border-subtle rounded-2xl p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-xs font-mono text-text-muted">
                {formatDateShort(sess.date)} · {weekdayCN(sess.date)}
              </p>
              <h2 className="text-base font-semibold text-text-primary mt-1">{sess.summary}</h2>
            </div>
            <span className="text-xs font-mono text-accent-green bg-accent-green/10 px-2 py-1 rounded">{sess.exercises.length} 个动作</span>
          </div>

          {sess.exercises.length === 0 ? (
            <div className="text-text-muted text-sm py-2">（仅有总览，无具体动作清单）</div>
          ) : (
...
function StrengthExerciseCard({ exercise }: { exercise: import("../types/strength").StrengthTabExercise }) {
  const { display_name, sets, target_kind, target_value, rest_seconds, image_url, key_points, muscle_focus, common_mistakes, note } = exercise;
  ...
  return (
    <div className="border border-border-subtle rounded-xl p-3 sm:p-4 bg-bg-secondary/40">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span className="text-xs font-mono text-accent-green whitespace-nowrap">{setLine}</span>
      </div>
      {image_url && (
        <div className="mt-3 w-full max-w-md mx-auto">
          <img src={image_url} alt={display_name} loading="lazy" className="w-full h-auto rounded-lg bg-white object-contain border border-border-subtle" />
        </div>
      )}
      {note && <p className="mt-3 text-xs font-mono text-text-muted">{note}</p>}
      {key_points.length > 0 && <DetailBlock title="动作要点" items={key_points} />}
      {muscle_focus.length > 0 && <DetailBlock title="发力部位" items={muscle_focus} inline />}
      {common_mistakes.length > 0 && <DetailBlock title="常见错误" items={common_mistakes} accent="red" />}
    </div>
  );
}

function DetailBlock({ title, items, inline = false, accent }: { title: string; items: string[]; inline?: boolean; accent?: "red" }) {
  const titleColor = accent === "red" ? "text-accent-red" : "text-text-secondary";
  return (
    <div className="mt-3">
      <p className={`text-[11px] font-mono tracking-wider mb-1 ${titleColor}`}>{title}</p>
      {inline ? (
        <div className="flex flex-wrap gap-1.5">
          {items.map((m, i) => (
            <span key={i} className="text-xs font-mono bg-accent-cyan/10 text-accent-cyan px-2 py-0.5 rounded">
              {m}
            </span>
          ))}
        </div>
...
function TabButton({ active, onClick, color, children }: { active: boolean; onClick: () => void; color: "green" | "cyan"; children: React.ReactNode }) {
  const activeClass = color === "cyan" ? "bg-accent-cyan/15 text-accent-cyan" : "bg-accent-green/15 text-accent-green";
  return (
    <button
      onClick={onClick}
      className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all ${active ? activeClass : "text-text-muted hover:text-text-secondary"}`}
    >
      {children}
    </button>
  );
    ...
    <div className="bg-bg-card border border-border-subtle rounded-2xl p-4 sm:p-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          {!isEmpty && <span className="text-[11px] font-mono text-text-muted bg-bg-secondary px-2 py-0.5 rounded">已保存</span>}
          {updatedAt && <span className="text-[11px] font-mono text-text-muted">{updatedAt.replace("T", " ").slice(0, 19)}</span>}
        </div>
        <div className="flex items-center gap-2">
          {!editing ? (
            <button
      ...
      {error && <div className="mb-3 px-3 py-2 rounded border border-accent-red/30 bg-accent-red/5 text-xs text-accent-red font-mono">{error}</div>}
      ...
      {editing ? (
        <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
          ...
          <div className="border border-border-subtle rounded-lg p-4 bg-bg-card overflow-auto min-h-[400px]">
            ...
            <div className="prose max-w-none">
              {draft.trim() ? <ReactMarkdown remarkPlugins={[remarkGfm]}>{draft}</ReactMarkdown> : <p className="text-text-muted text-sm">暂无内容</p>}
            </div>
          </div>
        </div>
      ) : isEmpty ? (
        activityCount > 0 ? (
          <div className="space-y-5 py-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-text-muted">{weekEnded ? "反馈帮助 AI 更好地规划下周训练" : "记录训练感受，帮助调整后续安排"}</p>
            </div>
            ...
            <div className="space-y-1">
              {activities.slice(0, 7).map((a) => (
                <div key={a.label_id} className="flex items-center gap-3 px-3 py-1.5 text-xs font-mono rounded-lg hover:bg-bg-secondary/50">
                  <span className="text-text-muted w-20 flex-shrink-0">
                    {shanghaiMonthDay(a.date)} {weekdayCN(a.date)}
                  </span>
              ...
              {activities.length > 7 && <p className="text-xs text-text-muted text-center pt-1">...及其他 {activities.length - 7} 次训练</p>}
            </div>
          ...
          <div className="text-text-muted text-center py-12 text-sm">本周暂无训练记录</div>
        )
      ) : (
        <div className="prose max-w-none">
  ...
  return (
    <div className="space-y-4">
      {Array.from(byDate.entries()).map(([date, acts], gi) => (
        <div key={date} className="animate-fade-in opacity-0" style={{ animationDelay: `${gi * 60}ms`, animationFillMode: "forwards" }}>
              ...
              <Link key={a.label_id} to={`/activity/${a.label_id}`}>
                <div className="group bg-bg-card border border-border-subtle rounded-xl px-4 sm:px-5 py-3.5 sm:py-4 hover:bg-bg-card-hover hover:border-border transition-all duration-200 cursor-pointer">
                  <div className="flex items-stretch gap-3 sm:gap-4">
                    <RouteThumbnail polyline={a.route_thumb} sportName={a.sport_name} size={56} color={sportColor(a.sport_name)} />

                    <div className="flex-1 min-w-0">
                      {/* Name + tags row — desktop also has metrics inline */}
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div className="flex-1 min-w-0 sm:flex-none sm:min-w-[150px]">
                          <p className="text-sm font-medium text-text-primary truncate">{a.name || sportNameCN(a.sport_name)}</p>
                        ...
                        <div className="text-text-muted group-hover:text-accent-green transition-colors text-sm flex-shrink-0">&rsaquo;</div>
                      </div>
...
function Metric({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
      ...
      <p className={`text-sm font-mono font-medium mt-0.5 ${accent ? "text-text-primary" : "text-text-secondary"}`}>{value}</p>
    </div>
  );
}

function SelectedVariantBar({ summary }: { summary?: WeekDetail["variants_summary"] }) {
  if (!summary || summary.selected_variant_id == null) return null;
  const idx = summary.selected_variant_id != null ? (summary.model_ids.length === 1 ? summary.model_ids[0] : "") : "";
  return (
    <div
...
function AbandonedBanner({ abandoned }: { abandoned?: WeekDetail["abandoned_scheduled_workouts"] }) {
  ...
  return (
    ...
    >
      <div className="font-semibold mb-1">⚠️ {abandoned.length} 条已推送训练在新计划中没有对应</div>
