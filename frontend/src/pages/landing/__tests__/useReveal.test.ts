...
describe("useReveal", () => {
  ...
  it("observes the attached element and reveals it on intersect", () => {
    ...
    last!.cb([{ isIntersecting: true, target: el } as unknown as IntersectionObserverEntry], last! as unknown as IntersectionObserver);
  ...
  it('does not add "in" when the element is not intersecting', () => {
    ...
    last!.cb([{ isIntersecting: false, target: el } as unknown as IntersectionObserverEntry], last! as unknown as IntersectionObserver);
...
describe("useCountUp", () => {
  ...
  it("starts at zero with decimals when decimals provided", () => {
    const { result } = renderHook(() => useCountUp(4.9, { decimals: 1, start: false }));
