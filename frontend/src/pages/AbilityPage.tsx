...
export default function AbilityPage() {
  ...
  const primaryEstimates: RaceEstimates = isHM ? (current?.half_marathon_estimates ?? EMPTY_ESTIMATES) : (current?.marathon_estimates ?? EMPTY_ESTIMATES);
  const secondaryEstimates: RaceEstimates = isHM ? (current?.marathon_estimates ?? EMPTY_ESTIMATES) : (current?.half_marathon_estimates ?? EMPTY_ESTIMATES);
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 sm:px-8 sm:py-8">
      {loading ? (
        ...
        <div className="animate-fade-in">
          <ViewHead eyebrow="训练能力 · 90 天数据" title="你的跑步能力画像" lede="基于过去 90 天的训练数据 + 训练负荷计算" />
          ...
          {secondaryEstimates.race_s != null && (
            <div className="mb-6">
              <p className="text-xs font-mono text-text-muted tracking-widest mb-3 uppercase">{secondaryTag} Race Estimate</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            ...
            <Vo2maxPanel vo2max={current.l3_dimensions.vo2max} dataSource={current.source} onRefresh={handleRefreshVo2max} refreshing={refreshing} />
          </div>
          ...
          <div className="mb-6">
            {backfilling ? (
              <div className="bg-bg-card border border-border-subtle rounded-2xl p-8 text-center animate-fade-in">
                ...
                <p className="text-xs font-mono text-text-muted">首次访问 — 正在回填 180 天成绩历史…</p>
              </div>
            ) : (
