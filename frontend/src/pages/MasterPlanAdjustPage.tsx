...
import { abandonCoachProposal, applyCoachMasterProposal, getMasterPlanById, type CoachApplyOutcome } from "../api";
...
import type { ApplyProposalRequest, MasterDiffProposal, ProposalTargetKey, StashedProposal } from "../components/coach-workspace/types";
...
export function MasterPlanAdjustView({
  ...
  const target = useMemo<ProposalTargetKey>(() => ({ userId, kind: "master", planId }), [userId, planId]);
  ...
  const summaryText = summaryLoading ? "加载当前赛季计划…" : summaryError ? `无法加载赛季计划：${summaryError}` : currentPlanSummary;

  if (!stashed) {
    return <PlanAdjustIntakeWorkspace kind="master" currentPlanSummary={summaryText} chat={renderChat(contextAnchor)} />;
  }
...
export default function MasterPlanAdjustPage() {
  ...
  return (
    <MasterPlanAdjustView
      ...
      summaryError={error}
      renderChat={(anchor) => <CoachChat contextAnchor={anchor} target={{ kind: "master", plan_id: planId }} />}
    />
  );
}
