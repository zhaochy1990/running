...
export default function TeamDetailPage() {
  ...
  const loadAll = () => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([getTeam(id), getTeamMembers(id), getTeamFeed(id, 30), listMyTeams()])
      .then(([t, m, f, mine]) => {
        setTeam(t);
  ...
  const allSelected = selectedUserIds !== null && members.length > 0 && members.every((m) => selectedUserIds.has(m.user_id));
  ...
  const filteredFeed = selectedUserIds === null ? feed : feed.filter((a) => selectedUserIds.has(a.user_id));

  const isOwner = Boolean(currentUserId && (team?.owner_user_id === currentUserId || members.some((m) => m.user_id === currentUserId && m.role === "owner")));
  ...
  if (!team) {
    return (
      <div className="max-w-5xl mx-auto px-8 py-20 text-center">
        ...
        <button onClick={() => navigate("/teams")} className="mt-4 text-sm text-accent-red hover:underline">
          返回团队列表
        </button>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 sm:px-8 sm:py-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          {team.description && <p className="text-sm text-text-secondary mt-2 max-w-2xl">{team.description}</p>}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-medium rounded-lg border transition-all disabled:opacity-50 ${
              isMember ? "border-border text-text-muted hover:bg-bg-card" : "border-accent-red/40 text-accent-red hover:bg-accent-red/10"
            }`}
          >
        ...
        {(["feed", "mileage"] as TeamTab[]).map((tab) => (
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
              activeTab === tab ? "text-accent-red bg-accent-red/10" : "text-text-muted hover:text-text-secondary hover:bg-bg-card"
            }`}
      ...
      {activeTab === "mileage" && id && <MileageLeaderboard teamId={id} />}
      ...
      {activeTab === "feed" && (
        <div className="grid gap-8 grid-cols-1 lg:grid-cols-[2fr_1fr]">
          ...
          <section>
            ...
            {feed.length === 0 ? (
              <div className="px-4 py-8 rounded-lg border border-border-subtle text-center text-sm text-text-muted font-mono">暂无动态</div>
              ...
              <div className="space-y-2">
                {filteredFeed.map((act) => {
                  const myMember = currentUserId ? members.find((m) => m.user_id === currentUserId) : null;
                  const myDisplayName = myMember?.display_name || myMember?.name || "你";
                  return (
                    <div
                    ...
                    >
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <RouteThumbnail polyline={act.route_thumb} sportName={act.sport_name} size={56} />
                          <div className="flex-1 min-w-0">
                            ...
                            {act.name && <p className="text-xs text-text-muted mt-2 line-clamp-1">{act.name}</p>}
                          </div>
                        </div>
                      </button>
                      {id && act.label_id && (
                        <div className="px-4 pb-3">
                          <LikeButton
                            ...
                            initialTopLikers={act.top_likers ?? []}
          ...
          {/* Sync summary modal */}
          {syncSummary && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => setSyncSummary(null)}>
              <div
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button onClick={() => setSyncSummary(null)} className="text-text-muted hover:text-text-primary text-xl leading-none px-2" aria-label="关闭">
                    ×
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 text-xs font-mono">
                  <div className="bg-bg-secondary rounded-lg p-3">
                    ...
                    <div className="text-lg text-accent-green mt-1">{syncSummary.totals.new_activities}</div>
                  </div>
                  <div className="bg-bg-secondary rounded-lg p-3">
                    <div className="text-text-muted">新增健康记录</div>
                    <div className="text-lg text-accent-green mt-1">{syncSummary.totals.new_health}</div>
                  </div>
                </div>

                <div className="space-y-2">
                  {syncSummary.results.map((r) => (
                    <div key={r.user_id} className="px-3 py-2 rounded-lg border border-border-subtle bg-bg-secondary text-sm">
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-text-primary truncate">{r.display_name}</span>
                        <span
                        ...
                        >
                          {r.status === "synced" ? "已同步" : r.status === "skipped_no_auth" ? "未登录手表" : "失败"}
                        </span>
                      </div>
                      ...
                      {r.status === "error" && r.error && <div className="text-xs font-mono text-accent-red mt-1 break-all">{r.error}</div>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xs font-mono text-text-muted tracking-wider">成员 ({members.length})</h2>
              {members.length > 0 && (
                <button onClick={toggleSelectAll} className="text-[11px] font-mono text-text-muted hover:text-text-secondary transition-colors">
                  {allSelected ? "全不选" : "全选"}
                </button>
              )}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg border transition-all ${
                      selected ? "border-accent-red/40 bg-accent-red/5 hover:bg-accent-red/10" : "border-border-subtle bg-bg-card opacity-50 hover:opacity-80"
                    }`}
                      ...
                      <span className="flex items-center gap-2 min-w-0">
                        <span
                          className={`inline-flex items-center justify-center w-4 h-4 rounded border flex-shrink-0 ${
                            selected ? "bg-accent-red border-accent-red text-white" : "border-border bg-bg-card"
                          }`}
                          aria-hidden
                        >
                          {selected && (
                        ...
                        <span className="text-sm font-medium text-text-primary truncate">{m.display_name || m.name || m.user_id.slice(0, 8)}</span>
                      </span>
                      {m.role === "owner" && (
                        <span className="text-[10px] font-mono text-accent-red bg-accent-red/10 px-1.5 py-0.5 rounded flex-shrink-0">OWNER</span>
                      )}
                    </div>
                  </button>
                );
            ...
            {isOwner && (
              <div className="mt-6 rounded-xl border border-red-500/30 bg-red-500/5 p-4">
                <h3 className="text-xs font-mono text-red-400 tracking-wider mb-2">队长操作</h3>
                <p className="text-xs text-text-muted mb-3">注销账号前，需要先转让或解散你拥有的团队。</p>

                <label className="block text-[11px] font-mono text-text-muted mb-1">转让队长给成员</label>
                ...
                {transferCandidates.length === 0 && <p className="mt-2 text-[11px] text-text-muted">当前没有其他成员可接任队长。</p>}

                <div className="mt-4 border-t border-red-500/20 pt-4">
                  <label className="block text-[11px] font-mono text-text-muted mb-1">输入团队名称确认解散</label>
                  <input
                    type="text"
