...
import { abandonCoachProposal, applyCoachWeekProposal, formatWeekRange, getPlanDays, getWeek, type CoachApplyOutcome, type PlanDay } from "../api";
...
import type { ApplyProposalRequest, ProposalTargetKey, StashedProposal, WeeklyProposal } from "../components/coach-workspace/types";
import type { CoachReviewContext } from "../types/coachChat";
import { clearStashedProposal, readStashedProposal } from "../lib/coachProposalStorage";
...
export interface WeeklyPlanAdjustPageDeps {
  ...
  readonly abandon: (target: { kind: "weekly" | "master"; folder?: string; planId?: string }) => Promise<boolean | void>;
  ...
  readonly renderChat: (contextAnchor: string, reviewContext?: CoachReviewContext) => React.ReactNode;
  /**
...
export function WeeklyPlanAdjustView({
  ...
  const target = useMemo<ProposalTargetKey>(() => ({ userId, kind: "weekly", folder }), [userId, folder]);
  ...
  const summaryText = summaryLoading ? "加载当前计划…" : summaryError ? `无法加载当前计划：${summaryError}` : currentPlanSummary;
...
export default function WeeklyPlanAdjustPage() {
  ...
  const currentPlanDetail = useMemo<React.ReactNode>(() => {
    ...
    return <CreateReview days={projection.days} strength={projection.strength} nutrition={projection.nutrition} notesMd={projection.notesMd} />;
  ...
  const refreshToken =
    typeof location.state === "object" && location.state !== null ? (location.state as { draftRevision?: string | number }).draftRevision : undefined;
  ...
  return (
    <WeeklyPlanAdjustView
      ...
      renderChat={(anchor, reviewContext) => <CoachChat contextAnchor={anchor} target={{ kind: "week", folder }} reviewContext={reviewContext} />}
    />
  );
}
