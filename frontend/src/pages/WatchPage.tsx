...
import { disconnectWatch, getWatchInfo, postWatchLogin, type WatchInfo } from "../api";
    ...
    <div className={embedded ? "" : "max-w-3xl mx-auto px-4 py-6 sm:px-8 sm:py-8"}>
      {!embedded && (
        <>
          ...
          <div className="mb-8">
            ...
            <p className="text-sm font-mono text-text-muted mt-1">管理你的运动手表绑定</p>
          </div>
        </>
      )}

      {error && <div className="mb-4 rounded-lg bg-red-500/10 border border-red-500/20 px-3 py-2 text-sm text-red-400">{error}</div>}
      {success && <div className="mb-4 rounded-lg bg-accent-green/10 border border-accent-green/30 px-3 py-2 text-sm text-accent-green">{success}</div>}

      {/* Current watch info */}
      <section className="rounded-2xl border border-border-subtle bg-bg-card p-5 mb-6">
        <h3 className="text-xs font-mono text-text-muted uppercase tracking-wider border-b border-border-subtle pb-2 mb-4">当前绑定</h3>
        ...
        {isConnected ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-text-primary">{watch.provider_display_name || watch.provider}</div>
                <div className="text-xs font-mono text-text-muted">{watch.email || "—"}</div>
              </div>
              ...
              <div className="pt-2 border-t border-border-subtle">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1.5">
                  {watch.capabilities.map((cap) => (
                    <span key={cap} className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-bg-base border border-border-subtle text-text-muted">
                      {cap.replace(/_/g, " ")}
                    </span>
                  ))}
                </div>
          ...
          <div className="text-center py-6">
            ...
            <p className="text-sm text-text-muted font-mono">未绑定手表</p>
            <p className="text-xs text-text-muted font-mono mt-1">绑定手表后可同步训练数据</p>
          </div>
        )}
      </section>
      ...
      {isConnected ? (
        <section className="rounded-2xl border border-red-500/30 bg-red-500/5 p-5">
          ...
          <p className="text-xs text-text-muted mb-3">解除绑定将清除手表登录凭据。已同步的训练数据不会被删除，但无法再同步新数据。</p>
        ...
        <section className="rounded-2xl border border-border-subtle bg-bg-card p-5">
          <h3 className="text-xs font-mono text-text-muted uppercase tracking-wider border-b border-border-subtle pb-2 mb-4">绑定手表</h3>
          ...
          {connectProvider === null ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-text-primary">{connectProvider === "coros" ? "COROS 高驰" : "Garmin 佳明"}</span>
                <button
                  onClick={() => {
                    setConnectProvider(null);
                    setError("");
              ...
              {connectProvider === "garmin" && (
                <select value={region} onChange={(e) => setRegion(e.target.value as "cn" | "global")} className={inputCls}>
