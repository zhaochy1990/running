...
describe("ActivityHeatmap", () => {
  ...
  it("renders 112 cell rects across 16 weeks × 7 days", () => {
    const { container } = render(<ActivityHeatmap weeks={16} series={[]} activitiesByDate={new Map()} />);
  ...
  it("colors cells by dose bucket", () => {
    ...
    const { container } = render(<ActivityHeatmap weeks={16} series={series} activitiesByDate={new Map()} />);
    // Pull cells indexed by data-date attribute.
    const cellAt = (date: string) => container.querySelector(`rect.heatmap-cell[data-date="${date}"]`);
    ...
    const series: any[] = [
      {
        date: "2026-05-20",
        training_dose: 0,
        coverage_status: "rest_confirmed",
        algorithm_version: 1,
        acute_load: 0,
        chronic_load: 0,
        form: 0,
