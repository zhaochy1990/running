...
describe('TrainingPlanSetup polling', () => {
  ...
  it('invalidates a timed-out sync so a late done response cannot start generation', async () => {
    ...
    const oldRun = new Promise<ReturnType<typeof run>>((resolve) => { resolveOldRun = resolve })
  ...
  it('uses a fresh idempotency key and run on retry', async () => {
    ...
    api.getPipelineRun
      .mockResolvedValueOnce(run('run-1', 'failed'))
      .mockResolvedValue(run('run-2', 'running'))
  ...
  it('only starts generation for a done sync response', async () => {
    api.getPipelineRun
      ...
      .mockResolvedValueOnce(run('run-1', 'done'))
  ...
  it('ignores a stale generation response after timeout and retry', async () => {
    ...
    const oldJob = new Promise<ReturnType<typeof job>>((resolve) => { resolveOldJob = resolve })
    api.getMasterPlanJob
      .mockImplementationOnce(() => oldJob)
