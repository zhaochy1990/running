...
import type { StashedProposal, WeeklyCreateProposal, WeeklyDiffProposal, WeeklyProposal } from "../../components/coach-workspace/types";
...
function renderView(stashed: StashedProposal<WeeklyProposal> | null, applyImpl?: () => Promise<CoachApplyOutcome>): Deps {
  ...
  const renderChat = vi.fn((anchor: string, reviewContext?: CoachReviewContext) => (
    <div data-testid="chat" data-anchor={anchor} data-has-review={reviewContext ? "yes" : "no"}>
      chat
    </div>
  ));
  render(
...
describe("WeeklyPlanAdjustView", () => {
  ...
  it("surfaces the season-impact gate on a needs_ack outcome", async () => {
    ...
    const { apply } = renderView(stash(), async () => {
      call += 1;
      return call === 1 ? { status: "needs_ack", seasonImpact: "会挤压下周长距离" } : { status: "ok" };
    });
    fireEvent.click(screen.getByRole("button", { name: "启用计划" }));
  ...
  it("re-reads the stash and swaps the Review when the refresh token changes", () => {
    ...
    const readStash = vi.fn<() => StashedProposal<WeeklyProposal> | null>().mockReturnValueOnce(firstStash).mockReturnValue(revisedStash);
    const noop = vi.fn();
  ...
  it("refreshed weekly-create stash: old calendar disappears, new content appears; nutrition/notes preserved; renderChat receives revised raw; apply submits revised raw", async () => {
    ...
    const readStash = vi.fn<() => StashedProposal<WeeklyProposal> | null>().mockReturnValueOnce(oldCreateStash).mockReturnValue(revisedCreateStash);
    const noop = vi.fn();
    const apply = vi.fn<
      (
        folder: string,
