...
import { getHealth, getHrv, getPMC, getPlanDays, getWeeks } from "../../api";
...
describe("HealthPage", () => {
  ...
  it("preserves prior state and logs warning when getHrv rejects", async () => {
    ...
    await screen.findByText("负荷曲线与恢复状态");
    expect(screen.queryByText("HRV 趋势")).not.toBeInTheDocument();
    expect(consoleWarn).toHaveBeenCalledWith("[HealthPage] HRV fetch failed; preserving prior state", expect.any(Error));
