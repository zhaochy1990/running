...
import type { MasterDiffProposal, StashedProposal } from "../../components/coach-workspace/types";
...
function renderView(stashed: StashedProposal<MasterDiffProposal> | null, applyImpl?: () => Promise<CoachApplyOutcome>) {
  const readStash = vi.fn(() => stashed);
  const clearStash = vi.fn();
  const apply = vi.fn<
    (planId: string, rawProposal: Readonly<Record<string, unknown>>, opIds: readonly string[], baseRevision: string) => Promise<CoachApplyOutcome>
  ...
  render(
    <MasterPlanAdjustView
      ...
      renderChat={(anchor) => (
        <div data-testid="chat" data-anchor={anchor}>
          chat
        </div>
      )}
    />,
