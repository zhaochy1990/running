...
async function chooseDirectionAndPhase(direction = "把周跑量降低到 45–50 公里", phase = "基础期") {
...
describe("TrainingPlanAdjustPage", () => {
  beforeEach(() => {
    ...
    vi.mocked(getStrideZones).mockResolvedValue({
      ...
      hr_zones: [{ name: "Z2", label: "有氧", lower_bpm: 135, upper_bpm: 150 }],
    });
  ...
  it("skips the phase question when the adjustment does not target a phase", async () => {
    ...
    await waitFor(() => {
      expect(sendMasterPlanAdjustMessage).toHaveBeenCalledWith("plan-1", direction, []);
    });
  ...
  it("keeps scan data locked when Coach asks for missing increase details", async () => {
    ...
    await waitFor(() => {
      expect(sendMasterPlanAdjustMessage).toHaveBeenLastCalledWith("plan-1", "我想要加量；专项期增加到 82–96 公里", []);
    });
  ...
  it("loads scan data only after the target phase is selected", async () => {
    ...
    expect(sendMasterPlanAdjustMessage).toHaveBeenCalledWith("plan-1", "基础期：把周跑量降低到 45–50 公里", []);
  ...
  it("previews and labels an atomic race-date proposal that only returns spec_patch", async () => {
    ...
    expect(
      screen.getByText('{"race_date":"2026-10-11"} -> {"race_date":"2026-11-08","phase_updates":[{"phase_id":"phase-2","end_date":"2026-11-08"}]}'),
    ).toBeInTheDocument();
  ...
  it("retries a failed assessment without exposing a local fake preview", async () => {
    vi.mocked(sendMasterPlanAdjustMessage).mockRejectedValueOnce(new Error("service unavailable")).mockResolvedValueOnce(proposalResponse());
