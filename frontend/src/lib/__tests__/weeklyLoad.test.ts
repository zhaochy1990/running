...
function rec(date: string, dose: number | null, coverage_status: string = "complete"): DailyDoseRow {
...
describe("aggregateWeeklyDose", () => {
  ...
  it("sums doses falling in the same Shanghai week", () => {
    const out = aggregateWeeklyDose([rec("2026-05-20", 30), rec("2026-05-21", 45), rec("2026-05-22", 60)], TODAY);
  ...
  it("handles UTC ISO dates that cross the Shanghai day boundary", () => {
    ...
    const out = aggregateWeeklyDose([rec("2026-05-24T17:00:00+00:00", 200)], TODAY);
    expect(out[7].totalDose).toBeNull();
  });
  ...
  it("counts only days with positive dose as active", () => {
    const out = aggregateWeeklyDose([rec("2026-05-19", 0), rec("2026-05-20", 25), rec("2026-05-21", 0)], TODAY);
    expect(out[7].totalDose).toBeCloseTo(25);
    expect(out[7].activeDays).toBe(1);
  });

  it("treats explicit rest rows as observed zero-load days, not coverage gaps", () => {
    const out = aggregateWeeklyDose([rec("2026-05-19", 0, "rest_confirmed"), rec("2026-05-20", 50)], TODAY);
  ...
  it("renders a week containing an unknown placeholder as a gap", () => {
    const out = aggregateWeeklyDose([rec("2026-05-19", 50), rec("2026-05-20", 0, "unknown")], TODAY);
