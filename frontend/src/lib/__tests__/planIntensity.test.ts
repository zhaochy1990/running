...
import { classifySessionByText, classifyStep, computeWeekPlanIntensity } from "../planIntensity";
import type { NormalizedRunWorkout, PlannedSession, WorkoutStep } from "../../types/plan";
...
describe("classifyStep", () => {
  ...
  it("hr_bpm classifies by avg", () => {
    ...
    expect(classifyStep(step({ target: { kind: "hr_bpm", low: 130, high: 140 } }))).toBe("low");
    // Between 155 and 160 → mid (the narrow Z3 band)
    expect(classifyStep(step({ target: { kind: "hr_bpm", low: 155, high: 159 } }))).toBe("mid");
    // ≥ HIGH_MIN 160 → high (tempo/threshold)
    expect(classifyStep(step({ target: { kind: "hr_bpm", low: 158, high: 162 } }))).toBe("high");
    expect(classifyStep(step({ target: { kind: "hr_bpm", low: 170, high: 180 } }))).toBe("high");
  });

  it("pace_s_km classifies by avg (lower s/km = faster = higher)", () => {
    // 5:00/km avg → low
    expect(classifyStep(step({ target: { kind: "pace_s_km", low: 290, high: 310 } }))).toBe("low");
    // 4:20/km avg → mid (between HIGH_MAX 250 and LOW_MIN 270)
    expect(classifyStep(step({ target: { kind: "pace_s_km", low: 255, high: 265 } }))).toBe("mid");
    // 4:07/km avg → high (tempo, ≤ 250 s/km)
    expect(classifyStep(step({ target: { kind: "pace_s_km", low: 245, high: 250 } }))).toBe("high");
    // 3:45/km avg → high
    expect(classifyStep(step({ target: { kind: "pace_s_km", low: 220, high: 230 } }))).toBe("high");
  });
...
describe("computeWeekPlanIntensity", () => {
  ...
  it("high reflects work-step distance exactly; rest goes to low remainder", () => {
    ...
    const r = computeWeekPlanIntensity([makeRunSession({ spec, total_distance_m: 6000 })]);
  ...
  it("time-based work step is estimated via time / pace target", () => {
    ...
    const r = computeWeekPlanIntensity([makeRunSession({ spec, total_distance_m: 5000 })]);
