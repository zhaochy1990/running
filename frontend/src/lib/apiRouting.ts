...
function resolveUpstream(method: string, pathname: string, routes: StrideRoute[], authPrefix: string): Upstream {
...
export function apiUrl(method: string, fullPath: string): string {
  ...
  const upstream = resolveUpstream(method, pathname, cfg.routes ?? [], cfg.authPrefix ?? DEFAULT_AUTH_PREFIX);
