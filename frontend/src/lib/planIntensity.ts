...
const HIGH_KEYWORDS = ["间歇", "interval", "冲刺", "sprint", "阈", "threshold", "vo2", "高强度", "5km配速", "10km配速"];
const LOW_KEYWORDS = ["恢复", "recovery", "轻松", "easy", "长距离", "long run", "base", "有氧", "aerobic", "慢跑"];

export function classifyStep(step: WorkoutStep): IntensityTier {
  if (step.step_kind === "warmup" || step.step_kind === "cooldown" || step.step_kind === "recovery") {
    return "low";
  }
 ...
 */
function breakdownFromSpec(spec: NormalizedRunWorkout, sessionTotalKm: number): SessionBreakdown {
  let high = 0;
  let mid = 0;
...
export function computeWeekPlanIntensity(sessions: PlannedSession[]): PlanIntensity {
  let total = 0;
  let low = 0;
  let mid = 0;
  let high = 0;
  ...
  for (const s of sessions) {
    ...
    const spec = s.spec && s.spec.schema === "run-workout/v1" ? (s.spec as NormalizedRunWorkout) : null;
    if (spec) {
