...
export function findCurrentWeek<T extends { readonly date_from: string; readonly date_to: string }>(weeks: readonly T[], today: string): T | null {
...
export function actualRunDistanceKm(activities: readonly Activity[]): number {
  return activities.filter(isRunActivity).reduce((total, activity) => total + (activity.distance_km ?? 0), 0);
}
...
export function actualStrengthStats(activities: readonly Activity[]): ActualStrengthStats {
  return activities.reduce<ActualStrengthStats>(
    (stats, activity) => {
...
export function pushableSessionsFor(sessions: readonly PlannedSession[], canPushRun: boolean, canPushStrength: boolean): PlannedSession[] {
