...
describe("useCoachChat", () => {
  ...
  it("optimistically appends the user message before the API resolves", async () => {
    ...
    apiMocks.sendCoachChatMessage.mockReturnValue(
      new Promise((res) => {
        resolveApi = res;
      }),
    );
    ...
    act(() => {
      result.current.sendMessage("我最近练得怎么样？");
    });
    ...
    act(() => {
      resolveApi(makeSuccessResp("ok"));
    });
    await waitFor(() => expect(result.current.loading).toBe(false));
  });

  it("appends the coach reply and clears loading on API success", async () => {
    apiMocks.sendCoachChatMessage.mockResolvedValueOnce(makeSuccessResp("### 结论\n你的跑量稳定。"));
    const { result } = renderHook(() => useCoachChat());
  ...
  it("retry() re-sends the last message and clears the error", async () => {
    apiMocks.sendCoachChatMessage.mockRejectedValueOnce(new Error("网络错误")).mockResolvedValueOnce(makeSuccessResp("重试后的回复"));
    const { result } = renderHook(() => useCoachChat());
  ...
  it("does not call the API or persist a turn when the message exceeds the limit", async () => {
    ...
    await act(async () => {
      result.current.sendMessage("x".repeat(8001));
    });
  ...
  it("ignores a second sendMessage call while the first is still pending", async () => {
    ...
    act(() => {
      resolveFirst(makeSuccessResp("回复"));
    });
    await waitFor(() => expect(result.current.loading).toBe(false));
  });
  ...
  it("passes the review context on send and reuses the same snapshot on retry", async () => {
    ...
    apiMocks.sendCoachChatMessage.mockRejectedValueOnce(new Error("网络错误")).mockResolvedValueOnce(makeSuccessResp("草案逻辑"));
    ...
    await act(async () => {
      result.current.sendMessage("这个课表的训练逻辑是什么");
    });
    await waitFor(() => expect(result.current.error).toBeTruthy());
  ...
  it("passes undefined review context for an ordinary chat", async () => {
    ...
    await act(async () => {
      result.current.sendMessage("我状态如何");
    });
    await waitFor(() => expect(result.current.loading).toBe(false));
