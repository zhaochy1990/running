...
function wrapper({ children }: { children: ReactNode }) {
  return <UserContext.Provider value={{ user: "user-1", displayName: "A", profileReady: true, refresh: async () => {} }}>{children}</UserContext.Provider>;
}
...
describe("useCoachChat — history + replay", () => {
  ...
  it("auto-replays a persisted pending turn after history load with the same id", async () => {
    sessionStorage.setItem("stride.coach.pendingTurn", JSON.stringify({ sessionId: "web-default", clientTurnId: "pending-42", message: "未完成的问题" }));
    apiMocks.fetchCoachHistory.mockResolvedValueOnce(historyResp([]));
  ...
  it("auto-replays the frozen request snapshot (target + context) verbatim", async () => {
    ...
    const { result } = renderHook(() => useCoachChat({ target: { kind: "week", folder: "2026-08-01_08-07" } }), { wrapper });
  ...
  it("does not replay when the pending turn already landed in history", async () => {
    sessionStorage.setItem("stride.coach.pendingTurn", JSON.stringify({ sessionId: "web-default", clientTurnId: "echoed-1", message: "已落库的问题" }));
    apiMocks.fetchCoachHistory.mockResolvedValueOnce(
      historyResp([
  ...
  it("reuses the same client_turn_id when retrying a failed send", async () => {
    apiMocks.fetchCoachHistory.mockResolvedValueOnce(historyResp([]));
    apiMocks.sendCoachChatMessage.mockRejectedValueOnce(new Error("网络错误")).mockResolvedValueOnce(chatResp("重试成功"));
    const { result } = renderHook(() => useCoachChat(), { wrapper });
    await waitFor(() => expect(result.current.historyLoading).toBe(false));
  ...
  it("keeps a pending proposal and its original anchor after a read-only follow-up", async () => {
    ...
    apiMocks.sendCoachChatMessage.mockResolvedValueOnce(chatResp("这是后续说明", { messageId: "follow-up-message" }));
    const { result } = renderHook(() => useCoachChat(), { wrapper });
    await waitFor(() => expect(result.current.historyLoading).toBe(false));
  ...
  it("clears a previously restored proposal when history reports none pending", async () => {
    ...
    apiMocks.fetchCoachHistory
      ...
      .mockResolvedValueOnce(historyResp([{ role: "user", content: "普通问题", parts: [], message_id: "m0" }]));
    const { result } = renderHook(() => useCoachChat(), { wrapper });
