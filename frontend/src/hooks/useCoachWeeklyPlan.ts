...
export function useCoachWeeklyPlan(): CoachWeeklyPlanState {
  ...
  useEffect(() => {
    ...
    getMyProfile()
      .then((profile) => {
        if (!cancelled) setProvider(profile.provider ?? "coros");
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    Promise.all([getWeek(user, folder), getWeekStrength(user, folder).catch(() => null)])
  ...
  const pushSession = useCallback(
    async (session: PlannedSession, targetDate?: string) => {
      if (!user || !week) return;
      const response = await pushPlannedSession(user, session.date, session.session_index, targetDate);
      if (!response.ok) {
