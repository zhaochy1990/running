...
import { fetchCoachHistory, sendCoachChatMessage, WEB_DEFAULT_SESSION_ID } from "../api";
...
function readPendingTurn(maxMessageChars = DEFAULT_COACH_CHAT_MAX_MESSAGE_CHARS): PendingTurn | null {
  try {
    ...
    if (typeof parsed.sessionId === "string" && typeof parsed.clientTurnId === "string" && typeof parsed.message === "string") {
      ...
      if (snap && typeof snap === "object") {
        base.requestSnapshot = {
          ...
          reviewContext: snap.reviewContext && typeof snap.reviewContext === "object" ? snap.reviewContext : null,
        };
...
export function clearPendingCoachTurnForWeek(folder: string): void {
  ...
  const contextFolder = proposal && typeof proposal.folder === "string" ? proposal.folder : null;
...
export function useCoachChat(options: UseCoachChatOptions = {}): CoachChatState {
  ...
  const maxMessageChars = coachChatMaxMessageChars ?? DEFAULT_COACH_CHAT_MAX_MESSAGE_CHARS;
  ...
  const runTurn = useCallback(
    async (message: string, turn: PendingTurn, snapshot: TurnRequestSnapshot) => {
      ...
      try {
        const res = await sendCoachChatMessage(message, turn.clientTurnId, turn.sessionId, snapshot.target ?? undefined, snapshot.reviewContext ?? undefined);
        if (res.ok) {
      ...
      .then((res) => {
        ...
        if (pending && !loadingRef.current) {
          ...
          if (alreadyEchoed) {
            ...
            const hasUserMsg = views.some((v) => v.role === "user" && v.content === pending.message);
            if (!hasUserMsg) {
  ...
  const retry = useCallback(() => {
    ...
    const snapshot: TurnRequestSnapshot = (reuse ? pending.requestSnapshot : null) ??
  ...
  return useMemo(
    ...
    [messages, loading, error, sendMessage, retry, proposals, activeTarget, proposalContextAnchor, historyLoading, historyError, reloadHistory],
  );
}
