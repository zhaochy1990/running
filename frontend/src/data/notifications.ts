...
export const NOTIFICATIONS: AppNotification[] = [
  {
    ...
    body: "STRIDE 已经启用全新的访问地址：https://stride-running.cn 。这个域名更短、更好记，欢迎大家更新书签并使用新地址访问。原来的 Azure 链接也仍然可用，登录信息无需重新配置。",
  ...
  },
  {
    ...
    title: "新功能：顶部导航 & 消息中心",
    body: "我们调整了页面布局：用户资料和登出按钮已经移动到屏幕右上角，旁边新增了消息中心入口。今后所有的产品更新都会通过消息中心推送，关闭弹窗后仍可在消息中心查看历史。",
  ...
  },
  {
    ...
    title: "训练点评现在自动生成",
    body: "每次同步 COROS 数据后，系统会自动用 GPT-4.1 为新的训练写一条点评草稿。你可以在活动详情页面手动重新生成或由教练进一步润色。",
...
export function getNotificationsNewestFirst(notifications: readonly AppNotification[] = NOTIFICATIONS): AppNotification[] {
  return [...notifications].sort((a, b) => (b.updatedAt ?? b.publishedAt).localeCompare(a.updatedAt ?? a.publishedAt));
}
