...
import { getActivityLikes, likeActivity, unlikeActivity, type ActivityLiker } from "../api";
 ...
 */
export default function LikeButton({ teamId, userId, labelId, initialCount, initialLiked, initialTopLikers, currentUserDisplayName }: LikeButtonProps) {
    ...
    <div className="flex items-center gap-2 relative">
      <button
        ...
        className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-xs font-mono transition-colors disabled:opacity-50 ${
          liked ? "text-accent-red bg-accent-red/10 hover:bg-accent-red/20" : "text-text-muted hover:text-accent-red hover:bg-accent-red/5"
        }`}
      >
        <svg viewBox="0 0 24 24" className="w-4 h-4" fill={liked ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" aria-hidden>
          <path
            strokeLinecap="round"
      ...
      {err && <span className="text-[11px] font-mono text-accent-red">{err}</span>}
      ...
      {popoverOpen && (
        ...
        >
          <div className="text-[11px] font-mono text-text-muted px-2 pb-1 border-b border-border-subtle mb-1">点赞的人 ({count})</div>
          {likersLoading ? (
            <div className="text-xs text-text-muted px-2 py-2 font-mono">加载中...</div>
          ) : likers && likers.length > 0 ? (
            <ul className="max-h-48 overflow-y-auto">
              {likers.map((l) => (
                <li key={l.user_id} className="text-xs text-text-primary px-2 py-1 truncate">
                  {l.display_name}
                </li>
              ))}
            </ul>
          ) : (
            <div className="text-xs text-text-muted px-2 py-2 font-mono">暂无</div>
          )}
        </div>
      )}
    </div>
