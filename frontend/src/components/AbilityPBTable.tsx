...
export default function AbilityPBTable({ pbs }: { pbs: PBEntry[] }) {
  ...
  return (
    <div>
      <p className="text-xs font-mono text-text-muted tracking-widest mb-3 uppercase">个人最佳 · Personal Bests</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {PB_ROWS.map((row) => {
                ...
                return (
                  <tr key={row.code} className="border-b border-border-subtle hover:bg-bg-card-hover transition-colors">
                    <td className="py-2.5 px-3 font-mono text-text-secondary">{row.label}</td>
                    <td className="py-2.5 px-3 text-right font-mono font-medium text-accent-green">{entry ? fmtClock(entry.pb_time_sec) : "—"}</td>
                    <td className="py-2.5 px-3 text-right font-mono text-text-secondary">{entry ? fmtPace(entry.pb_time_sec, row.km) : "—"}</td>
                    <td className="py-2.5 px-3 text-right font-mono text-text-muted text-xs">{entry ? entry.achieved_at : "—"}</td>
