...
function Card({
  ...
  return (
    <div className={`rounded-2xl p-6 border transition-all ${highlighted ? "bg-accent-green/8 border-accent-green/40" : "bg-bg-card border-border-subtle"}`}>
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className={`text-xs font-medium ${highlighted ? "text-accent-green" : "text-text-secondary"}`}>{label}</p>
      ...
      <p className="text-3xl font-bold font-mono tracking-tight" style={{ color: highlighted ? "#00a85a" : "#1a1c2e" }}>
        {value}
      </p>
      {pace !== "—" && (
        <p className="text-sm font-mono mt-1" style={{ color: highlighted ? "#00a85a" : "#6b6b80" }}>
          配速 {pace}
        </p>
      )}
