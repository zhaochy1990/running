...
const DIM_EXPLAIN: Record<string, DimExplain> = {
  ...
  vo2max: {
    formula:
      "三路径估算取最高：Daniels VDOT（间歇/5K 公式反算）、HR-pace 回归（E 跑外推到 HRmax）、Uth-Sørensen（15.3 × HRmax/RHR）。全马成绩用 Daniels 表逆查（公式对 3h+ 低估）",
...
export default function AbilityRadar({ current, weights }: { current: AbilityCurrent; weights: Record<string, number> | null }) {
    ...
    <div className="bg-bg-card border border-border-subtle rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        {composite != null && (
          <div className="text-right">
            ...
            <p className="text-2xl font-bold font-mono text-accent-green tracking-tight">{fmtScore(composite, 2)}</p>
          </div>
        )}
      </div>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <RadarChart data={data} outerRadius="75%">
          ...
          <PolarAngleAxis dataKey="dimension" tick={{ fontSize: 11, fontFamily: "JetBrains Mono", fill: "#4a4c60" }} />
          <PolarRadiusAxis domain={[0, 100]} tickCount={6} tick={{ fontSize: 9, fontFamily: "JetBrains Mono", fill: "#8888a0" }} axisLine={false} />
          <Radar name="实力" dataKey="score" stroke="#00a85a" fill="#00a85a" fillOpacity={0.25} strokeWidth={2} dot={{ fill: "#00a85a", r: 3 }} />
          <Tooltip
            contentStyle={{
              background: "#ffffff",
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-4">
        {data.map((d) => (
          <div key={d.key} className="flex items-baseline justify-between px-3 py-2 bg-bg-secondary rounded-lg">
            <div>
            ...
            <p className="text-lg font-bold font-mono text-accent-green tracking-tight">{fmtScore(d.rawScore, 2)}</p>
          </div>
        ))}
      </div>
    </div>
