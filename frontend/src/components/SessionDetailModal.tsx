...
import type { PlannedSession, NormalizedRunWorkout, NormalizedStrengthWorkout, WorkoutStep } from "../types/plan";
  ...
  return (
    <div className="space-y-2">
      {spec.blocks.map((block, bi) => (
        <div key={bi}>
          {block.repeat > 1 && <p className="text-xs font-mono text-text-muted mb-1">重复 {block.repeat} 次</p>}
                  ...
                  <td className="py-1.5 pr-2">
                    <span
                      ...
                      style={{
                        ...
                        backgroundColor: (step.step_kind === "work" ? "#00a85a" : "#8888a0") + "15",
                      }}
                    >
                      {STEP_LABEL[step.step_kind] ?? step.step_kind}
                    </span>
                  </td>
                  <td className="py-1.5 pr-2 text-text-secondary">{stepDurationText(step)}</td>
                  <td className="py-1.5 pr-2 text-text-primary">
                    {stepTargetText(step)}
                    {step.hr_cap_bpm != null && <span className="ml-1.5 text-text-muted">HR ≤{Math.round(step.hr_cap_bpm)}</span>}
                  </td>
      ...
      {spec.note && <p className="text-xs text-text-muted mt-2">{spec.note}</p>}
    </div>
  );
}
          ...
          {spec.exercises.map((ex, i) => (
            <tr key={i} className="border-b border-border-subtle/50">
              ...
              <td className="py-1.5 pr-2 text-text-secondary">
                {ex.sets}×{ex.target_kind === "reps" ? `${ex.target_value}` : fmtDuration(ex.target_value)}
              </td>
...
export default function SessionDetailModal({ session, onClose }: SessionDetailModalProps) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="bg-bg-card border border-border-subtle rounded-2xl p-5 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-[11px] font-mono px-1.5 py-0.5 rounded" style={{ color, backgroundColor: color + "15" }}>
                {KIND_LABEL[s.kind]}
              </span>
              <h2 id="session-detail-title" className="text-sm font-medium text-text-primary mt-1">
                {s.summary}
              </h2>
              <p className="text-xs font-mono text-text-muted">{s.date}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-text-muted hover:text-text-primary text-lg leading-none p-1" aria-label="关闭">
            ×
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-mono text-text-secondary mb-4 pb-3 border-b border-border-subtle">
          ...
          {s.total_duration_s != null && <span>时长 {fmtDuration(s.total_duration_s)}</span>}
          {rpeMatch && <span>RPE {rpeMatch[1]}</span>}
          {s.scheduled_workout_id != null && <span className="text-accent-green">✓ 已推送</span>}
        </div>

        {/* Detail section */}
        {s.spec?.schema === "run-workout/v1" && <RunDetail spec={s.spec as NormalizedRunWorkout} />}
        {s.spec?.schema === "strength-workout/v1" && <StrengthDetail spec={s.spec as NormalizedStrengthWorkout} />}
        {!s.spec && s.kind !== "rest" && <p className="text-xs text-text-muted">暂无详细训练结构</p>}
        {s.kind === "rest" && <p className="text-xs text-text-muted">休息日 — 充分恢复</p>}
