...
function DebugParts({ parts }: { parts: AssistantPart[] }) {
  ...
  return (
    <details className="mt-2 rounded-md border border-border-subtle bg-bg-secondary/50 px-3 py-2 text-xs text-text-muted">
      <summary className="cursor-pointer select-none font-mono text-text-muted">推理 / 工具调用（{debug.length}）</summary>
      <div className="mt-2 space-y-2">
        {debug.map((p, i) => (
          <div key={p.id ?? i} className="font-mono">
            <span className="text-accent-green/70">[{p.kind}]</span> <span className="whitespace-pre-wrap break-words">{p.text}</span>
          </div>
        ))}
      </div>
    </details>
...
export default function CoachChatMessage({ role, content, refusal = false, parts, showDebug = false, toolName, eventStatus }: CoachChatMessageProps) {
  if (role === "event") {
    ...
    return (
      <div
        ...
        className={`flex items-center gap-2 rounded-md border px-3 py-1.5 text-xs ${
          applied ? "border-accent-green/30 bg-accent-green/10 text-accent-green" : "border-border-subtle bg-bg-secondary/60 text-text-muted"
        }`}
      >
  ...
  if (role === "user") {
    return (
      <div role="group" aria-label="你的消息" className="flex justify-end" data-role="user">
  ...
  if (role === "tool") {
    ...
    return (
      <details data-role="tool" className="rounded-md border border-border-subtle bg-bg-secondary/50 px-3 py-2 text-xs text-text-muted">
        <summary className="cursor-pointer select-none font-mono text-text-muted">工具输出{toolName ? ` · ${toolName}` : ""}</summary>
