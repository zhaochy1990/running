    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
        ...
        <section className="rounded-2xl border border-border-subtle bg-bg-card p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-6 grid gap-4 xl:grid-cols-2">
            {data.sessions.map((session) => (
              <StrengthSessionCard key={`${session.date}-${session.session_index}`} session={session} />
            ))}
          </div>
        </section>
      ...
      <aside className="space-y-4">
        <div className="rounded-xl border border-border-subtle bg-bg-card p-5 shadow-sm">
          ...
          <ul className="mt-4 space-y-3 text-xs leading-5 text-text-secondary">
            <li>动作名和组数来自本周结构化计划。</li>
            <li>左右侧动作按每侧完成。</li>
            <li>动作质量优先于次数和负重。</li>
          </ul>
        </div>
