    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
                ...
                <div key={activity.label_id} className="grid gap-3 p-4 sm:grid-cols-[minmax(0,1fr)_120px_120px_86px] sm:items-center">
                  <div className="min-w-0">
                    <Link
                      ...
                      className="block truncate text-sm font-bold text-text-primary hover:text-accent-green hover:underline"
                    >
      ...
      <aside className="space-y-4">
        <div className="rounded-xl border border-border-subtle bg-bg-card p-5 shadow-sm">
          ...
          <Progress label="跑量" value={`${actualRunKm.toFixed(1)} / ${stats.plannedRunKm.toFixed(1)} km`} percent={mileagePct} />
          <SummaryValue label="已同步活动" value={`${activities.length} 次`} />
        </div>
        <div className="rounded-xl border border-amber-soft bg-amber-soft/40 p-5">
          <h3 className="text-sm font-bold text-text-primary">Coach 复盘提示</h3>
          <p className="mt-3 font-editorial text-sm italic leading-6 text-text-secondary">
            同步后优先查看关键课目标是否稳定、轻松跑是否守住低强度，以及营养日是否按计划执行。
          </p>
        </div>
      </aside>
    </div>
...
function Progress({ label, value, percent }: ProgressProps) {
  return (
    <div className="mt-5">
...
function QualityCard({ label }: QualityCardProps) {
  return (
