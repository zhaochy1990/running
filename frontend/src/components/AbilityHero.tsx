...
export default function AbilityHero({
  ...
  return (
    <div className="bg-bg-card border border-border-subtle rounded-2xl p-10 mb-6 text-center animate-fade-in">
      ...
      <p className="text-7xl md:text-8xl font-bold font-mono tracking-tight leading-none" style={{ color: onPace ? "#00a85a" : "#1a1c2e" }}>
        {fmtHMS(raceS)}
      </p>
      {pace !== "—" && (
        <p className="text-lg font-mono mt-2" style={{ color: onPace ? "#00a85a" : "#6b6b80" }}>
          配速 {pace}
        </p>
      )}
      {hasTarget && targetLabel ? (
        <div className="mt-5 flex items-center justify-center gap-4 flex-wrap">
          <span className="text-sm font-mono text-text-secondary">目标 {targetLabel}</span>
          <span
      ...
      <p className="text-xs font-mono text-text-muted mt-4 leading-relaxed">完美赛日 / 未减量 · Race-day execution assuming taper & perfect conditions</p>
    </div>
  );
}
