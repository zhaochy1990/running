...
export default function PushAllPlannedButton({
  ...
  const targets = useMemo(() => pushableSessionsFor(sessions, canPushRun, strengthCap), [sessions, canPushRun, strengthCap]);
  ...
  const label = pushing ? `推送中… (${progress?.done ?? 0}/${progress?.total ?? total})` : total === 0 ? "✓ 全部已推送" : `一键推送 (${total})`;

  const ariaLabel = pushing ? "推送中" : total === 0 ? "全部已推送" : "一键推送本周训练";
  ...
  return (
    <div data-testid="push-all-container" className="flex flex-col gap-1.5">
      ...
      {results && results.length > 0 && (
        <div data-testid="push-all-results" role="status" className="px-4 py-1 text-[11px] font-mono">
