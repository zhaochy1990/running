...
export default function PlanVsActualCard({ session, activity }: PlanVsActualCardProps) {
  ...
  const distJudge = judgeNumeric(actualDistanceM, planDistanceM != null ? planDistanceM * 0.95 : null, planDistanceM != null ? planDistanceM * 1.05 : null);
  ...
  return (
    <div data-testid="plan-vs-actual-card" className="bg-bg-card border border-border-subtle rounded-2xl p-5 mb-6">
      <h3 className="text-sm font-semibold text-text-secondary mb-4 tracking-wide">训练计划对照</h3>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Row label="距离" plan={fmtKm(planDistanceM)} actual={fmtKm(actualDistanceM)} judgement={distJudge} />
        <Row
          label="平均配速"
          plan={planPace.low != null && planPace.high != null ? `${fmtPaceSecKm(planPace.high)} – ${fmtPaceSecKm(planPace.low)}` : "—"}
          actual={fmtPaceSecKm(activity.avg_pace_s_km)}
          judgement={paceJudge}
        />
        <Row
          label="平均心率"
          plan={planHR.low != null && planHR.high != null ? `${Math.round(planHR.low)}–${Math.round(planHR.high)} bpm` : "—"}
...
function Row({ label, plan, actual, judgement }: { label: string; plan: string; actual: string; judgement: AdherenceJudgement }) {
