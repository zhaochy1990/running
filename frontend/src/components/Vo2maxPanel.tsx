...
export default function Vo2maxPanel({
  ...
  const hint = !hasBreakdown ? "今日从快照读取；刷新后将显示三路径对比" : "主路径 = 近期最佳表现；备选 = HR-pace 回归；基线 = UTH/Sörensen";
  ...
  return (
    <div className="bg-bg-card border border-border-subtle rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs font-mono text-text-muted">
            三路径对比 · 当前源: <span style={{ color: SOURCE_COLORS[source] }}>{SOURCE_LABELS[source]}</span>
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
        <Path label="主路径" sublabel="Primary · best performance" value={primary} unit="ml/kg/min" active={source === "primary"} color="#00a85a" />
        <Path label="备选路径" sublabel="Secondary · HR-pace" value={secondary} unit="ml/kg/min" active={source === "secondary"} color="#0097a7" />
        <Path label="基线估算" sublabel="Floor · UTH/Sörensen" value={floor} unit="ml/kg/min" active={source === "floor"} color="#8888a0" />
      </div>
      ...
      <div className="flex items-center justify-between bg-bg-secondary rounded-lg px-4 py-3">
        <div>
          ...
          <p className="text-lg font-bold font-mono text-text-primary tracking-tight">{used != null ? `${fmtScore(used, 1)} ml/kg/min` : "—"}</p>
        </div>
        <div className="text-right">
          <p className="text-xs font-mono text-text-muted">等效 VDOT</p>
          <p className="text-lg font-bold font-mono text-text-primary tracking-tight">{fmtScore(usedVdot, 1)}</p>
        </div>
      </div>
...
function Path({
  ...
  return (
    <div
      className={`rounded-xl p-3 border transition-all ${active ? "" : "opacity-70"}`}
      style={{
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {active && (
          <span className="text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded" style={{ color, backgroundColor: color + "20" }}>
            ACTIVE
          </span>
        )}
      </div>
