...
function useIsDesktop(): boolean {
  ...
  const [isDesktop, setIsDesktop] = useState(() => canMatch && window.matchMedia("(min-width: 1024px)").matches);
  useEffect(() => {
...
function readStoredWidth(): number {
  try {
    ...
    if (raw !== null) {
      ...
      if (Number.isFinite(parsed) && parsed >= CHAT_WIDTH_MIN_PCT && parsed <= CHAT_WIDTH_MAX_PCT) {
        return parsed;
      }
    }
  } catch {
...
export function WorkspaceLayout({ children, chat, title }: WorkspaceLayoutProps) {
  ...
  return (
    ...
    >
      <main className="flex min-h-0 min-w-0 flex-1 flex-col lg:h-full" aria-label={title}>
        ...
        <div data-testid="coach-workspace-review-scroll" className="min-h-0 flex-1 lg:overflow-y-auto lg:pr-2">
          {children}
        </div>
      </main>
      ...
      */}
      ...
      {drawerOpen && <div className="fixed inset-0 z-40 bg-black/40 lg:hidden" aria-hidden onClick={closeDrawer} />}
      ...
      */}
      ...
      >
        ...
        <div data-testid="coach-workspace-chat-scroll" className="min-h-0 flex-1 overflow-hidden">
          {chat}
        </div>
      </aside>
    </div>
