...
export function DiffReview({ changes }: DiffReviewProps) {
  return (
    <ul className="space-y-3">
      {changes.map((change) => (
        <li key={change.opId} className="rounded-lg border border-border-subtle bg-bg-primary p-3">
          <div className="mb-2 flex items-center justify-between gap-2">
            <span className="text-sm font-medium text-text-primary">{change.label}</span>
            <span className={`text-xs font-medium ${CHANGE_TONE[change.changeType]}`}>{CHANGE_LABEL[change.changeType]}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            {change.oldValue != null && <span className="text-text-muted line-through">{change.oldValue}</span>}
            ...
            {change.newValue != null && <span className="text-text-primary">{change.newValue}</span>}
          </div>
        </li>
      ))}
    </ul>
