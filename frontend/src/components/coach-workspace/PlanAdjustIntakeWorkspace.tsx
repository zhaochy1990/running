 ...
 */
export function PlanAdjustIntakeWorkspace({ kind, currentPlanSummary, chat, emptyTarget = false, currentPlanDetail }: PlanAdjustIntakeWorkspaceProps) {
  const title = kind === "weekly" ? "调整训练周" : "调整赛季计划";
  return (
    <WorkspaceLayout title={title} chat={chat}>
      <section className="space-y-4">
        <div className="rounded-lg border border-border-subtle bg-bg-card p-4">
          <div className="text-xs font-medium uppercase tracking-wide text-text-muted">{kind === "weekly" ? "当前计划" : "当前赛季计划"}</div>
          <div className="mt-1 text-sm text-text-primary">{currentPlanSummary}</div>
          {currentPlanDetail && <div className="mt-4 border-t border-border-subtle pt-4">{currentPlanDetail}</div>}
        </div>

        <div className="rounded-lg border border-border-subtle bg-bg-card p-4 text-sm text-text-muted">
          {emptyTarget ? "这一周还没有计划。告诉 Coach 你的目标，让它为你生成一份。" : "告诉 Coach 你想怎么调整，它会给出一份可启用的调整方案。"}
        </div>
      </section>
    </WorkspaceLayout>
  );
