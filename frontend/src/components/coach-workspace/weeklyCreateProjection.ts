 ...
 */
import type { CreatePlanDay, CreatePlanMeal, CreatePlanNutritionDay, CreatePlanStrengthDay, CreatePlanStrengthExercise } from "./types";
...
function projectStrengthDay(session: Record<string, unknown>): CreatePlanStrengthDay | null {
  ...
  const exercises = rawExercises.map(projectExercise).filter((e): e is CreatePlanStrengthExercise => e !== null);
  return {
    label,
...
export function projectWeeklyCreate(rawProposal: unknown): WeeklyCreateProjection {
  ...
  const nutrition = rawNutrition.map(projectNutritionDay).filter((d): d is CreatePlanNutritionDay => d !== null);
