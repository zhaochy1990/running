...
import type { ApplyOutcome, ApplyProposalRequest, StashedProposal, WeeklyCreateProposal, WeeklyDiffProposal } from "../types";
...
function renderWorkspace(proposal: WeeklyDiffProposal, overrides: Partial<{ apply: (req: ApplyProposalRequest) => Promise<ApplyOutcome> }> = {}): Harness {
  const onApply = vi.fn(overrides.apply ?? (async () => ({ status: "ok" }) as ApplyOutcome));
...
describe("WeeklyPlanAdjustWorkspace", () => {
  ...
  it("applies a create proposal with its op ids and keeps rawProposal untouched", async () => {
    ...
    const onApply = vi.fn<(req: ApplyProposalRequest) => Promise<ApplyOutcome>>(async () => ({ status: "ok" }) as ApplyOutcome);
    render(<WeeklyPlanAdjustWorkspace stashed={stashed} currentPlanSummary="尚无本周计划" onApply={onApply} onDiscard={vi.fn()} chat={<div>chat</div>} />);
    fireEvent.click(screen.getByRole("button", { name: "启用计划" }));
    await waitFor(() => expect(onApply).toHaveBeenCalledTimes(1));
    const req = onApply.mock.calls[0][0] as ApplyProposalRequest;
