...
describe("CreateReview", () => {
        ...
        strength={[
          {
            ...
            title: "下肢力量 A",
            note: "动作缓慢控制",
            exercises: [{ name: "高脚杯深蹲", sets: 3, target: "12 次", rest: "休息 90 秒", note: "核心收紧" }],
          },
        ]}
      />,
    );
  ...
  it("renders the weekly note first and never renders a duplicate coach explanation", () => {
    const { container } = render(
      <CreateReview days={[{ label: "2026-07-20", detail: "轻松跑" }]} notesMd={'本周重点：**恢复**为主 <img src=x onerror="alert(1)">'} />,
    );
    const noteHeading = screen.getByText("本周说明");
    const calendarHeading = screen.getByText("本周训练日历");
    expect(noteHeading.compareDocumentPosition(calendarHeading) & Node.DOCUMENT_POSITION_FOLLOWING).toBeTruthy();
  ...
  it("renders the weekly note as Markdown and never as raw HTML", () => {
    const { container } = render(<CreateReview days={[]} notesMd={'本周重点：**恢复**为主 <img src=x onerror="alert(1)">'} />);
