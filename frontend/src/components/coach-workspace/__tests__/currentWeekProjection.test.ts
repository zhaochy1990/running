...
describe("projectCurrentWeekPlan", () => {
  ...
  it("projects strength specs into the standalone strength section", () => {
    const days: PlanDay[] = [{ date: "2026-07-28", sessions: [strengthSession()], nutrition: null }];
    const projection = projectCurrentWeekPlan(days);
  ...
  it("projects nutrition days and meals", () => {
    const days: PlanDay[] = [{ date: "2026-07-27", sessions: [runSession()], nutrition: nutrition() }];
    const projection = projectCurrentWeekPlan(days);
  ...
  it("falls back to the kind label when a session has no summary or note", () => {
    const days: PlanDay[] = [
      {
        date: "2026-07-29",
        sessions: [runSession({ date: "2026-07-29", kind: "rest", summary: "", notes_md: null, spec: null })],
        nutrition: null,
      },
    ];
    const projection = projectCurrentWeekPlan(days);
