...
export default function TopNav({ onOpenMobileSidebar }: TopNavProps) {
  ...
  return (
    <header data-testid="top-nav" className="sticky top-0 z-30 flex items-center gap-3 h-[var(--topbar-h)] px-3.5 bg-bg-card border-b border-border-subtle">
      <button
      ...
      <button onClick={() => navigate("/")} className="hidden sm:flex items-center gap-2 cursor-pointer" style={{ width: 206 }}>
        <span
      ...
      >
        ...
        <span className="hidden sm:inline text-xs font-mono text-text-secondary truncate max-w-[120px]">{displayName}</span>
