...
export default function SyncStatusPill() {
  ...
  const label = syncing ? "同步中..." : syncError ? "同步失败 · 重试" : haveLastSync ? `已同步 · ${relativeTime(Date.now() - lastSyncMs)}` : "未同步";
  ...
  return (
    ...
    >
      {!syncing && haveLastSync && <span className="w-1.5 h-1.5 rounded-full bg-accent-green animate-pulse" aria-hidden />}
      {label}
    </button>
  );
}
