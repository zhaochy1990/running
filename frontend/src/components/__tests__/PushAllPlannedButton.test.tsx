...
import type { NormalizedRunWorkout, NormalizedStrengthWorkout, PlannedSession } from "../../types/plan";
...
describe("pushableSessionsFor", () => {
  ...
  it("drops sessions that are already pushed", () => {
    const list = pushableSessionsFor([makeRun({ scheduled_workout_id: 99 })], true, true);
    expect(list).toEqual([]);
  });
...
describe("PushAllPlannedButton", () => {
  it("renders button with the count of pushable sessions", () => {
    ...
    expect(screen.getByRole("button", { name: "一键推送本周训练" })).toHaveTextContent("一键推送 (2)");
  });
  ...
  it("returns null when status is parse_failed", () => {
    const { container } = render(<PushAllPlannedButton sessions={[makeRun()]} structuredStatus="parse_failed" canPushRun onPush={() => Promise.resolve()} />);
    expect(container.firstChild).toBeNull();
  });

  it("returns null when no eligible sessions exist (all rest)", () => {
    const { container } = render(<PushAllPlannedButton sessions={[makeRest()]} structuredStatus="fresh" canPushRun onPush={() => Promise.resolve()} />);
    expect(container.firstChild).toBeNull();
  });

  it('shows "全部已推送" when every eligible session has scheduled_workout_id', () => {
    render(<PushAllPlannedButton sessions={[makeRun({ scheduled_workout_id: 1 })]} structuredStatus="fresh" canPushRun onPush={() => Promise.resolve()} />);
  ...
  it("calls onPush sequentially for each pushable session", async () => {
    ...
    expect(calls).toEqual(["2026-04-20#0", "2026-04-22#0", "2026-04-21#1"]);
  });
  ...
  it("reports failure summary when some pushes throw", async () => {
    ...
    render(
      <PushAllPlannedButton
        sessions={[makeRun({ date: "2026-04-20", session_index: 0 }), makeRun({ date: "2026-04-22", session_index: 0 })]}
        structuredStatus="fresh"
        canPushRun
        onPush={onPush}
      />,
