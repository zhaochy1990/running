...
vi.mock("../CoachProposalUpgradeCard", () => ({
  default: ({ proposal, activeTarget, contextAnchor }: { proposal?: unknown; activeTarget?: unknown; contextAnchor?: string }) => (
    <div data-testid="upgrade-card" data-kind={proposal ? "proposal" : activeTarget ? "active" : "none"} data-context-anchor={contextAnchor} />
  ),
}));
...
describe("CoachChat — proposals", () => {
  ...
  it("keeps a pending proposal anchored to the turn that produced it", () => {
    ...
    renderChat();
    expect(screen.getByTestId("upgrade-card")).toHaveAttribute("data-context-anchor", "proposal-message");
  });
