...
describe("LikeButton", () => {
  it("shows initial count and liked state", () => {
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={3} initialLiked={true} initialTopLikers={["Alice", "Bob", "Carol"]} />);
  ...
  it('shows "等 N 人赞过" when count exceeds top liker names', () => {
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={5} initialLiked={false} initialTopLikers={["Alice", "Bob", "Carol"]} />);
    expect(screen.getByText("Alice、Bob、Carol 等 5 人赞过")).toBeInTheDocument();
  });

  it('falls back to "N 人赞过" when top likers list is empty', () => {
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={2} initialLiked={false} />);
  ...
  it("optimistically prepends caller name on like and calls likeActivity", async () => {
    ...
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={0} initialLiked={false} currentUserDisplayName="Me" />);
  ...
  it("reverts on like failure and shows error", async () => {
    ...
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={0} initialLiked={false} />);
    await act(async () => {
  ...
  it("calls unlikeActivity when already liked and removes self from inline list", async () => {
    ...
    render(
      <LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={1} initialLiked={true} initialTopLikers={["Me"]} currentUserDisplayName="Me" />,
    );
    await act(async () => {
  ...
  it("opens popover and lazy-loads likers when inline summary is clicked", async () => {
    ...
    render(<LikeButton teamId={TEAM} userId={USER} labelId={LABEL} initialCount={2} initialLiked={false} initialTopLikers={["Alice", "Bob"]} />);
