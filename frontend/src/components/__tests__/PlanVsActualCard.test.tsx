...
describe("PlanVsActualCard", () => {
  ...
  it("marks distance red when actual is half the plan", () => {
    const { container } = render(<PlanVsActualCard session={session} activity={makeActivity({ distance_m: 10000, distance_km: 10 })} />);
    const node = container.querySelector(".adherence-red");
    expect(node).not.toBeNull();
  });

  it("marks pace green when inside target band", () => {
    const { container } = render(<PlanVsActualCard session={session} activity={makeActivity({ avg_pace_s_km: 345 })} />);
