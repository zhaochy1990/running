...
const WEEK_DATES = ["2026-04-20", "2026-04-21", "2026-04-22", "2026-04-23", "2026-04-24", "2026-04-25", "2026-04-26"];
...
describe("PlannedCalendar", () => {
  it("renders 7 day-cards even when only some have sessions", () => {
    render(<PlannedCalendar weekDates={WEEK_DATES} sessions={sessions} nutrition={[]} structuredStatus="fresh" canPushRun={true} onPush={() => {}} />);
  ...
  it("shows backfill banner when status=backfilled", () => {
    render(<PlannedCalendar weekDates={WEEK_DATES} sessions={sessions} nutrition={[]} structuredStatus="backfilled" canPushRun={true} onPush={() => {}} />);
    expect(screen.getByTestId("backfill-banner")).toBeInTheDocument();
  });

  it("shows empty fallback when status=parse_failed", () => {
    render(<PlannedCalendar weekDates={WEEK_DATES} sessions={sessions} nutrition={[]} structuredStatus="parse_failed" canPushRun={true} onPush={() => {}} />);
  ...
  it("shows hr_cap_bpm next to the work pace target (HR ≤N)", () => {
    ...
    render(<PlannedCalendar weekDates={WEEK_DATES} sessions={[tempoSession]} nutrition={[]} structuredStatus="fresh" canPushRun={true} onPush={() => {}} />);
  ...
  it("shows the WORK step pace/HR target, not warmup/cooldown", () => {
    ...
    render(<PlannedCalendar weekDates={WEEK_DATES} sessions={[intervalSession]} nutrition={[]} structuredStatus="fresh" canPushRun={true} onPush={() => {}} />);
