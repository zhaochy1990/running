...
describe("CoachChatMessage — coach bubble", () => {
  it("wraps content in a prose article element", () => {
    const { container } = render(<CoachChatMessage role="coach" content="# Hello" />);
...
| 本月跑量 | 186 km | 稳定 |`;
  ...
  it("renders > blockquote as a <blockquote> element", () => {
    const { container } = render(<CoachChatMessage role="coach" content="> 建议本周继续保持有氧主体。" />);
...
- 如果恢复好，继续课表。`;
  ...
  it("includes an accessible label for the coach avatar", () => {
    ...
    const hasAccessibleAvatar =
      screen.queryByRole("img") !== null || document.querySelector('[aria-label*="coach"], [aria-label*="Coach"], [aria-label*="教练"]') !== null;
...
describe("CoachChatMessage — user bubble", () => {
  it("does not wrap content in a prose article", () => {
    const { container } = render(<CoachChatMessage role="user" content="我最近练得怎么样？" />);
  ...
  it("renders the user message text with a sender label", () => {
    ...
    expect(screen.getByRole("group", { name: "你的消息" })).toHaveTextContent("我最近练得怎么样？");
  });
});

describe("CoachChatMessage — tool debug bubble", () => {
  it("renders raw tool content inside a collapsible details element", () => {
    const { container } = render(<CoachChatMessage role="tool" content="RAW OUTPUT" toolName="search" />);
...
describe("CoachChatMessage — event receipt bar", () => {
  it("renders an applied event as a positive (green) status bar, not markdown", () => {
    const { container } = render(<CoachChatMessage role="event" content="已启用本周课表调整" eventStatus="applied" />);
  ...
  it("renders an abandoned event as a neutral status bar", () => {
    const { container } = render(<CoachChatMessage role="event" content="已放弃该调整方案" eventStatus="abandoned" />);
    const bar = container.querySelector('[data-role="event"]');
  ...
  it("does not interpret event content as markdown", () => {
    const { container } = render(<CoachChatMessage role="event" content="# 不是标题" eventStatus="applied" />);
    expect(container.querySelector("h1")).not.toBeInTheDocument();
