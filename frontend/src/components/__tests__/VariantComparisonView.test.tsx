...
describe("VariantComparisonView", () => {
  ...
  it("renders 3 columns when 3 variants are returned", async () => {
    ...
    render(<VariantComparisonView user="zhaochaoyi" folder="2026-04-20_04-26" initialData={data} />);
  ...
  it("clicking the select button dispatches selectPlanVariant with force=false", async () => {
    ...
    expect(api.selectPlanVariant).toHaveBeenCalledWith("zhaochaoyi", "2026-04-20_04-26", 7, false);
  });
  ...
  it("409 selection_conflict opens confirm dialog → confirm dispatches force=true", async () => {
    ...
    expect(api.selectPlanVariant).toHaveBeenLastCalledWith("zhaochaoyi", "2026-04-20_04-26", 9, true);
  });
  ...
  it("rating slider change debounces 800ms then calls ratePlanVariant", async () => {
    ...
    expect(api.ratePlanVariant).toHaveBeenCalledWith("zhaochaoyi", 11, expect.objectContaining({ overall: 4 }), null);
  });
  ...
  it("show historical toggle fetches variants with includeSuperseded=true", async () => {
    ...
    const dataWithHistory = makeResponse([makeVariant({ variant_id: 1 }), makeVariant({ variant_id: 0, superseded_at: "2026-04-19T00:00:00Z" })]);
    ...
    expect(api.getPlanVariants).toHaveBeenCalledWith("zhaochaoyi", "2026-04-20_04-26", true);
    await waitFor(() => {
  ...
  it("disables select button on already-selected variant", () => {
    const data = makeResponse([makeVariant({ variant_id: 1, is_selected: true })]);
    render(<VariantComparisonView user="zhaochaoyi" folder="2026-04-20_04-26" initialData={data} />);
