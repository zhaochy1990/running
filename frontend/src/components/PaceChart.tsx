...
function HoverLabel({ viewBox, text, color }: { viewBox: unknown; text: string; color: string }) {
  ...
  return (
    <g pointerEvents="none">
      <rect x={boxX} y={boxY} width={boxWidth} height={boxHeight} fill="#ffffff" stroke="#d8dae5" strokeWidth={1} rx={4} />
      <text x={boxX + boxWidth / 2} y={boxY + 14} fontSize={11} fontFamily="JetBrains Mono" fill={color} textAnchor="middle">
        {text}
      </text>
    </g>
  );
...
export default function PaceChart({ data, startTs: startTsProp, hoverElapsed, onHover }: Props) {
  ...
  const hoverText = hoverElapsed != null ? `${formatTime(hoverElapsed)}  ${hoverPoint?.pace ? formatPace(hoverPoint.pace) + "/km" : "—"}` : "";
    ...
    <ResponsiveContainer width="100%" height={200}>
      ...
      >
        ...
        <ReferenceLine
          ...
          strokeDasharray="4 4"
          strokeOpacity={0.6}
          label={{ value: `avg ${formatPace(avgPace)}`, position: "right", fill: "#00a85a", fontSize: 10, fontFamily: "JetBrains Mono" }}
        />
        {hoverElapsed != null && (
          <ReferenceLine x={hoverElapsed} stroke="#1a1c2e" strokeOpacity={0.35} strokeWidth={1} ifOverflow="extendDomain">
            <Label content={(props: LabelProps) => <HoverLabel viewBox={props.viewBox} text={hoverText} color="#00a85a" />} />
          </ReferenceLine>
        )}
        <Area
          type="monotone"
