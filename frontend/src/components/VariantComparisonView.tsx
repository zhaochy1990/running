...
import { getPlanVariants, overallRating, ratePlanVariant, selectPlanVariant, totalKcalTarget, totalRunKm, type SelectVariantConflict } from "../api";
...
export default function VariantComparisonView({ user, folder, initialData, onChange }: Props) {
  ...
  useEffect(() => {
    ...
    let cancel = false;
    getPlanVariants(user, folder, false)
      .then((d) => {
        if (!cancel) setData(d);
      })
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <button type="button" onClick={onToggleHistorical} data-testid="toggle-historical">
      ...
      {confirmDialog && (
        <div
          role="dialog"
          aria-modal="true"
          data-testid="select-confirm-dialog"
          style={{
            position: "fixed",
            top: 0,
            left: 0,
        ...
        >
          <div style={{ background: "white", padding: 24, borderRadius: 8, maxWidth: 480 }}>
            ...
            <p>
              改选会丢弃 <strong>{confirmDialog.alreadyPushed}</strong> 条已推送的手表训练映射。 新计划中匹配的训练需要在 COROS 上手动清理。继续?
            </p>
...
function VariantCard({ variant: v, user, isBusy, onSelectClick, onRated, readOnly }: CardProps) {
  ...
  const longRunM = useMemo(() => {
    return v.sessions.filter((s) => s.kind === "run" && typeof s.total_distance_m === "number").reduce((max, s) => Math.max(max, s.total_distance_m ?? 0), 0);
  }, [v.sessions]);

  const selectButtonLabel = v.is_selected ? "已选定 ✓" : !v.selectable ? `不可用${v.unselectable_reason ? ` (${v.unselectable_reason})` : ""}` : "选定";

  return (
    <div data-testid={`variant-card-${v.variant_id}`} style={{ border: "1px solid #d1d5db", borderRadius: 8, padding: 12, background: "white" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
        <span style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 600 }}>{v.model_id}</span>
        <VariantStatusBadge status={v.variant_parse_status} unselectableReason={v.unselectable_reason} />
        {overall !== null && (
          <span title={`总体评分 ${overall}/5`}>
            {"★".repeat(overall)}
            {"☆".repeat(5 - overall)}
          </span>
        )}
      ...
      <dl data-testid="variant-stats" style={{ fontSize: 12, margin: 0 }}>
        <div>
          <dt style={{ display: "inline", color: "#6b7280" }}>周跑量</dt>: <dd style={{ display: "inline", margin: 0 }}>{km.toFixed(1)} km</dd>
        </div>
        <div>
          <dt style={{ display: "inline", color: "#6b7280" }}>长跑</dt>: <dd style={{ display: "inline", margin: 0 }}>{(longRunM / 1000).toFixed(1)} km</dd>
        </div>
        <div>
          <dt style={{ display: "inline", color: "#6b7280" }}>总热量</dt>: <dd style={{ display: "inline", margin: 0 }}>{kcal.toLocaleString()} kcal</dd>
        </div>
        <div>
          <dt style={{ display: "inline", color: "#6b7280" }}>类型</dt>:{" "}
          <dd style={{ display: "inline", margin: 0 }}>
            {Object.entries(sessionByKind)
              .map(([k, n]) => `${k}×${n}`)
              .join(" / ") || "—"}
          </dd>
        </div>
