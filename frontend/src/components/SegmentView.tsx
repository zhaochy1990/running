...
export default function SegmentView({ segments, laps }: { segments: Segment[]; laps: Lap[] }) {
  ...
  return (
    <div className="space-y-2">
      {segments.map((seg, i) => (
        <SegmentRow key={i} index={i + 1} segment={seg} childLaps={groupedLaps.get(i) || []} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className={`rounded-lg overflow-hidden ${isRecovery ? "border border-border-subtle/30" : "border border-border-subtle"}`}>
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 text-left transition-colors ${isRecovery ? "py-1 opacity-35" : "py-3"} ${
      ...
      >
        ...
        <div className="flex items-center gap-2 min-w-[140px]">
          {hasChildren ? (
            <span className={`text-xl text-accent-green transition-transform duration-200 w-6 text-center leading-none ${expanded ? "rotate-90" : ""}`}>
              &#9656;
            </span>
          ) : (
            <span className="w-6" />
          ...
          {hasChildren && <span className="text-[11px] font-mono text-text-muted ml-1">{childLaps.length} 圈</span>}
        </div>
            ...
            <tbody>
              {childLaps.map((lap, i) => (
                <tr key={lap.lap_index} className="border-b border-border-subtle/50 hover:bg-bg-card-hover/50 transition-colors">
                  ...
                  <td className="py-1.5 px-3 text-right font-mono text-text-muted">{lap.avg_cadence || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <table className="w-full text-sm">
      ...
      <tbody>
        {laps.map((lap) => (
          <tr
            key={lap.lap_index}
            className={`border-b border-border-subtle hover:bg-bg-card-hover transition-colors ${lap.avg_pace === fastestPace ? "bg-accent-green/5" : ""}`}
          >
            ...
            <td className="py-2 px-3 text-right font-mono font-medium" style={{ color: getPaceColor(lap.avg_pace) }}>
              {lap.pace_fmt}
            </td>
            <td className="py-2 px-3 text-right font-mono" style={{ color: getHRColor(lap.avg_hr) }}>
              {lap.avg_hr || "—"}
            </td>
