...
export default function AppLayout() {
  ...
  return (
    <div className="flex flex-col h-screen">
      {mobileOpen && <div className="fixed inset-0 bg-black/50 z-30 sm:hidden" onClick={() => setMobileOpenPath(null)} />}
        ...
        >
          ...
          <div className="sidebar-nav flex-1 overflow-y-auto py-3 px-2.5 flex flex-col gap-px">
            <NavSection label="主功能" collapsed={collapsed}>
              <NavItem to="/" exact collapsed={collapsed} icon={<GridIcon />} text="本周训练" />
              ...
              {coachChat && <NavItem to="/coach" collapsed={collapsed} icon={<ChatIcon />} text="STRIDE Coach" />}
            </NavSection>

            <NavSection label="数据 / 分析" collapsed={collapsed}>
              <NavItem to="/ability" collapsed={collapsed} icon={<TargetIcon />} text="训练能力" />
              <NavItem to="/health" collapsed={collapsed} icon={<PulseIcon />} text="身体指标" />
              <NavItem to="/body-composition" collapsed={collapsed} icon={<UserIcon />} text="体测记录" />
              <NavItem to="/training-status" collapsed={collapsed} icon={<PulseIcon />} text="训练状态（STRIDE）" />
            </NavSection>
          ...
          <SidebarFoot>
            ...
            <FootRow collapsed={collapsed}>
              <FootBtn to="/settings" icon={<GearIcon />} label="设置" collapsed={collapsed} />
              <FootBtn onClick={toggleCollapsed} icon={<CollapseIcon collapsed={collapsed} />} title={collapsed ? "展开" : "折叠"} collapsed={collapsed} />
            </FootRow>
          </SidebarFoot>
        </nav>
