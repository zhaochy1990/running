...
export default function WeeksGrid() {
  ...
  if (loading) {
    return <div className="py-10 text-center text-text-muted text-sm font-mono">加载中...</div>;
  }
  if (weeks.length === 0) {
    return <div className="py-10 text-center text-text-muted text-sm">还没有训练周记录。</div>;
  }
        ...
        return (
          ...
          >
            <div className="flex justify-between items-baseline">
              <span className="font-mono text-[11px] font-semibold text-text-primary">{formatWeekRange(w.date_from, w.date_to)}</span>
              {w.has_feedback && <span className="font-mono text-[9px] px-1.5 py-px rounded bg-accent-cyan/10 text-accent-cyan">反馈</span>}
            </div>
            <p className="text-[12px] text-text-secondary mt-2 line-clamp-2 leading-snug min-h-[2.6em]">{w.plan_title || "—"}</p>
