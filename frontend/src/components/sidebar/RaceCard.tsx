...
export default function RaceCard({ collapsed }: RaceCardProps) {
  ...
  const p = profile?.profile as { target_race?: string; target_race_date?: string; target_distance?: string } | null | undefined;
  ...
  return (
    <div className="rounded-xl px-3 py-2.5 border border-accent-green/30 bg-gradient-to-br from-accent-green/10 to-accent-cyan/10">
      <p className="font-mono text-[9px] text-accent-green-dim tracking-wider font-semibold uppercase">目标赛事</p>
      <p className="text-[12px] font-semibold leading-tight mt-0.5 text-text-primary truncate">{p.target_race}</p>
