...
export default function NavItem({ to, icon, text, tag, hasDot, exact, collapsed }: NavItemProps) {
  return (
    ...
    >
      ...
      {!collapsed && <span className="flex-1 min-w-0 whitespace-nowrap overflow-hidden text-ellipsis">{text}</span>}
      {!collapsed && tag && (
        <span className="font-mono text-[9px] px-1.5 py-px rounded bg-bg-elevated text-text-muted tracking-wider flex-shrink-0">{tag}</span>
      )}
      {!collapsed && hasDot && <span className="w-1.5 h-1.5 rounded-full bg-accent-red flex-shrink-0" />}
    </NavLink>
  );
}
