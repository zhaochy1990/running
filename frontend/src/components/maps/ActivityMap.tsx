...
export default function ActivityMap({ points, pauses, startTs, hoverElapsed, onHover, height = 450 }: Props) {
  ...
  if (error) {
    return <div className="text-text-muted text-sm font-mono py-8 text-center">地图加载失败：{error}</div>;
  }

  if (validGpsCount < 50) {
    return <div className="text-text-muted text-sm font-mono py-8 text-center">无路径数据</div>;
  }
        ...
        {(["none", "hr", "pace"] as Coloring[]).map((c) => (
          <button
            ...
            className={`px-3 py-1.5 transition-colors cursor-pointer ${
              coloring === c ? "bg-accent-green text-white" : "text-text-secondary hover:bg-bg-subtle"
            }`}
          >
