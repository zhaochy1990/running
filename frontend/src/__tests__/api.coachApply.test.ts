...
describe("applyCoachWeekProposal", () => {
  ...
  it('maps a 409 "changed" string detail to stale', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(resp(409, { detail: "weekly plan changed since this proposal was created" }));
    vi.stubGlobal("fetch", fetchMock);
...
describe("applyCoachMasterProposal", () => {
  ...
  it("POSTs the diff with accepted_op_ids, change_reason, base_revision", async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(resp(200, { applied: 2, plan_id: "plan-9", version: 5, updated_at: "t", affected_weeks: [] }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it('maps 409 "changed" to stale', async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(resp(409, { detail: "master plan changed since this proposal was created" }));
    vi.stubGlobal("fetch", fetchMock);
...
describe("abandonCoachProposal", () => {
  it("records a weekly abandonment as a trusted event", async () => {
    const fetchMock = vi.fn().mockResolvedValueOnce(resp(200, { recorded: true, created_at: "2026-07-18T00:00:00Z" }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("returns false when the trusted-event endpoint rejects the request", async () => {
    ...
    await expect(abandonCoachProposal({ kind: "weekly", folder: "2026-07-13_07-19" })).resolves.toBe(false);
  });
});
