...
import { applyMasterPlanAdjustDiff, getActivities, getAllActivities, getCurrentMasterPlan, sendMasterPlanAdjustMessage, type Activity } from "../api";
...
describe("getAllActivities", () => {
  ...
  it("fetches all pages for a date range", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(1, "/api/user-1/activities?date_from=2026-05-01&date_to=2026-05-31&limit=200&offset=0", {
      method: "GET",
      headers: {},
      body: undefined,
    });
    expect(fetchMock).toHaveBeenNthCalledWith(2, "/api/user-1/activities?date_from=2026-05-01&date_to=2026-05-31&limit=200&offset=200", {
      method: "GET",
      headers: {},
      body: undefined,
    });
  ...
  it("passes server-side pagination and filter parameters to the activity endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/user-1/activities?limit=12&offset=12&sport_category=run&min_distance_km=10", {
      method: "GET",
      headers: {},
      body: undefined,
    });
...
describe("master plan API clients", () => {
  ...
  it("returns null only when the current season-plan endpoint returns 404", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(new Response(JSON.stringify({ detail: "not found" }), { status: 404 }));
  ...
  it("rejects non-404 current season-plan failures", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(new Response(JSON.stringify({ error: "internal error" }), { status: 500 }));
  ...
  it("posts adjust messages with history to the active master plan endpoint", async () => {
    ...
    const response = await sendMasterPlanAdjustMessage("plan-1", "减量一周", [{ role: "assistant", content: "当前计划正常" }]);
