...
describe("notificationsStore server-backed read state", () => {
  ...
  it("hides historical notifications for users who completed onboarding later", () => {
    ...
    expect(getNotificationsForUser("2026-04-23T00:00:00+08:00").map((n) => n.id)).toEqual(NOTIFICATIONS.map((n) => n.id));
  });
  ...
  it("marks a notification read through the API and keeps local render state in sync", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ read_ids: [], notifications: [] }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }),
      )
  ...
  it("hydrates before marking read when called from an idle store", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ read_ids: [newestNotificationId, secondNotificationId] }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }),
      );
  ...
  it("hydrates server inbox notifications and treats updated unread state as authoritative", async () => {
    ...
    expect(useNotificationsStore.getState().unreadCount([useNotificationsStore.getState().serverNotifications[0]])).toBe(1);
  });
  ...
  it("marks hydrated server inbox notifications read immediately after API success", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ read_ids: ["sync:full"] }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        }),
      );
    ...
    expect(useNotificationsStore.getState().unreadCount([useNotificationsStore.getState().serverNotifications[0]])).toBe(0);
  });
});
