...
改了**手机号 + 验证码登录**时，还要跑手机号这一段：`npm run smoke:web:local:sms`（= `smoke:web:local` + `STRIDE_SMS_TEST_MODE=1`）。它只在后端处于 SMS 测试模式（固定码 `123456`，不发真短信）时可用 —— 即把 Vite 代理指到 stride-devops 的本地栈（`VITE_DEV_API_PROXY=http://localhost:5173`，auth-backend 默认已开 `AUTH_SMS_TEST_MODE=true`）。它用一个随机新手机号走完发送 → 倒计时 → 验证 → 自动注册，所以每跑一次都会新建一个账号。不开 `STRIDE_SMS_TEST_MODE` 时该段自动跳过（默认代理是 prod，真短信的码脚本读不到）。
