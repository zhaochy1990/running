...
**命名规则（HARD）**：训练负荷 / 恢复相关指标写成"中文名(英文缩写)"，例如 `静息心率(RHR)`、`心率变异性(HRV)`、`急性训练负荷(ATI)`、`慢性训练负荷(CTI)`、`训练压力平衡(TSB)`。不要只写英文缩写。

包含：
- **疲劳度**：最新值 + 7天趋势（改善/恶化/稳定）
- **急性训练负荷(ATI) / 慢性训练负荷(CTI)**：数值 + 训练负荷比
- **训练负荷状态**：来自COROS（Low/Optimal/High/Very High）
- **静息心率(RHR)**：最新值 vs 基线（运动员档案中47 bpm）。RHR升高 = 疲劳信号。
- **恢复百分比**：来自dashboard
- **心率变异性(HRV)**：最近 last_night_avg + 7 天趋势（来自 `daily_hrv`，COROS 与 Garmin 都写）。结合 `baseline_balanced_low/upper` 判定是否落入个人 baseline 区间。若 `daily_hrv` 为空（首次同步、还没攒到数据），降级用 dashboard 的 `avg_sleep_hrv` 单点 + `hrv_normal_low/high`，并在报告里注明"暂时只有单点快照"。**必须**在"当前值"列把均值和正常范围一起展示（例如 `46 ms (正常 30-44)`），让用户一眼看出是否偏离。HRV 持续低于 baseline 或下降 = 恢复不良 / 过度训练信号。
- **训练压力平衡(TSB)**：CTI - ATI。TSB > 10 = 比赛就绪，-10~10 = 过渡区，-30~-10 = 正常训练，< -30 = 过度负荷

使用 TRAINING_PLAN.md 中的阈值：

| 指标 | 绿灯 | 黄灯 | 红灯 |
|------|------|------|------|
| 静息心率(RHR) | < 50 | 50-55 | > 55 |
| 心率变异性(HRV) | 稳定/上升 | 下降10% | 下降20% |
| 疲劳度 | < 40 | 40-50 | > 50 |
...
- **周跑量趋势**（最近4-6周表格）
- **配速区间 / 心率区间参考表**：基于运动员当前 lactate threshold / HRmax 计算的训练区间。**格式（HARD）**：用单列"区间"表示边界，不要拆"慢边 / 快边"两列。开口区间用 `≤` / `≥`，闭区间用 `慢边 – 快边`（配速）或 `下限 – 上限`（心率）。例：
  ...
  心率区间表同理：单列"心率"，例如 `≤ 130` / `130-145` / `≥ 167`（单位 bpm）。
- **心率区间分布**：实际 Z1-Z2 占比 vs 目标 80%。标记 Z3"垃圾跑量"是否过多。分布表的"区间"列也用同一格式（如 `Z2 (130-145)`）。
