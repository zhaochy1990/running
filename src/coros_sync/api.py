...
def get_training_plan(user: str):
    ...
    # Parse phase timeline from the "时间线总览" table only
    phases: list[dict] = []
    today = date.today()
    current_phase = None
    ...
    for name, y, sm, sd, em, ed in PHASE_DEFS:
        ...
        phase = {"name": name, "start": str(start_date), "end": str(end_date)}
        phases.append(phase)
        if start_date <= today <= end_date:
            current_phase = name

    # If before Phase 1 but after race (recovery period)
