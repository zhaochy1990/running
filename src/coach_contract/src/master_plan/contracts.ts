...
const CompletedOutcomeSchema = OutcomeIdentitySchema.extend({
	...
	.superRefine((outcome, ctx) => {
		const ids = outcome.artifact.strategy_candidates.map(
		...
		for (const id of ids) {
			const reports = outcome.artifact.judgments.filter(
				(judgment) => judgment.candidate_id === id,
			);
			if (
				reports.length !== 3 ||
				new Set(reports.map((report) => report.judge)).size !== judges.length
			)
				ctx.addIssue({
					code: "custom",
		...
		if (
			outcome.artifact.judgments.some(
				(judgment) => !ids.includes(judgment.candidate_id),
			)
		)
			ctx.addIssue({
				code: "custom",
		...
		if (
			!ids.includes(outcome.artifact.selected_strategy.candidate.candidate_id)
		)
			ctx.addIssue({
				code: "custom",
		...
		const reviewerTypes = outcome.artifact.review_reports.map(
			(report) => report.reviewer_type,
		);
		if (
			outcome.artifact.review_reports.some(
				(report) =>
					report.artifact_revision !== outcome.artifact.artifact_revision,
			) ||
			REQUIRED_REVIEWERS.some(
				(reviewer) =>
					reviewerTypes.filter((item) => item === reviewer).length !== 1,
			)
		)
			ctx.addIssue({
				code: "custom",
				...
				message:
					"completed outcome requires one current report per required reviewer",
			});
		try {
			const expected = adjudicateMasterPlanReviews(
				outcome.artifact.artifact_revision,
				outcome.artifact.review_reports,
			...
			const expectedWarnings = expected.issues
				.filter((item) => item.severity === "warning")
				.map((item) => item.issue_id);
			if (
				expected.decision === "revise" ||
				expected.decision === "block" ||
				JSON.stringify(expected) !==
					JSON.stringify(outcome.artifact.adjudication)
			)
				ctx.addIssue({
					code: "custom",
					path: ["artifact", "adjudication"],
					message:
						"completed outcome requires the deterministic current adjudication",
				});
			if (
				JSON.stringify(expectedWarnings) !==
				JSON.stringify(outcome.artifact.warnings)
			)
				ctx.addIssue({
					code: "custom",
