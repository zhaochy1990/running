...
import { DataProviderWeeklyPlanContextProvider } from "./weeklyPlanContextProvider.js";
...
function providerUsingMySqlFeedback(store: ConstructorParameters<typeof DataProviderWeeklyPlanContextProvider>[0]): DataProviderWeeklyPlanContextProvider {
  return new DataProviderWeeklyPlanContextProvider(store, {
...
test("weekly context combines all weekly planning evidence", async () => {
  ...
  assert.equal((snapshot.fitness_state.stride_training_load as Record<string, unknown>).load_ratio, 1.1);
...
test("weekly context keeps a Monday as the planning start", async () => {
  ...
  const provider = new DataProviderWeeklyPlanContextProvider({
    async getUserProfile() {
      return null;
    },
    async getMasterPlanMetadataForDate() {
...
test("weekly context reports unavailable STRIDE load without fallback", async () => {
  ...
  const snapshot = await provider.loadSnapshot("athlete", "2026-08-14T00:00:00Z");
...
test("weekly context treats null PMC values as not computed", async () => {
  ...
  const snapshot = await provider.loadSnapshot("athlete", "2026-08-14T00:00:00Z");
  assert.equal((snapshot.fitness_state.stride_training_load as Record<string, unknown>).available, false);
});
