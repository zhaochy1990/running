import { addDays, mondayOnOrBefore as monday, planningStartDate, shanghaiDay } from "@stride/contract";
import { type ContextSnapshot, ContextSnapshotSchema, type MasterPlanContextProvider } from "../graph/master_plan/index.js";
...
export class DataProviderMasterPlanContextProvider implements MasterPlanContextProvider {
  constructor(private readonly store: ContextStore) {}

  async loadSnapshot(userId: string, asOf = new Date().toISOString()): Promise<ContextSnapshot> {
    const end = shanghaiDay(asOf);
    // past 2 years
    const macroStart = addDays(end, -730);
    const recentStart = addDays(end, -111);
    const [profile, injuries, activities, loads, recovery, pbs, calibration, races, activePlan, raceTarget] = await Promise.all([
...
const durationWeightedActivityAverage = (activities: Activity[], metric: (activity: Activity) => number | null): number | null => {
  const measured = activities.filter((activity) => metric(activity) !== null && (activity.durationS ?? 0) > 0);
  const duration = measured.reduce((total, activity) => total + (activity.durationS ?? 0), 0);
  return duration ? Math.round(measured.reduce((total, activity) => total + (metric(activity) ?? 0) * (activity.durationS ?? 0), 0) / duration) : null;
};
...
function injuryShape(rows: UserInjury[]) {
  return rows.map((injury) => ({
    ...
    occurred_on: null,
    source: "data_provider.user_injury",
  }));
}
...
function raceShape(rows: RaceEffort[]) {
  return rows.map(({ date, distanceKm, durationMin, avgPaceSKm, avgHr, maxHr, feel }) => ({
    date,
...
function weeklyHistory(runs: Activity[], loads: DailyTrainingLoad[], recovery: DailyRecovery[], start?: string, end?: string) {
  const keys = new Set([...runs.map((a) => monday(activityDay(a))), ...loads.map((x) => monday(x.date)), ...recovery.map((x) => monday(x.date))]);
  if (start && end) for (let day = monday(start); day <= monday(end); day = addDays(day, 7)) keys.add(day);
  return [...keys].sort().map((week) => {
    ...
    return {
      week_start: week,
      distance_km: round(acts.reduce((s, a) => s + (a.distanceM ?? 0), 0) / 1000),
      ...
      long_run_km: acts.length ? round(Math.max(...acts.map((a) => a.distanceM ?? 0)) / 1000) : null,
      speed_session_count: new Set(acts.filter(isQualityRunningActivity).map(activityDay)).size,
      race_count: acts.filter((a) => /race|比赛|马拉松/i.test(a.name ?? "")).length,
...
function macroHistory(runs: Activity[], start: string, end: string) {
  ...
  const intervals = dates.length ? [[start, dates[0]!], ...dates.slice(1).map((date, i) => [dates[i]!, date]), [dates.at(-1)!, end]] : [[start, end]];
  ...
  return {
    start_date: start,
    end_date: end,
    months: [...months].sort().map(([month, xs]) => ({
      month,
      distance_km: round(xs.reduce((s, a) => s + (a.distanceM ?? 0), 0) / 1000),
      hours: round(xs.reduce((s, a) => s + (a.durationS ?? 0), 0) / 3600, 2),
      avg_pace_s_km: durationWeightedActivityAverage(xs, (a) => a.avgPaceSKm),
      avg_hr: durationWeightedActivityAverage(xs, (a) => a.avgHr),
      run_count: xs.length,
    })),
    peak_weekly_distance_km: weeks.length ? Math.max(...weeks.map((w) => w.distance_km)) : null,
    longest_run_km: runs.length ? round(Math.max(...runs.map((a) => a.distanceM ?? 0)) / 1000) : null,
    longest_road_run_km: roadRuns.length ? round(Math.max(...roadRuns.map((a) => a.distanceM ?? 0)) / 1000) : null,
    gap_periods: gaps,
    consistency_pct: weeks.length ? round((100 * weeks.filter((w) => w.run_count > 0).length) / Math.max(1, Math.ceil(dayDiff(start, end) / 7))) : null,
  };
}
...
function continuity(runs: Activity[], p: ActiveMasterPlanMetadata | null, end: string) {
...
function coverage(
  ...
  return [
    ...
    },
    {
      domain: "active_plan",
      status: "complete",
      detail: plan ? null : "no active plan, expected for new-season generation",
    },
  ] as const;
}
...
function isRoadRun(run: Activity): boolean {
  const token = `${run.sport ?? ""} ${run.sportName ?? ""} ${run.name ?? ""}`.toLowerCase();
  if (/trail|越野|track|场地|treadmill|跑步机|indoor|室内/.test(token)) return false;
