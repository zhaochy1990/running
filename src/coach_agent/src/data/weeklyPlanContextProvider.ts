import { addDays, mondayOnOrBefore as monday, planningStartDate, shanghaiDay, weekFolder } from "@stride/contract";
import { median } from "../utils/statistics.js";
import { isQualityRunningActivity, isRunningActivity } from "./activityClassification.js";
import type {
...
import { createWeeklyFeedbackSource, type WeeklyFeedbackRecord, type WeeklyFeedbackSource } from "./weeklyFeedbackSource.js";
...
export class DataProviderWeeklyPlanContextProvider implements WeeklyPlanContextProvider {
  ...
  constructor(
    ...
    this.feedbackSource = createWeeklyFeedbackSource(store, {
      ...(options.weeklyFeedbackCutoverComplete === undefined ? {} : { cutoverComplete: options.weeklyFeedbackCutoverComplete }),
      ...(options.legacyDataDir === undefined ? {} : { legacyDataDir: options.legacyDataDir }),
    });
  }

  public async loadSnapshot(userId: string, asOf: string): Promise<WeeklyPlanContext> {
    ...
    const trainingHistoryStart = earliestWeekStart !== undefined && earliestWeekStart < start ? earliestWeekStart : start;
    const [profile, plan, activities, feedback, loads, recovery, injuries, calibration, hrvBaseline, ...weeklyPlans] = await Promise.all([
      ...
      ...recentWeekStarts.map((weekStart) => this.store.getWeeklyPlan(userId, weekFolder(weekStart))),
    ]);

    const recentWeeks = recentTrainingWeeks(recentWeekStarts, weeklyPlans, activities, end);
    return {
      as_of: end,
      ...
      recent_activities: activities.filter((activity) => activityDay(activity) >= start).map(activityShape),
...
function trainingPosition(plan: ActiveMasterPlanMetadata | null, day: string) {
  ...
  const stage = weeks.find((candidate) => string(candidate.week_start) === weekStart);
  return {
    phase: phase
      ? {
          ...
          milestones: milestones
            .filter((milestone) => containsDay(phase, string(milestone.date) ?? ""))
            .map((milestone) => ({
    ...
    stage: stage
      ? {
          ...
          key_sessions: Array.isArray(stage.key_sessions) ? stage.key_sessions : [],
        }
      : null,
  };
}
...
function recentTrainingWeeks(weekStarts: string[], plans: Array<Record<string, unknown> | null>, activities: Activity[], asOf: string): RecentTrainingWeek[] {
    ...
    return {
      ...
      actual: {
        total_run_distance_km: round(
          runActivities.reduce((sum, activity) => sum + (activity.distanceM ?? 0) / 1000, 0),
          2,
        ),
        total_training_dose: round(
          weekActivities.reduce((sum, activity) => sum + (activity.strideDose ?? 0), 0),
          2,
        ),
        run_days: new Set(runActivities.map(activityDay)).size,
        longest_run: longestRunDay,
        quality_stimulus_days: [...qualityByDay.entries()].map(([date, dayActivities]) => ({
          date,
          training_types: unique(dayActivities.map((activity) => activity.strideSessionClass)),
...
function plannedWeekShape(plan: Record<string, unknown> | null): PlannedWeekSummary {
  if (!plan) return { available: false, total_run_distance_km: null, run_sessions: [] };
  ...
  const completeDistance = runSessions.every((session) => session.distance_km !== null);
  return {
    available: true,
    distance_coverage: completeDistance ? "complete" : "partial",
    total_run_distance_km: completeDistance
      ? round(
          runSessions.reduce((sum, session) => sum + (session.distance_km ?? 0), 0),
          2,
        )
      : null,
...
function activityDistanceKm(activities: Activity[]): number {
  return round(
    activities.reduce((sum, activity) => sum + (activity.distanceM ?? 0) / 1000, 0),
    2,
  );
}
...
function activityShape(activity: Activity) {
  return {
    ...
    distance_km: activity.distanceM === null ? null : round(activity.distanceM / 1000, 2),
    duration_min: activity.durationS === null ? null : round(activity.durationS / 60, 1),
...
function fitnessShape(loads: DailyTrainingLoad[], recovery: DailyRecovery[], day: string): WeeklyFitnessState {
  const latest = loads.at(-1);
  const available = latest !== undefined && [latest.acuteLoad, latest.chronicLoad, latest.form, latest.loadRatio].some((value) => value !== null);
  if (!latest || !available) {
    return {
      as_of_date: day,
...
function fitnessTrend(loads: DailyTrainingLoad[], recovery: DailyRecovery[]): FitnessTrendPoint[] {
  const loadsByDate = new Map(loads.map((load) => [load.date, load]));
  const recoveryByDate = new Map(recovery.map((point) => [point.date, point]));
  const dates = [...new Set([...loadsByDate.keys(), ...recoveryByDate.keys()])].sort().slice(-14);
...
function recoveryShape(recovery: DailyRecovery[], asOfDay: string) {
  const measured = recovery.filter((row) => row.rhr !== null || row.hrv !== null);
  const latest = measured.at(-1) ?? null;
  const sevenDayStart = addDays(asOfDay, -6);
  const recent = measured.filter((row) => row.date >= sevenDayStart && row.date <= asOfDay);
  return {
    latest,
    seven_day_average: {
...
function userProfileShape(
  ...
  return {
    ...
    threshold_pace_s_per_km: calibration?.thresholdSpeedMps && calibration.thresholdSpeedMps > 0 ? round(1000 / calibration.thresholdSpeedMps) : null,
...
function ageOn(asOfDay: string, dob: string | null): number | null {
  ...
  return birthdayThisYear <= asOfDay ? asOfYear - dobYear : asOfYear - dobYear - 1;
}

function average(values: Array<number | null>): number | null {
  const measured = values.filter((value): value is number => value !== null);
  return measured.length ? round(measured.reduce((sum, value) => sum + value, 0) / measured.length, 1) : null;
}
