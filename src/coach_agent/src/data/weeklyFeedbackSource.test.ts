...
test("weekly feedback source follows the cutover marker", () => {
  ...
  assert.ok(createWeeklyFeedbackSource(store, { cutoverComplete: false }) instanceof LegacyFileWeeklyFeedbackSource);
  assert.ok(createWeeklyFeedbackSource(store, { cutoverComplete: true }) instanceof DataProviderWeeklyFeedbackSource);
});

test("MySQL feedback source delegates the bounded range", async () => {
  const calls: unknown[][] = [];
  const source = new DataProviderWeeklyFeedbackSource({
  ...
  assert.deepEqual(await source.getByDateRange("athlete", "2026-06-01", "2026-06-30"), []);
...
test("legacy feedback source uses completed weeks, not unstable file mtimes", async (t) => {
  ...
  await Promise.all([
    writeFile(join(logsDir, "2026-05-25_05-31(P1W4)", "feedback.md"), "too old"),
    writeFile(join(logsDir, "2026-06-01_06-07(P2W1)", "feedback.md"), "first week"),
    writeFile(join(logsDir, "2026-06-08_06-14(P2W2)", "feedback.md"), "second week"),
  ]);
  await Promise.all([
    utimes(join(logsDir, "2026-05-25_05-31(P1W4)", "feedback.md"), new Date("2026-05-31T00:00:00Z"), new Date("2026-05-31T00:00:00Z")),
    utimes(join(logsDir, "2026-06-01_06-07(P2W1)", "feedback.md"), new Date("2026-07-01T00:00:00Z"), new Date("2026-07-01T00:00:00Z")),
    utimes(join(logsDir, "2026-06-08_06-14(P2W2)", "feedback.md"), new Date("2026-06-01T00:00:00Z"), new Date("2026-06-01T00:00:00Z")),
  ]);

  const rows = await new LegacyFileWeeklyFeedbackSource(dataDir).getByDateRange("athlete-1", "2026-06-01", "2026-06-08");
...
test("legacy feedback source rejects unsafe user paths", async () => {
  ...
  await assert.rejects(() => source.getByDateRange("../athlete", "2026-06-01", "2026-06-08"), /invalid weekly feedback user id/);
});
