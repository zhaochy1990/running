...
4. 赛前减量期（Taper Phase）：这个周期一般时长2周，其中第二周为比赛周，从第一周逐步减少训练量，但需要保持训练强度（减跑量不减跑步强度），调整身体状态，确保身体充分恢复，为比赛做好准备，注意要把比赛本身的负荷计算进去。对于全马比赛，赛前减量期的第一周长距离通常不超过25km，对于半马比赛，赛前减量期的第一周长距离训练通常不超过15km。
...
2. 我们要根据用户设定的比赛日，把比赛的公里数和预计负荷也安排在当周的训练计划里，不要遗漏比赛的负荷。
3. 当用户的备赛时间不足8周时，我们不安排测试赛（自测），因为测试赛会占用训练时间，影响训练计划的连续性和训练效果。
