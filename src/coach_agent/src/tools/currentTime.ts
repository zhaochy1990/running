...
export function createCurrentTimeTools(now: Date = new Date()): StructuredTool[] {
  return defineCoachTools([
    {
      ...
      handler: () => {
        ...
        const nextWeek = weekNameForShanghaiDate(nextMonday.toISOString().slice(0, 10));
        return { timezone: "Asia/Shanghai", today, current_week: currentWeek, next_week: nextWeek };
      },
    },
  ]);
}
