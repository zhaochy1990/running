...
class MasterPlanContextTool {
  ...
  async getMasterPlanContext(_input: z.infer<typeof getMasterPlanContextSchema>, runtime: CoachToolRuntime) {
    const userId = runtime.context?.userId;
    const asof = runtime.context?.asof;
    if (!userId) {
      throw new Error("get_master_plan_context: missing userId in runtime context");
    }
    if (!asof) {
      throw new Error("get_master_plan_context: missing asof in runtime context");
    }
 ...
 */
export function createMasterPlanContextTools(provider: MasterPlanContextProvider): StructuredTool[] {
