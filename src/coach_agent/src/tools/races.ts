...
const getRaceHistorySchema = z.object({
	minDistanceKm: z
		...
		.optional()
		.describe(
			"比赛候选的最小距离（公里），缺省 20（半马及以上）。想连长距离拉练一起看可调小。",
		),
	limit: z
		.number()
		.int()
...
class RaceToolImpl {
	...
	async getPersonalBests(
		_input: z.infer<typeof getPersonalBestsSchema>,
		runtime: CoachToolRuntime,
...
export function createRaceTools(store: DataProvider): StructuredTool[] {
	...
	return defineCoachTools([
		...
		},
		{
			...
			description:
				"获取运动员各标准距离（5K/10K/HM/FM 等）的个人最好成绩（用时秒 + 取得日期），作为“这次比赛本该多快”的参照系。",
