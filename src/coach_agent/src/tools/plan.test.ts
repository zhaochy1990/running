...
test("StrideDataStore queries active structured plans by user and week", async () => {
  ...
  const pool = {
    async query(sql: string, values: unknown[]) {
      ...
      return [
        [{ content: JSON.stringify({ plan_id: queries.length === 1 ? "master-1" : "week-1" }) }],
      ];
    },
  };
  ...
  assert.deepEqual(await store.getWeeklyPlan(userId, "2026-07-20_07-26"), { plan_id: "week-1" });
  assert.match(queries[0]!.sql, /user_id = \? AND content_version = 2 AND status = 'active'/);
  assert.deepEqual(queries[0]!.values, [userId]);
  assert.match(queries[1]!.sql, /user_id = \? AND week_start = \? AND content_version = 2 AND status = 'active'/);
  assert.deepEqual(queries[1]!.values, [userId, "2026-07-20"]);
});

test("StrideDataStore rejects malformed plan JSON", async () => {
  const pool = { async query() { return [[{ content: "not-json" }]]; } };
  const store = new StrideDataStore(pool as never);
  await assert.rejects(() => store.getMasterPlan(userId), /master_plan contains invalid JSON/);
});
