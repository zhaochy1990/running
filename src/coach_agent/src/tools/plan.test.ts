...
test("StrideDataStore queries structured plans by requested day and week", async () => {
  ...
  assert.match(queries[0]!.sql, /user_id = \?[\s\S]*content_version = 2[\s\S]*status IN \('active', 'archived'\)/);
...
test("StrideDataStore rejects malformed plan JSON", async () => {
  ...
  const store = new StrideDataStore(pool as never);
  await assert.rejects(() => store.getMasterPlan(userId, "2026-07-20"), /master_plan contains invalid JSON/);
});
...
test("StrideDataStore rejects ambiguous overlapping master plans", async () => {
  ...
  const store = new StrideDataStore(pool as never);
  await assert.rejects(() => store.getMasterPlan(userId, "2026-06-15"), /multiple master plans cover 2026-06-15/);
});
