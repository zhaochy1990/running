...
export function estimatePlannedRunLoad(
  ...
  for (const block of workout.blocks)
    for (let repetition = 0; repetition < block.repeat; repetition += 1)
      for (const step of block.steps) {
        ...
        const intensity = stepIntensity(step, thresholdSpeedMps, thresholdHr, rhr);
        if (!intensity) {
          unestimatedSteps += 1;
          continue;
        }
        const minutes = stepMinutes(step, intensity.expected, thresholdSpeedMps);
        if (!minutes) {
          unestimatedSteps += 1;
          continue;
        }
        ...
        if (step.duration.kind === "distance_m") {
          ...
          lowMinutes = stepMinutes(step, intensity.low, thresholdSpeedMps) ?? minutes;
          highMinutes = stepMinutes(step, intensity.high, thresholdSpeedMps) ?? minutes;
        } else {
          estimatedDistanceKm += (minutes * 60 * thresholdSpeedMps * intensity.expected) / 1000;
        }
  ...
  if (lowDose === null || expectedDose === null || highDose === null) return null;
  return {
    lowDose,
    expectedDose,
    highDose,
...
function stepIntensity(step: Step, thresholdSpeedMps: number, thresholdHr?: number | null, rhr?: number | null): IntensityTriple | null {
  const target = step.target;
  if (target.kind === "pace_s_km" && target.low !== null && target.high !== null && target.low > 0 && target.high > 0) {
  ...
  if (target.kind === "hr_bpm" && target.low !== null && target.high !== null && target.low > 0 && target.high > 0) {
    if (!thresholdHr || rhr === null || rhr === undefined || thresholdHr <= rhr) return null;
...
function stepMinutes(step: Step, intensity: number, thresholdSpeedMps: number): number | null {
...
function doseFromDwells(dwells: readonly Dwell[]): number | null {
  ...
  const weighted = dwells.reduce((sum, [minutes, intensity]) => sum + minutes * intensity ** NORMALIZATION_EXPONENT, 0);
  const normalizedIf = (weighted / totalMinutes) ** (1 / NORMALIZATION_EXPONENT);
