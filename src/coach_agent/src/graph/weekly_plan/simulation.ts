...
const DailySimulationSchema = z.strictObject({
	date: z.iso.date(),
...
const SessionSimulationSchema = z.strictObject({
	date: z.iso.date(),
...
export function simulateWeeklyPlanLoad(
	plan: WeeklyPlan,
	...
	if (thresholdSpeed === null)
		missingReasons.push("threshold_speed_calibration_missing");
	for (const session of plan.sessions) {
		if (session.kind !== "run") continue;
		const declaredDistance =
			session.total_distance_m === null
				? null
				: session.total_distance_m / 1000;
		const estimate =
			thresholdSpeed !== null && session.spec !== null
				? estimatePlannedRunLoad(session.spec, thresholdSpeed, thresholdHr, rhr)
				: null;
		let missingReason: string | null = null;
		if (session.spec === null) missingReason = "run_workout_structure_missing";
		else if (thresholdSpeed === null)
			missingReason = "threshold_speed_calibration_missing";
		else if (estimate === null || estimate.unestimatedSteps > 0)
			missingReason = "planned_session_uncomputable";

		const sessionAssumptions = estimate?.assumptions ?? [];
		if (
			estimate &&
			declaredDistance !== null &&
			materiallyDifferent(declaredDistance, estimate.estimatedDistanceKm)
		) {
		...
		sessionReports.push({
			...
			estimated_dose_high: missingReason
				? null
				: round(estimate?.highDose ?? 0),
			declared_distance_km: declaredDistance,
			estimated_structured_distance_km:
				estimate === null ? null : round(estimate.estimatedDistanceKm),
	...
	const maxSessionDose = available
		? Math.max(
				0,
	...
	return WeeklyPlanSimulationReportSchema.parse({
		...
		estimated: true,
		provenance:
			"deterministic run-workout/v1 segment integration; actual session dates drive 7/42-day PMC projection",
		available,
		...
		maximum_session_dose_share:
			totalDose && maxSessionDose !== null
				? round(maxSessionDose / totalDose)
				: null,
...
function dailyDoses(
	...
	return Array.from({ length: 7 }, (_, index) => {
		...
		return sessions
			...
			.reduce((total, session) => total + (session[field] ?? 0), 0);
	});
}
...
function projectPmc(
	...
	if (gapDays < 0)
		throw new Error("weekly plan starts before the fitness-state snapshot");
	const beforePlan =
		gapDays > 1
			? simulatePmcDays(
			...
			: null;
	return simulatePmcDays(
		doses,
		beforePlan ? { atl: beforePlan.atl, ctl: beforePlan.ctl } : initial,
	);
}
