...
import { createWeeklyPlanSimulationContext as context, createWeeklyPlanForSimulation as weeklyPlan } from "./testFixtures.js";
...
test("weekly simulator uses real session dates and projects the 7/42 PMC path", () => {
  ...
  assert.equal(report.days[0]?.estimated_dose, report.sessions[0]?.estimated_dose);
  assert.equal(report.days[1]?.estimated_dose, 0);
  assert.equal(report.days[2]?.estimated_dose, report.sessions[1]?.estimated_dose);
  assert.ok(report.days.every((day) => day.end_ctl !== null));
  assert.ok(report.sessions[1]?.load_assumptions.includes("heart_rate_target_used_as_intensity_proxy"));
});
...
test("weekly simulator rejects incomplete structured distance coverage", () => {
  ...
  assert.deepEqual(report.missing_dose_reasons, ["structured_distance_differs_from_session_total"]);
...
test("weekly simulator reports missing athlete calibration without fallback constants", () => {
  ...
  assert.equal(report.available, false);
  assert.deepEqual(report.missing_dose_reasons, ["threshold_speed_calibration_missing"]);
...
test("weekly simulator flags three consecutive projected overreach days", () => {
  ...
  assert.deepEqual(report.safety_issues, ["planned_load_extends_overreach_more_than_1_25_to_3_consecutive_days"]);
});
...
test("weekly simulator does not blame a rest plan for pre-existing overreach", () => {
  ...
  assert.ok(report.load_assumptions.includes("preexisting_overreach_persists_without_planned_load"));
});
