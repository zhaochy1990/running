...
const IF_BY_TYPE: Record<
	MasterPlan["weeks"][number]["key_sessions"][number]["type"],
	number
> = {
	long_run: 0.78,
	threshold: 0.98,
...
export const SimulationWeekSchema = z
	.object({
		...
		provenance: z.enum([
			"weekly_midpoint+key_sessions+remaining_easy_volume",
...
export const SimulationReportSchema = z
	.object({
		algorithm_version: z.literal("master-plan-load-v1"),
		estimated: z.literal(true),
		provenance: z.string(),
...
export function simulateMasterPlanLoad(
	...
	const weeks = plan.weeks.map((week) => {
		const weeklyKm =
			(week.target_weekly_km_low + week.target_weekly_km_high) / 2;
		let keyDistance = 0;
		let keyDose = 0;
		let longRunDose = 0;
		let uncertain = false;
		for (const session of week.key_sessions) {
			...
			if (
				...
				duration = (session.distance_km / (threshold * 3.6 * intensity)) * 60;
			if (duration === null) {
				uncertain = true;
				continue;
			}
			if (
				...
				intensity = Math.max(
					...
					Math.min(
						1.2,
						(session.distance_km * 1000) / (duration * 60) / threshold,
					),
				);
			...
			if (session.distance_km !== null) keyDistance += session.distance_km;
				...
				keyDistance += (duration / 60) * threshold * 3.6 * intensity;
			else uncertain = true;
		...
		const easyDose =
			threshold && threshold > 0
				? (easyDistance / (threshold * 3.6 * 0.75)) * 0.75 ** 2 * 100
				: 0;
		const estimatedDose =
			threshold && threshold > 0 ? keyDose + easyDose : null;
		...
		return {
			...
			provenance:
				week.week_index === 1 && partialCurrentWeek
					? ("partial_current_week+weekly_midpoint+key_sessions+remaining_easy_volume" as const)
					: ("weekly_midpoint+key_sessions+remaining_easy_volume" as const),
			confidence:
				estimatedDose === null
					? ("low" as const)
					: !initialPmcAvailable ||
			...
			long_run_dose_share:
				estimatedDose && estimatedDose > 0
					? round(longRunDose / estimatedDose)
					: null,
		};
	});
	return SimulationReportSchema.parse({
		algorithm_version: "master-plan-load-v1",
		estimated: true,
		provenance:
			"deterministic strategic estimate; threshold-speed calibrated TSS scale; availability-adjusted Mon-Sun shares per week",
		weeks,
	});
}
...
function distributionForWeek(
	...
	const retainedDays = eligible
		.map((allowed, index) => ({ allowed, index, share: DISTRIBUTION[index]! }))
