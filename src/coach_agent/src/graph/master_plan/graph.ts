...
import { simulateMasterPlanLoad, type SimulationReport } from "./simulation.js";
import { runMasterPlanRuleFilter, type RuleReport } from "./rules.js";
import { ReviewReportSchema, ReviewWorkerErrorSchema, adjudicateMasterPlanReviews, mergeReviewReportsByRevisionAndTask, mergeReviewWorkerErrors, reviewTaskId, REQUIRED_REVIEWERS, type ReviewAdjudication, type ReviewerType, type ReviewReport, type ReviewWorkerError } from "./review.js";
...
interface ReviewModel { invoke(input: { reviewerType: ReviewerType; reviewTaskId: string; artifactRevision: number; request: MasterPlanGraphRequest; plan: z.infer<typeof MasterPlanSchema>; facts: AssessmentFacts; athleteAssessment: AthleteAssessment; goalAssessment: GoalAssessment; selectedStrategy: SelectedStrategy; simulationReport: SimulationReport; ruleReport: RuleReport }): Promise<unknown>; }
...
const GraphState = new StateSchema({
  ...
  simulationReport: z.custom<SimulationReport>().optional(), ruleReport: z.custom<RuleReport>().optional(),
  reviewerType: z.custom<ReviewerType>().optional(), reviewTaskId: z.string().optional(), artifactRevision: z.int().positive().default(1),
  reviewReports: new ReducedValue(z.array(ReviewReportSchema).default(() => []), { reducer: mergeReviewReportsByRevisionAndTask }),
  reviewWorkerErrors: new ReducedValue(z.array(ReviewWorkerErrorSchema).default(() => []), { reducer: mergeReviewWorkerErrors }), adjudication: z.custom<ReviewAdjudication>().optional(),
});
...
export function createMasterPlanGraph(dependencies: MasterPlanGraphDependencies) {
  ...
  const initialize = async (state: typeof GraphState.State, runtime: { context?: MasterPlanGraphContext }) => {
    ...
    if (request.requested_mode !== "new_season") return { context, artifactRevision, outcome: MasterPlanGraphOutcome.parse({ decision: "unsupported", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "capability_gap", requested_mode: request.requested_mode, supported_modes: ["new_season"] } }) };
  ...
  const expandSkeleton = async (state: typeof GraphState.State) => {
    ...
    if (schemaReport.violations.some((item) => item.rule_id === "schema_validity")) return { ruleReport: schemaReport, outcome: qualityFailureWithRuleReport(request, context, schemaReport) };
  ...
  const finalize = (state: typeof GraphState.State) => {
    ...
    return { outcome: MasterPlanGraphOutcome.parse({ decision: "completed", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "master_plan_draft", activation_status: "inactive", plan, facts, athlete_assessment: athleteAssessment, goal_assessment: goalAssessment, strategy_candidates: state.strategyCandidates, judgments: state.judgments, selected_strategy: state.selectedStrategy, simulation_report: state.simulationReport, rule_report: state.ruleReport, artifact_revision: state.artifactRevision, review_reports: state.reviewReports, adjudication: state.adjudication, warnings: state.adjudication?.issues.filter((item) => item.severity === "warning").map((item) => item.issue_id) ?? [] } }) };
  };
  const simulateLoad = (state: typeof GraphState.State) => { try { return { simulationReport: simulateMasterPlanLoad(state.plan!, state.snapshot!, state.request) }; } catch (error) { return { outcome: qualityFailure(state.request, state.context!, `load_simulation_failed:${error instanceof Error ? error.message : "unknown"}`) }; } };
  const filterRules = (state: typeof GraphState.State) => { const report = runMasterPlanRuleFilter(state.plan!, state.request, state.snapshot!); return report.has_errors ? { ruleReport: report, outcome: qualityFailureWithReports(state.request, state.context!, state.simulationReport!, report) } : { ruleReport: report }; };
  const validateSelected = (state: typeof GraphState.State) => { try { validateSkeletonAgainstStrategy(state.plan!, state.selectedStrategy!, state.athleteAssessment!); return {}; } catch (error) { return { outcome: qualityFailureWithReports(state.request, state.context!, state.simulationReport!, state.ruleReport!, `candidate_plan_strategy_mismatch:${error instanceof Error ? error.message : "unknown"}`) }; } };
  const reviewWorker = async (state: typeof GraphState.State) => { const input = { reviewerType: state.reviewerType!, reviewTaskId: state.reviewTaskId!, artifactRevision: state.artifactRevision, request: state.request, plan: state.plan!, facts: state.facts!, athleteAssessment: state.athleteAssessment!, goalAssessment: state.goalAssessment!, selectedStrategy: state.selectedStrategy!, simulationReport: state.simulationReport!, ruleReport: state.ruleReport! }; try { const report = ReviewReportSchema.parse(await dependencies.reviewModel.invoke(input)); if (report.review_task_id !== input.reviewTaskId || report.reviewer_type !== input.reviewerType || report.artifact_revision !== input.artifactRevision) throw new Error("review identity mismatch"); return { reviewReports: [report] }; } catch (error) { return { reviewWorkerErrors: [ReviewWorkerErrorSchema.parse({ review_task_id: input.reviewTaskId, reviewer_type: input.reviewerType, artifact_revision: input.artifactRevision, kind: isInfrastructureError(error) ? "infrastructure" : "contract", code: isInfrastructureError(error) ? "review_model_unavailable" : "review_contract_invalid" })] }; } };
  const adjudicateReviews = (state: typeof GraphState.State) => { const currentErrors = state.reviewWorkerErrors.filter((item) => item.artifact_revision === state.artifactRevision); if (currentErrors.some((item) => item.kind === "infrastructure")) return { outcome: infrastructureFailure(state.request, state.context!, "review_model_unavailable") }; if (currentErrors.length) return { outcome: qualityFailureWithReviews(state, "review_contract_invalid", undefined, currentErrors) }; try { const adjudication = adjudicateMasterPlanReviews(state.artifactRevision, state.reviewReports, state.facts!, { simulation: state.simulationReport!, rules: state.ruleReport! }); return adjudication.decision === "pass" || adjudication.decision === "pass_with_warnings" ? { adjudication } : { adjudication, outcome: qualityFailureWithReviews(state, `review_${adjudication.decision}`, adjudication) }; } catch { return { outcome: qualityFailureWithReviews(state, "review_adjudication_invalid") }; } };
  ...
  const fanReviewers = (state: typeof GraphState.State) => REQUIRED_REVIEWERS.map((reviewerType) => new Send("review_worker", { ...sharedWorkerState(state), plan: state.plan, simulationReport: state.simulationReport, ruleReport: state.ruleReport, artifactRevision: state.artifactRevision, reviewerType, reviewTaskId: reviewTaskId(reviewerType, state.artifactRevision) }));
  ...
  return new StateGraph({ state: GraphState, input: GraphInput, output: GraphOutput, context: MasterPlanGraphContext })
    ...
    .addNode("expand_skeleton", expandSkeleton).addNode("simulate_load", simulateLoad).addNode("filter_rules", filterRules).addNode("validate_selected", validateSelected).addNode("review_worker", reviewWorker).addNode("adjudicate_reviews", adjudicateReviews).addNode("finalize", finalize)
    ...
    .addConditionalEdges("expand_skeleton", stopOr("simulate_load"), ["simulate_load", END]).addConditionalEdges("simulate_load", stopOr("filter_rules"), ["filter_rules", END])
    .addConditionalEdges("filter_rules", stopOr("validate_selected"), ["validate_selected", END])
    .addConditionalEdges("validate_selected", (state) => state.outcome ? END : fanReviewers(state), ["review_worker", END])
    .addEdge("review_worker", "adjudicate_reviews").addConditionalEdges("adjudicate_reviews", stopOr("finalize"), ["finalize", END]).addEdge("finalize", END).compile();
}
...
function qualityFailure(request: MasterPlanGraphRequest, context: MasterPlanGraphContext, ...issues: string[]) { return MasterPlanGraphOutcome.parse({ decision: "failed_quality_gate", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "quality_failure_report", unresolved_issues: issues.length ? issues : ["quality_gate_failed"], attempt_history: [] } }); }
function qualityFailureWithReports(request: MasterPlanGraphRequest, context: MasterPlanGraphContext, simulation: SimulationReport, rules: RuleReport, ...additionalIssues: string[]) { return MasterPlanGraphOutcome.parse({ decision: "failed_quality_gate", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "quality_failure_report", unresolved_issues: [...rules.violations.filter((item) => item.severity === "error").map((item) => item.rule_id), ...additionalIssues], attempt_history: [], simulation_report: simulation, rule_report: rules } }); }
function qualityFailureWithRuleReport(request: MasterPlanGraphRequest, context: MasterPlanGraphContext, rules: RuleReport) { return MasterPlanGraphOutcome.parse({ decision: "failed_quality_gate", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "quality_failure_report", unresolved_issues: rules.violations.filter((item) => item.severity === "error").map((item) => item.rule_id), attempt_history: [], rule_report: rules } }); }
function qualityFailureWithReviews(state: typeof GraphState.State, issue: string, adjudication?: ReviewAdjudication, errors: ReviewWorkerError[] = []) { return MasterPlanGraphOutcome.parse({ decision: "failed_quality_gate", request_id: state.request.request_id, generation_id: state.context!.generationId, artifact: { type: "quality_failure_report", unresolved_issues: [issue], attempt_history: [], simulation_report: state.simulationReport, rule_report: state.ruleReport, review_reports: state.reviewReports, review_worker_errors: errors, ...(adjudication ? { adjudication } : {}) } }); }
