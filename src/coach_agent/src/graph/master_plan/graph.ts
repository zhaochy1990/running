...
export function createMasterPlanGraph(
  ...
  const archetypes = dependencies.strategyArchetypes ?? [
    "conservative", // 保守
    "balanced", //平衡
    "aggressive_gated", // 激进（有门槛）
  ];
    ...
    try {
      const assessment = canonicalizeAssessmentSummary(
        AthleteAssessmentSchema.parse(
          await dependencies.assessmentModel.invoke({
            request,
            snapshot,
            facts,
          }),
    ...
    try {
      const assessment = canonicalizeAssessmentSummary(
        GoalAssessmentSchema.parse(
          await dependencies.goalAssessmentModel.invoke({
            request,
            snapshot,
            facts,
          }),
  ...
  const expandSkeleton = async (state: typeof GraphState.State) => {
    ...
    let raw: unknown;
    try {
      raw = await dependencies.skeletonModel.invoke({
        request,
        context,
        snapshot,
        facts,
  ...
  const reviewWorker = async (state: typeof GraphState.State) => {
    ...
    try {
      const report = ReviewReportSchema.parse(
        await dependencies.reviewModel.invoke(input),
      );
      if (
  ...
  const stopOr = (next: string) => (state: typeof GraphState.State) =>
    state.outcome ? END : next;
  const fanStrategies = (state: typeof GraphState.State) =>
    archetypes.map(
      (archetype) =>
  ...
  const fanJudges = (state: typeof GraphState.State) =>
  ...
  const fanReviewers = (state: typeof GraphState.State) =>
  ...
  return new StateGraph({
    ...
    .addNode("assess_athlete", assessAthlete)
    .addNode("assess_goal", assessGoal)
    .addNode("strategy_worker", strategyWorker)
    .addNode("dispatch_judges", dispatchJudges)
    .addNode("judge_worker", judgeWorker)
    .addNode("select_strategy", selectStrategy)
    .addNode("expand_skeleton", expandSkeleton)
    .addNode("simulate_load", simulateLoad)
    .addNode("filter_rules", filterRules)
    .addNode("validate_selected", validateSelected)
    .addNode("review_worker", reviewWorker)
    .addNode("adjudicate_reviews", adjudicateReviews)
    .addNode("finalize", finalize)
    .addEdge(START, "initialize")
    .addConditionalEdges("initialize", stopOr("assess_athlete"), [
      "assess_athlete",
      END,
    ])
    .addConditionalEdges("assess_athlete", stopOr("assess_goal"), [
      "assess_goal",
      END,
    ])
    .addConditionalEdges(
      "assess_goal",
      (state) => (state.outcome ? END : fanStrategies(state)),
      ["strategy_worker", END],
    )
    .addEdge("strategy_worker", "dispatch_judges")
    .addConditionalEdges(
      "dispatch_judges",
      (state) => (state.outcome ? END : fanJudges(state)),
      ["judge_worker", END],
    )
    .addEdge("judge_worker", "select_strategy")
    .addConditionalEdges("select_strategy", stopOr("expand_skeleton"), [
      "expand_skeleton",
      END,
    ])
    .addConditionalEdges("expand_skeleton", stopOr("simulate_load"), [
      "simulate_load",
      END,
    ])
    .addConditionalEdges("simulate_load", stopOr("filter_rules"), [
      "filter_rules",
      END,
    ])
    .addConditionalEdges("filter_rules", stopOr("validate_selected"), [
      "validate_selected",
      END,
    ])
    .addConditionalEdges(
      "validate_selected",
      (state) => (state.outcome ? END : fanReviewers(state)),
      ["review_worker", END],
    )
    .addEdge("review_worker", "adjudicate_reviews")
    .addConditionalEdges("adjudicate_reviews", stopOr("finalize"), [
      "finalize",
      END,
    ])
