...
import { createMasterPlanNodes, GraphInput, GraphOutput, GraphState, type MasterPlanGraphDependencies } from "./nodes.js";
...
export function createMasterPlanGraph(dependencies: MasterPlanGraphDependencies) {
  ...
  return new StateGraph({
    ...
    .addConditionalEdges("initialize", nodes.stopOr("assess_athlete"), ["assess_athlete", END])
    .addConditionalEdges("assess_athlete", nodes.stopOr("assess_goal"), ["assess_goal", END])
    .addConditionalEdges("assess_goal", (state) => (state.outcome ? END : nodes.fanStrategies(state)), ["strategy_worker", END])
    .addEdge("strategy_worker", "dispatch_judges")
    .addConditionalEdges("dispatch_judges", (state) => (state.outcome ? END : nodes.fanJudges(state)), ["judge_worker", END])
    .addEdge("judge_worker", "select_strategy")
    .addConditionalEdges("select_strategy", nodes.stopOr("expand_skeleton"), ["expand_skeleton", END])
    .addConditionalEdges("expand_skeleton", nodes.stopOr("simulate_load"), ["simulate_load", END])
    .addConditionalEdges("simulate_load", nodes.stopOr("filter_rules"), ["filter_rules", END])
    .addConditionalEdges("filter_rules", nodes.stopOr("validate_selected"), ["validate_selected", END])
    .addConditionalEdges("validate_selected", (state) => (state.outcome ? END : nodes.fanReviewers(state)), ["review_worker", END])
    .addEdge("review_worker", "adjudicate_reviews")
    .addConditionalEdges("adjudicate_reviews", nodes.stopOr("finalize"), ["finalize", END])
