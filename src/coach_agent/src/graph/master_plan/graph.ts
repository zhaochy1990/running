...
export function createMasterPlanGraph(dependencies: MasterPlanGraphDependencies) {
  ...
  const finalize = (state: typeof GraphState.State) => {
    ...
    return { outcome: MasterPlanGraphOutcome.parse({ decision: "completed", request_id: request.request_id, generation_id: context.generationId, artifact: { type: "master_plan_draft", activation_status: "inactive", plan, facts, athlete_assessment: athleteAssessment, goal_assessment: goalAssessment, strategy_candidates: state.strategyCandidates, judgments: state.judgments, selected_strategy: state.selectedStrategy, simulation_report: state.simulationReport, rule_report: state.ruleReport } }) };
  };
  const simulateLoad = (state: typeof GraphState.State) => { try { return { simulationReport: simulateMasterPlanLoad(state.plan!, state.snapshot!) }; } catch (error) { return { outcome: qualityFailure(state.request, state.context!, `load_simulation_failed:${error instanceof Error ? error.message : "unknown"}`) }; } };
  ...
  return new StateGraph({ state: GraphState, input: GraphInput, output: GraphOutput, context: MasterPlanGraphContext })
    ...
    .addNode("expand_skeleton", expandSkeleton).addNode("simulate_load", simulateLoad).addNode("filter_rules", filterRules).addNode("validate_selected", validateSelected).addNode("finalize", finalize)
    ...
    .addConditionalEdges("validate_selected", stopOr("finalize"), ["finalize", END]).addEdge("finalize", END).compile();
}
