...
function estimate(
	week: Partial<Week> & Pick<Week, "key_sessions">,
	goal: MasterPlan["goal"] = createTestMasterPlan().goal,
) {
		...
		key_sessions: [
			{
				...
				duration_min: 75,
				intensity:
					"3 km warm-up + 6 x 800 m Z5 with jog recoveries + cool-down",
...
test("estimator consumes persisted canonical pace-zone boundaries", () => {
	...
	assert.equal(
		result.assumptions.includes("pace_zone_calibration_missing"),
		false,
	);
});
...
test("an explicitly embedded sibling replaces part of its parent without double-counting distance", () => {
	...
	assert.ok(
		result.assumptions.includes(
			"race_pace_embedded_in_parent_not_double_counted",
		),
	);
	assert.ok(
		result.assumptions.includes("embedded_segments_integrated_by_distance"),
	);
});
...
test("race sessions use the goal pace while tune-up races use their own result", () => {
	...
	assert.deepEqual(
		[race.expectedDose, race.lowDose, race.highDose],
		[303, 303, 303],
	);
	assert.deepEqual(
		[tuneUp.expectedDose, tuneUp.lowDose, tuneUp.highDose],
		[231.4, 168.9, 243.9],
	);
});
...
test("missing threshold calibration makes all dose bounds unavailable", () => {
	...
	assert.ok(week);
	const unavailable = estimateMasterPlanWeekLoad(
		week,
		createTestMasterPlan().goal,
		null,
	);
