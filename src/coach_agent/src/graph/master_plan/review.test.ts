...
import { adjudicateMasterPlanReviews, mergeReviewReportsByRevisionAndTask, mergeReviewWorkerErrors, type ReviewReport } from "@stride/contract";
...
test("adjudicator rejects unknown rule and simulation evidence", () => {
  ...
  assert.throws(
    () =>
      adjudicateMasterPlanReviews(
        1,
        [report("periodization", { evidence_refs: ["rule:not-a-rule"] }), report("load_progression"), report("constraint_grounding")],
        facts,
        evidence,
      ),
  ...
  assert.throws(
    () =>
      adjudicateMasterPlanReviews(
        1,
        [report("periodization", { evidence_refs: ["simulation:missing"] }), report("load_progression"), report("constraint_grounding")],
        facts,
        evidence,
      ),
