...
const ProfileSchema = z
	.object({
		...
		running_age_range: z.string().nullable(),
	})
	.strict();
const WeeklyMetricsSchema = z
	.object({
		week_start: day,
		distance_km: z.number().nonnegative(),
		hours: z.number().nonnegative(),
		avg_pace_s_km: nullableNumber,
		avg_hr: nullableNumber,
		run_count: z.number().int().nonnegative(),
		run_day_count: z.number().int().min(0).max(7).optional(),
		long_run_km: nullableNumber,
		speed_session_count: z.number().int().nonnegative(),
		race_count: z.number().int().nonnegative(),
		training_dose: z.number().nonnegative(),
		ctl: nullableNumber,
		atl: nullableNumber,
		form: nullableNumber,
		rhr: nullableNumber,
	...
	.transform((week) => ({
		...week,
		run_day_count: week.run_day_count ?? Math.min(7, week.run_count),
	}));
	...
	.object({
		...
		injuries: z.array(
			z
				.object({
					body_area: z.string(),
					status: z.string(),
					occurred_on: day.nullable(),
					source: z.string(),
				})
				.strict(),
		),
		personal_bests: z.array(
			z
				.object({
					distance: z.string(),
		...
		running_calibration: z
			.object({
				...
				pace_zones: z.array(z.unknown()),
			})
			.strict()
			.nullable(),
		race_history: z.array(
			z
				.object({
					date: day,
		...
		macro_history: z
			.object({
				...
				longest_road_run_km: nullableNumber.optional(),
				gap_periods: z.array(
					z
						.object({
							start_date: day,
			...
			.transform((history) => ({
				...history,
				longest_road_run_km:
					history.longest_road_run_km === undefined
		...
		recent_history: z
			.object({
				start_date: day,
				end_date: day,
				weeks: z.array(WeeklyMetricsSchema),
			})
			.strict(),
		fitness_state: z
			.object({
		...
		current_phase: z
			.object({
				...
				start_date: day.nullable(),
				end_date: day.nullable(),
				source: z.enum(["active_plan", "inferred"]),
			})
			.strict()
			.nullable(),
		continuity: z
			.object({
				active_plan_continuation: z.boolean(),
				last_activity_date: day.nullable(),
				days_since_last_run: z.number().int().nonnegative().nullable(),
			})
			.strict(),
		coverage: z.array(
			z
...
export type ContextSnapshot = DeepReadonly<
	z.infer<typeof ContextSnapshotSchema>
>;
...
export class FrozenMasterPlanContextProvider
	implements MasterPlanContextProvider
{
	readonly #snapshot: ContextSnapshot;
	constructor(snapshot: z.input<typeof ContextSnapshotSchema>) {
