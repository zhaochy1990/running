...
const WeeklyMetricsSchema = z.object({ week_start: day, distance_km: z.number().nonnegative(), hours: z.number().nonnegative(), avg_pace_s_km: nullableNumber, avg_hr: nullableNumber, run_count: z.number().int().nonnegative(), run_day_count: z.number().int().min(0).max(7).optional(), long_run_km: nullableNumber, speed_session_count: z.number().int().nonnegative(), race_count: z.number().int().nonnegative(), training_dose: z.number().nonnegative(), ctl: nullableNumber, atl: nullableNumber, form: nullableNumber, rhr: nullableNumber, hrv: nullableNumber }).strict().transform((week) => ({ ...week, run_day_count: week.run_day_count ?? Math.min(7, week.run_count) }));
...
export const ContextSnapshotSchema = z.object({
  ...
  race_history: z.array(z.object({ date: day, distance_km: nullableNumber, duration_min: nullableNumber, avg_pace_s_km: nullableNumber, avg_hr: nullableNumber, max_hr: nullableNumber, feel: nullableNumber }).strict()),
  macro_history: z.object({ start_date: day, end_date: day, months: z.array(z.object({ month: z.string().regex(/^\d{4}-\d{2}$/), distance_km: z.number().nonnegative(), hours: z.number().nonnegative(), run_count: z.number().int().nonnegative() }).strict()), peak_weekly_distance_km: nullableNumber, longest_run_km: nullableNumber, longest_road_run_km: nullableNumber.optional(), gap_periods: z.array(z.object({ start_date: day, end_date: day, days: z.number().int().nonnegative() }).strict()), consistency_pct: nullableNumber }).strict().transform((history) => ({ ...history, longest_road_run_km: history.longest_road_run_km === undefined ? history.longest_run_km : history.longest_road_run_km })),
