...
const assessmentDependencies = {
  ...
  judgmentModel: {
    async invoke({
      ...
      return createTestJudgments(candidate.candidate_id).find((item) => item.judge === judge)!;
    },
  },
  reviewModel: {
    async invoke({ reviewerType }: { reviewerType: "periodization" | "load_progression" | "constraint_grounding" }) {
...
test("fans out independent strategies and judges, then passes only selected strategy to skeleton", async () => {
  ...
  const graph = createMasterPlanGraph({
    ...
    strategyModel: {
      async invoke(input) {
        ...
        await new Promise((resolve) => setTimeout(resolve, input.archetype === "conservative" ? 20 : 1));
    ...
    judgmentModel: {
      async invoke(input) {
        ...
        return createTestJudgments(input.candidate.candidate_id).find((item) => item.judge === input.judge)!;
      },
    },
    skeletonModel: {
      async invoke(input) {
  ...
  assert.deepEqual(
    outcome.artifact.strategy_candidates.map((candidate) => candidate.candidate_id),
    ["strategy-aggressive-gated-v1", "strategy-balanced-v1", "strategy-conservative-v1"],
  );
  assert.equal(strategyInputs.length, 3);
  assert.equal(
    strategyInputs.every((input) => !("strategy_candidates" in (input as object))),
    true,
  );
  ...
  const graph = createMasterPlanGraph({
    ...
    judgmentModel: {
      async invoke({ judge, candidate }) {
        return {
          ...createTestJudgments(candidate.candidate_id).find((item) => item.judge === judge)!,
          veto: true,
        };
      },
    },
...
test("deterministic rule errors block completion after simulation", async () => {
  ...
  plan.weeks[0]!.key_sessions[0]!.workout_structure = structuredRun("30km long run", "2026-08-15", 30000, 300);
  const graph = createMasterPlanGraph({
    ...assessmentDependencies,
    contextProvider: {
      async loadSnapshot() {
...
test("dispatches all current artifact reviewers independently after rules pass", async () => {
  ...
  const graph = createMasterPlanGraph({
    ...
    reviewModel: {
      async invoke(input) {
        calls.push(input);
        await new Promise((resolve) => setTimeout(resolve, input.reviewerType === "periodization" ? 20 : 1));
