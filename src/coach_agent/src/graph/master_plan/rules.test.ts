...
const mutated = (change: (plan: ReturnType<typeof MasterPlanSchema.parse>) => void) => {
...
test("load_week_ramp retains the pre-recovery anchor", () => {
  const plan = mutated((p) => {
    ...
    p.phases = [{ ...p.phases[0]!, end_date: "2026-08-30", weekly_distance_km_high: 90 }];
  });
...
test("long_run_share applies default and frequency_limited thresholds", () => {
  const plan = mutated((p) => {
    ...
    const step = p.weeks[0]!.key_sessions[0]!.workout_structure?.blocks[0]?.steps[0];
