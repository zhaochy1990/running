...
export function estimateMasterPlanWeekLoad(
	...
	for (const session of week.key_sessions) {
		...
		if (session.workout_structure) {
			const estimate = estimatePlannedRunLoad(
		...
		if (
			...
			const intensity = sessionIntensityRange(
				session,
				goal,
				goalRaceIf,
				zoneRanges,
		...
		const estimate =
			intensity && hasPositiveExtent(session)
				? estimateSession(session, intensity, thresholdSpeedMps)
				: null;
	...
	if (embedded.length > 0 && computable) {
		const parent = week.key_sessions.find(
			(session) => session.type === "long_run" && hasPositiveExtent(session),
		);
		if (parent) {
			const parentIntensity = sessionIntensityRange(
				parent,
				goal,
				goalRaceIf,
				zoneRanges,
			...
			if (parentIntensity) {
				const replacement = estimateEmbeddedParent(
					parent,
					parentIntensity,
					embedded,
					thresholdSpeedMps,
				...
				if (replacement) {
					expectedKeyDose +=
						replacement.estimate.expectedDose -
						replacement.original.expectedDose;
					lowKeyDose +=
						replacement.estimate.lowDose - replacement.original.lowDose;
					highKeyDose +=
						replacement.estimate.highDose - replacement.original.highDose;
					longRunDose = Math.max(
						longRunDose,
						replacement.estimate.expectedDose,
					);
	...
	const remainingEasyKmLow = Math.max(
		0,
		week.target_weekly_km_low - keySessionKm,
	);
	const remainingEasyKm = Math.max(
		0,
		week.target_weekly_km_high - keySessionKm,
	);
	let lowEasyLower = 0;
	let highEasyExpected = 0;
	let highEasyUpper = 0;
	if (computable && (remainingEasyKmLow > 0 || remainingEasyKm > 0)) {
		const easy = intensityRange(zoneRanges, "easy", [
	...
	return {
		...
		longRunDoseShare:
			expectedDose > 0 ? round(longRunDose / expectedDose, 4) : null,
		assumptions: unique(assumptions),
...
function sessionIntensityRange(
	session: KeySession,
	goal: Goal,
	goalRaceIf: number | null,
	zoneRanges: PaceZoneRanges,
): IntensityRange | null {
	const text =
	...
	switch (session.type) {
		...
		case "vo2max":
		case "time_trial":
			return intensityRange(zoneRanges, "interval", [
				`${session.type}_zone_range`,
			]);
		case "tempo":
			return {
		...
		case "hill":
			return intensityRange(zoneRanges, "threshold", [
				"hill_effort_flat_equivalent_range",
			]);
		case "race":
		case "race_pace": {
			if (goalRaceIf !== null)
			...
			return distanceOnlyRaceRange(
				RACE_DISTANCE_KM[goal.distance] ?? session.distance_km,
				"goal_race",
				zoneRanges,
			);
		}
		...
		case "long_run": {
			if (hasRacePaceMarker(text)) {
				const distanceOnly = distanceOnlyRaceRange(
					RACE_DISTANCE_KM[goal.distance],
					"goal_race",
					zoneRanges,
				);
...
function estimateSession(
	...
	if (session.distance_km && session.distance_km > 0)
		return estimateDistance(session.distance_km, resolved, thresholdSpeedMps);
	if (session.duration_min && session.duration_min > 0)
		return estimateDuration(session.duration_min, resolved, thresholdSpeedMps);
	return null;
}
...
function estimateDistance(
	...
	return {
		expectedDose: round(
			distanceDose(distanceKm, expectedIf, thresholdSpeedMps),
			4,
		),
		lowDose: round(
			distanceDose(distanceKm, intensity.low, thresholdSpeedMps),
			4,
		),
		highDose: round(
			distanceDose(distanceKm, intensity.high, thresholdSpeedMps),
			4,
		),
...
function estimateDuration(
	...
	return {
		...
		estimatedDistanceKm: round(
			(durationMin * 60 * thresholdSpeedMps * expectedIf) / 1000,
			4,
		),
	};
}
...
function estimateEmbeddedParent(
	...
	const exactByDistance =
		Boolean(parent.distance_km) &&
		embedded.every(({ session }) => session.distance_km !== null);
	const exactByDuration =
		Boolean(parent.duration_min) &&
		embedded.every(({ session }) => session.duration_min !== null);
	if (exactByDistance || exactByDuration) {
		const useDistance = exactByDistance;
		const parentAmount = useDistance
			? (parent.distance_km ?? 0)
			: (parent.duration_min ?? 0);
		const embeddedAmount = embedded.reduce(
			(sum, { session }) =>
				sum +
				(useDistance
		...
		if (embeddedAmount > 0 && embeddedAmount <= parentAmount) {
			...
			if (remainder > 0)
				pieces.push(
					useDistance
						? estimateDistance(remainder, parentIntensity, thresholdSpeedMps)
						: estimateDuration(remainder, parentIntensity, thresholdSpeedMps),
				);
			for (const item of embedded)
				pieces.push(
					useDistance
						? estimateDistance(
								item.session.distance_km ?? 0,
								item.intensity,
								thresholdSpeedMps,
							)
						: estimateDuration(
								item.session.duration_min ?? 0,
								item.intensity,
								thresholdSpeedMps,
							),
				);
			return {
				...
				original,
				assumption: `embedded_segments_integrated_by_${
					useDistance ? "distance" : "duration"
				}`,
			};
		}
	}
	...
	const ranged = estimateSession(
		parent,
		{
			low: Math.min(
				parentIntensity.low,
				...embedded.map(({ intensity }) => intensity.low),
			),
			high: Math.max(
				parentIntensity.high,
				...embedded.map(({ intensity }) => intensity.high),
			),
			assumptions: [],
		},
		thresholdSpeedMps,
	...
	return {
		estimate: {
			expectedDose:
				original.expectedDose +
				Math.max(0, ranged.highDose - original.expectedDose) / 2,
...
function sumEstimates(estimates: SessionEstimate[]): SessionEstimate {
	return estimates.reduce(
		(sum, estimate) => ({
			...
			estimatedDistanceKm:
				sum.estimatedDistanceKm + estimate.estimatedDistanceKm,
		}),
		{ expectedDose: 0, lowDose: 0, highDose: 0, estimatedDistanceKm: 0 },
	);
}

function distanceDose(
	distanceKm: number,
	intensity: number,
	thresholdSpeedMps: number,
): number {
...
function resolvePaceZoneRanges(
	paceZones: readonly unknown[],
	thresholdSpeedMps: number,
): PaceZoneRanges | null {
	...
	for (const name of supported) {
		...
		const minPace = positiveNumber(
			zone.minPaceSPerKm ?? zone.min_pace_s_per_km,
		);
	...
	return required.every((name) => resolved[name])
		? (resolved as PaceZoneRanges)
		: null;
}
...
function distanceOnlyRaceRange(
	distanceKm: number | null | undefined,
	prefix: "goal_race" | "tune_up",
	zoneRanges: PaceZoneRanges,
): IntensityRange | null {
	if (!distanceKm || distanceKm <= 0) return null;
	if (distanceKm <= 7.5)
		return intensityRange(zoneRanges, "interval", [
			`${prefix}_distance_only_short_race_interval_range`,
		]);
	if (distanceKm <= 15)
		return intensityRange(zoneRanges, "threshold", [
			`${prefix}_distance_only_10k_threshold_range`,
		]);
	if (distanceKm <= 30)
		return {
			low: zoneRanges.marathon[0],
	...
	return intensityRange(zoneRanges, "marathon", [
		`${prefix}_distance_only_long_race_marathon_range`,
	]);
}

function raceIntensityFactor(
	goal: Goal,
	thresholdSpeedMps: number,
): number | null {
	...
	return clamp(
		(RACE_DISTANCE_KM[goal.distance] * 1000) / seconds / thresholdSpeedMps,
		0,
		2,
	);
}
