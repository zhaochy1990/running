...
test("invokeStructured retries typed parser failures", async () => {
  let calls = 0;
  const result = await invokeStructured(MODEL, { parse: (value) => value as { accepted: boolean } }, "submit_test", [["user", "test"]], (value) => value, {
    buildStructuredModel: () => ({
      async invoke() {
        calls += 1;
        if (calls === 1) {
...
test("invokeStructured does not retry provider failures based on message text", async () => {
  ...
  await assert.rejects(
    invokeStructured(MODEL, { parse: (value) => value }, "submit_test", [["user", "test"]], (value) => value, {
      buildStructuredModel: () => ({
        async invoke() {
          calls += 1;
