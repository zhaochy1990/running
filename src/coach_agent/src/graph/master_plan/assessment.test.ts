...
import { createAssessmentSnapshot, createTestJudgments, createTestMasterPlan, createTestRequest, createTestReviewReport, createTestStrategyCandidate } from "./testFixtures.js";
...
function validGoalAssessment() {
  return {
    ...
    summary: "The confirmed 2:50 goal is plausible only through explicit gates.",
    material_conclusions: [{
    ...
    abc_gates: {
      A: { target: { kind: "time" as const, time_seconds: 10200, label: "2:50:00" }, conditions: [{ description: "Pass race-specific checks", fact_ids: ["goal.a.improvement_pct"] }] },
      B: { target: { kind: "time" as const, time_seconds: 10380, label: "2:53:00" }, conditions: [{ description: "Use if A checks are incomplete", fact_ids: ["goal.a.matching_pb_seconds"] }] },
      C: { target: { kind: "finish" as const, time_seconds: null, label: "Safe completion" }, conditions: [{ description: "Protect health and completion", fact_ids: ["load.current_form"] }] },
    },
    conflicts: [],
    multi_cycle_path: [],
  };
...
test("derives deterministic assessment facts from worked literals and passes assessments to skeleton", async () => {
  ...
  const graph = createMasterPlanGraph(dependencies({
    skeletonModel: { async invoke(input: unknown) { skeletonInputs.push(input); return createTestMasterPlan(); } },
  }));
  ...
  const values = Object.fromEntries(outcome.artifact.facts.facts.map((fact) => [fact.fact_id, fact.value]));
  ...
  const input = skeletonInputs[0] as { facts: unknown; athleteAssessment: unknown; goalAssessment: unknown };
...
test("rejects an assessment citation whose fact id does not exist", async () => {
  ...
  const graph = createMasterPlanGraph(dependencies({ assessmentModel: { async invoke() { return assessment; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
...
test("rejects a goal gate whose fact id does not exist", async () => {
  ...
  const graph = createMasterPlanGraph(dependencies({ goalAssessmentModel: { async invoke() { return assessment; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("rejects B targets that are faster than the confirmed A target", async () => {
  const assessment = validGoalAssessment();
  assessment.abc_gates.B.target = { kind: "time", time_seconds: 10000, label: "2:46:40" };
  const graph = createMasterPlanGraph(dependencies({ goalAssessmentModel: { async invoke() { return assessment; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("rejects supportive load claims when deterministic form is negative", async () => {
  const assessment = AthleteAssessmentSchema.parse(validAthleteAssessment());
  assessment.material_conclusions = [{ claim: "load_state_supportive", explanation: "Load state supports progression", fact_ids: ["load.current_form"] }];
  const graph = createMasterPlanGraph(dependencies({ assessmentModel: { async invoke() { return assessment; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("missing athlete baseline stops before goal assessment", async () => {
  ...
  const graph = createMasterPlanGraph(dependencies({
    ...
    goalAssessmentModel: { async invoke() { goalCalls += 1; return validGoalAssessment(); } },
  }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
...
test("assessment facts exclude the current incomplete Shanghai week", async () => {
  const snapshot = createAssessmentSnapshot();
  snapshot.recent_history.weeks.push({ ...snapshot.recent_history.weeks.at(-1)!, week_start: "2026-08-10", distance_km: 1, run_count: 1, run_day_count: 1, long_run_km: 1 });
  const graph = createMasterPlanGraph(dependencies({ contextProvider: { async loadSnapshot() { return snapshot; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "completed");
  if (outcome.decision !== "completed") assert.fail("expected completed");
  const recent = outcome.artifact.facts.facts.find((fact) => fact.fact_id === "volume.recent_weekly_km");
...
test("zero recent running baseline stops before assessment models", async () => {
  ...
  const graph = createMasterPlanGraph(dependencies({
    contextProvider: { async loadSnapshot() { return snapshot; } },
    assessmentModel: { async invoke() { modelCalls += 1; return validAthleteAssessment(); } },
  }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "needs_baseline");
...
test("zero quality-session history cannot authorize quality sessions", async () => {
  ...
  const assessment = AthleteAssessmentSchema.parse(validAthleteAssessment());
  assessment.safe_training_ranges.quality_sessions_per_week = { low: 0, high: 1 };
  const graph = createMasterPlanGraph(dependencies({
    contextProvider: { async loadSnapshot() { return snapshot; } },
    assessmentModel: { async invoke() { return assessment; } },
  }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("zero quality-session history cannot support an established-tolerance claim", async () => {
  ...
  for (const week of snapshot.recent_history.weeks.slice(-4)) week.speed_session_count = 0;
  const assessment = AthleteAssessmentSchema.parse(validAthleteAssessment());
  assessment.safe_training_ranges.quality_sessions_per_week = { low: 0, high: 0 };
  assessment.material_conclusions = [{ claim: "quality_tolerance_established", explanation: "Quality tolerance is established", fact_ids: ["tolerance.quality_sessions_per_week"] }];
  const graph = createMasterPlanGraph(dependencies({ contextProvider: { async loadSnapshot() { return snapshot; } }, assessmentModel: { async invoke() { return assessment; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});

test("missing road-run evidence cannot authorize a positive long-run range", async () => {
  const snapshot = ContextSnapshotSchema.parse({ ...createAssessmentSnapshot(), macro_history: { ...createAssessmentSnapshot().macro_history, longest_road_run_km: null } });
  const graph = createMasterPlanGraph(dependencies({ contextProvider: { async loadSnapshot() { return snapshot; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("typed stop outcomes do not invoke the skeleton", async () => {
  let skeletonCalls = 0;
  const goal = { ...validGoalAssessment(), level: "multi_cycle_required" as const, multi_cycle_path: ["Build baseline", "Reassess target"] };
  const request = createTestRequest();
  request.goals[0]!.target_time = "2:10:00";
  goal.abc_gates.A.target = { kind: "time", time_seconds: 7800, label: "2:10:00" };
  const graph = createMasterPlanGraph(dependencies({
    goalAssessmentModel: { async invoke() { return goal; } },
...
test("rejects a model-declared multi-cycle stop that contradicts deterministic facts", async () => {
  const goal = { ...validGoalAssessment(), level: "multi_cycle_required" as const, multi_cycle_path: ["Build baseline", "Reassess target"] };
  const graph = createMasterPlanGraph(dependencies({ goalAssessmentModel: { async invoke() { return goal; } } }));
  const { outcome } = await graph.invoke({ request: createTestRequest() }, { context });
  assert.equal(outcome.decision, "failed_quality_gate");
});
...
test("explicit current acute restrictions stop before either assessment model", async () => {
  let modelCalls = 0;
  const request = createTestRequest();
  const injuredRequest = { ...request, injury_declarations: [{ kind: "current" as const, body_area: "Achilles", status: "acute pain", training_impact: "stop running" }] };
  const graph = createMasterPlanGraph(dependencies({
    assessmentModel: { async invoke() { modelCalls += 1; return validAthleteAssessment(); } },
  }));
  const { outcome } = await graph.invoke({ request: injuredRequest }, { context });
...
test("a negated symptom does not hide a later explicit stop-running restriction", async () => {
  const request = MasterPlanGraphRequest.parse({ ...createTestRequest(), injury_declarations: [{ kind: "current", body_area: "Achilles", status: "no acute pain, but doctor says stop running", training_impact: "stop running" }] });
  const graph = createMasterPlanGraph(dependencies());
  const { outcome } = await graph.invoke({ request }, { context });
  assert.equal(outcome.decision, "blocked_for_safety");
});
...
test("a conjunction does not let a negated injury hide a doctor restriction", async () => {
  const request = MasterPlanGraphRequest.parse({ ...createTestRequest(), injury_declarations: [{ kind: "current", body_area: "Achilles", status: "no acute injury and doctor restriction: stop running", training_impact: "stop running" }] });
  const graph = createMasterPlanGraph(dependencies());
  const { outcome } = await graph.invoke({ request }, { context });
  assert.equal(outcome.decision, "blocked_for_safety");
});
...
test("an elapsed target race produces a goal conflict instead of a model-defined plan", async () => {
  ...
  const goal = { ...validGoalAssessment(), level: "unsafe_or_incompatible" as const, conflicts: [{ description: "Race date has elapsed", fact_ids: ["race.a.weeks_to_race"] }] };
  const graph = createMasterPlanGraph(dependencies({ goalAssessmentModel: { async invoke() { return goal; } } }));
  const { outcome } = await graph.invoke({ request }, { context });
...
test("confirmed constraints incompatible with marathon preparation produce a goal conflict", async () => {
  const request = MasterPlanGraphRequest.parse({ ...createTestRequest(), prohibited_arrangements: ["no long run"] });
  const goal = { ...validGoalAssessment(), level: "unsafe_or_incompatible" as const, conflicts: [{ description: "Long runs are prohibited", fact_ids: ["constraints.goal_incompatible"] }] };
  const graph = createMasterPlanGraph(dependencies({ goalAssessmentModel: { async invoke() { return goal; } } }));
  const { outcome } = await graph.invoke({ request }, { context });
  assert.equal(outcome.decision, "goal_conflict");
});
