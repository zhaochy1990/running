...
test("weekly simulator deterministically estimates a dose range and applies the default distribution", () => {
  ...
  assert.equal(first.provenance, "weekly_high_target+key_sessions+remaining_easy_volume");
  ...
  assert.deepEqual(first.load_assumptions, ["structured_workout_segments_integrated", "remaining_weekly_distance_in_easy_zone"]);
...
test("simulator does not report high confidence from low-confidence calibration", () => {
  ...
  assert.equal(report.weeks[0]!.estimated_dose_low, report.weeks[0]!.estimated_dose_high);
...
test("simulator decays PMC state across the gap before plan start", () => {
  ...
  assert.ok(report.weeks[0]!.end_ctl !== null && report.weeks[0]!.end_ctl < 66.5048);
});
...
test("simulator distinguishes an uncomputable session from missing calibration", () => {
  ...
  assert.equal(report.weeks[0]!.estimated_dose, null);
  assert.equal(report.weeks[0]!.missing_dose_reason, "planned_session_uncomputable");
});
...
test("simulator limits routine load to explicit windows and weekly run-day maximum", () => {
  ...
  request.availability.available_training_windows = request.availability.available_training_windows.filter((window) =>
...
test("recovery weeks reduce expected dose while retaining an auditable daily Form path", () => {
  ...
  const firstWeekDays = simulatePmcDays(
    loadWeek.daily_distribution.map((share) => share * loadWeek.estimated_dose!),
    { atl: 70, ctl: 65 },
  );
  const firstWeekEnd = firstWeekDays.at(-1)!;
  const recoveryDays = simulatePmcDays(
    recoveryWeek.daily_distribution.map((share) => share * recoveryWeek.estimated_dose!),
