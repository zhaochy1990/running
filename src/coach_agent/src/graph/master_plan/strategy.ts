...
export function mergeWorkerErrors(current: readonly string[], update: readonly string[]): string[] {
...
export function validateStrategyJudgment(judgment: StrategyJudgment, candidate: StrategyCandidate, facts: AssessmentFacts): void {
  ...
  for (const id of judgment.evidence_fact_ids) if (!ids.has(id)) throw new Error(`judgment cites unknown fact_id: ${id}`);
}
...
export function validateCandidateDiversity(candidates: readonly StrategyCandidate[]): void {
  const fingerprints = candidates.map((candidate) =>
    JSON.stringify({
      ...
      phase_ranges: candidate.phases.map((phase) => [phase.weekly_km_low, phase.weekly_km_high]),
  ...
  if (new Set(fingerprints).size !== candidates.length) throw new Error("strategy candidates must be materially different");
}
      ...
      const scores = {
        ...
        weighted_total: Number(
          (
            ...
            score("constraint_feasibility") * STRATEGY_WEIGHTS.constraint_feasibility
          ).toFixed(2),
        ),
      };
