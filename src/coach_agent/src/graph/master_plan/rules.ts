...
export function runMasterPlanRuleFilter(
	rawPlan: unknown,
	rawRequest: MasterPlanGraphRequest,
	...
	for (const week of plan.weeks) {
		...
		if (
			...
			addDays(week.week_start, 6) > phase.end_date
		)
			add(
				"week_phase_alignment",
				"error",
	...
	for (const week of plan.weeks) {
		...
		if (!exception) {
			if (
				anchor &&
				week.target_weekly_km_high > anchor.target_weekly_km_high * 1.1 + 1e-9
			)
				add(
					"load_week_ramp",
					"error",
	...
	const preTaper = taper
		? plan.weeks.filter(
				(w) => w.week_start < taper.week_start && !w.is_recovery_week,
			)
		: [];
	...
	for (const week of plan.weeks) {
		const hard = week.key_sessions.filter(
			(s) =>
				HARD.has(s.type) ||
				(s.type === "long_run" &&
					positiveRacePace(`${s.intensity ?? ""} ${s.purpose ?? ""}`)),
		).length;
		if (hard > 2)
			add(
				"hard_stimulus_density",
		...
		const runningSessions = week.key_sessions.filter(
			(session) => session.type !== "strength_key",
		).length;
		...
		for (const [sessionIndex, session] of week.key_sessions.entries()) {
			...
			if (
				duration !== null &&
				duration > request.availability.max_session_duration_min
			)
				add(
					"availability_constraints",
					"error",
					"Key session exceeds maximum session duration",
					{
						...
						max_session_duration_min:
							request.availability.max_session_duration_min,
						provenance:
							session.duration_min === null
								? "threshold_speed_conservative_slow_edge"
								: "explicit_duration",
					},
...
function positiveRacePace(text: string): boolean {
	...
	return (
		marker.test(text) &&
		!/(?:不|无|非|不含|没有|no|without|free)[^。；,;]{0,20}(?:MP|HMP|RP|race[- ]?pace|target[- ]?pace|目标配速|比赛配速|马拉松配速|半马配速)/i.test(
			text,
		)
	);
}
function conservativePace(
	type: MasterPlan["weeks"][number]["key_sessions"][number]["type"],
	snapshot: ContextSnapshot,
): number | null {
	...
	return speed && speed > 0
		? 1000 /
				(speed * (type === "long_run" ? 0.7 : HARD.has(type) ? 0.85 : 0.75))
		: null;
}
