...
export function buildResponsesModel(model: ModelConfig): ChatOpenAIResponses {
  if (model.provider !== "openai-compatible" || model.api_kind !== "responses") {
    throw new Error(
      `ChatOpenAIResponses requires an openai-compatible Responses model; ` + `"${model.name}" is provider=${model.provider} api_kind=${model.api_kind}`,
    );
  }
  ...
  if (!apiKey?.trim()) {
    throw new Error(`Environment variable "${model.api_key_env}" is required for model "${model.name}"`);
  }
