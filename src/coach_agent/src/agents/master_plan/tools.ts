...
const getMasterPlan = tool(
  async (_input: z.infer<typeof getMasterPlanSchema>, runtime: ToolRuntime<unknown, typeof CoachContext>) => {
  ...
  },
  {
    ...
    description: "获取运动员的赛季/总体训练计划：目标、阶段、当前位置、下一里程碑",
    schema: getMasterPlanSchema,
  },
);

const proposeMasterAdjustmentSchema = z.object({
  request: z.string().describe("运动员要求的赛季计划调整，如“把基础期延长两周”"),
});

const proposeMasterAdjustment = tool(
  async ({ request }: z.infer<typeof proposeMasterAdjustmentSchema>, runtime: ToolRuntime<unknown, typeof CoachContext>) => {
    const userId = runtime.context?.userId;
    console.log(`[mock] propose_master_adjustment user=${userId} request=${request}`);
    return {
