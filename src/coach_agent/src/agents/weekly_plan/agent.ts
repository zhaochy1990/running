...
function createWeeklyPlanSubagent(store: DataProvider, config: ModelConfig, generatesPlan: boolean) {
  ...
  return {
    ...
    middleware: [createTurnScopeMiddleware(), createLoggingMiddleware(generatesPlan ? "agent:generate_weekly_plan" : "agent:weekly_plan")],
...
export function getWeeklyPlanGeneratorSubagent(store: DataProvider, config: ModelConfig) {
