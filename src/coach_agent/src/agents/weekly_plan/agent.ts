...
import { MySqlWeeklyPlanContextProvider, type StrideDataStore } from "../../persistence/index.js";
...
function createWeeklyPlanSubagent(store: StrideDataStore, config: ModelConfig, generatesPlan: boolean) {
  ...
  const weeklyPlanContextTools = createWeeklyPlanContextTools(weeklyPlanContextProvider);
  return {
    name: generatesPlan ? "generate_weekly_plan" : "weekly_plan",
    description: generatesPlan ? "generate a new structured weekly training plan" : "read, explain, or discuss an existing weekly training plan",
    systemPrompt: WeeklyPlanPrompt,
    tools: [...weeklyPlanContextTools, ...planTools, ...activitiesTools, ...trainingLoadTools, ...raceTools, ...runningCalibrationTools],
    model: buildResponsesModel(config),
    ...(generatesPlan ? { responseFormat: WeeklyPlanDirectResponseSchema } : {}),
    middleware: [createLoggingMiddleware(generatesPlan ? "agent:generate_weekly_plan" : "agent:weekly_plan")],
...
export function getWeeklyPlanGeneratorSubagent(store: StrideDataStore, config: ModelConfig) {
