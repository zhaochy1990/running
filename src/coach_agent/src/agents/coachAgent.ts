...
export async function createCoachAgent(store: StrideDataStore, config: CoachAgentConfig) {
  ...
  const weeklyPlanGenerator = getWeeklyPlanGeneratorSubagent(store, getAgentConfig(config, "weekly_plan"));
  ...
  return createDeepAgent({
    ...
    middleware: [createPlanPassthroughMiddleware(), createLoggingMiddleware("agent")],
