...
export async function createCoachAgent(dataProvider: DataProvider, config: CoachAgentConfig, options: CoachAgentOptions = {}) {
  ...
  const weeklyPlanGenerator = getWeeklyPlanGeneratorSubagent(dataProvider, getAgentConfig(config, "weekly_plan"));

  const masterSubagent = getMasterPlanSubagent(dataProvider, getAgentConfig(config, "master_plan"));
  const masterPlanGenerator = getMasterPlanGeneratorSubagent(dataProvider, getAgentConfig(config, "master_plan"));
