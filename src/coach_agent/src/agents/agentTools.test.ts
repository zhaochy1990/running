...
const masterPlan = {
  ...
  goal: { race_name: "测试马拉松", distance: "FM" as const, race_date: "2026-10-18", target_time: "2:50:00", timezone: "Asia/Shanghai" as const, location: "西安" },
  start_date: "2026-08-10",
  end_date: "2026-10-18",
  total_weeks: 1,
  phases: [{ name: "基础期" as const, start_date: "2026-08-10", end_date: "2026-10-18", focus: "基础", weekly_distance_km_low: 70, weekly_distance_km_high: 80, key_session_types: ["长跑"], milestones: [], key_workouts: "长跑", monitoring_triggers: ["疼痛"], coach_note: "注意恢复", strength: { sessions_per_week: 2, focus: "核心", timing: "轻松跑后" }, recovery: { focus: "睡眠和补给", sleep_target_hours: "7-9", adjustment_trigger: "疼痛降量" }, is_completed: false, summary: null }],
  weeks: [{ week_index: 1, week_start: "2026-08-10", phase_name: "基础期" as const, target_weekly_km_low: 70, target_weekly_km_high: 80, key_sessions: [], is_recovery_week: false }],
...
test("orchestrator forwards a validated master-plan task result byte-for-byte", () => {
  ...
  const result = getMasterPlanTaskResult([
    { type: "ai", tool_calls: [{ id: "task-1", name: "task", args: { subagent_type: "generate_master_plan" } }] },
...
test("orchestrator does not bypass the model for unrelated or invalid task results", () => {
  assert.equal(getMasterPlanTaskResult([
    { type: "ai", tool_calls: [{ id: "task-1", name: "task", args: { subagent_type: "qa" } }] },
    { type: "tool", name: "task", tool_call_id: "task-1", content: JSON.stringify(masterPlan) },
  ]), undefined);
  assert.equal(getMasterPlanTaskResult([
    { type: "ai", tool_calls: [{ id: "task-1", name: "task", args: { subagent_type: "generate_master_plan" } }] },
    { type: "tool", name: "task", tool_call_id: "task-1", content: "not JSON" },
  ]), undefined);
});
...
test("orchestrator only forwards an immediately preceding generator task result", () => {
  ...
  assert.equal(getMasterPlanTaskResult([
    { type: "ai", tool_calls: [{ id: "old-task", name: "task", args: { subagent_type: "generate_master_plan" } }] },
...
test("orchestrator does not replay a generator result after a later tool call", () => {
  const content = JSON.stringify(masterPlan);
  assert.equal(getMasterPlanTaskResult([
    { type: "ai", tool_calls: [{ id: "generator-task", name: "task", args: { subagent_type: "generate_master_plan" } }] },
    { type: "tool", name: "task", tool_call_id: "generator-task", content },
    { type: "ai", tool_calls: [{ id: "memory", name: "remember_athlete_fact", args: {} }] },
    { type: "tool", name: "remember_athlete_fact", tool_call_id: "memory", content: "已记住" },
  ]), undefined);
});
