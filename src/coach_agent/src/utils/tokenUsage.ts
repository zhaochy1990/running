...
export class LlmTokenUsageTracker extends BaseCallbackHandler {
  ...
  private startCall(runId: string, extraParams?: Record<string, unknown>, metadata?: Record<string, unknown>): void {
    ...
    const call: MutableCall = {
      ...
      model: readString(metadata, "ls_model_name") ?? readNestedString(extraParams, "invocation_params", "model") ?? "unknown",
      inputTokens: null,
      outputTokens: null,
      totalTokens: null,
      status: "running",
...
export function formatTokenUsageReport(summary: LlmTokenUsageSummary): string {
  const lines = ["===== LLM token usage =====", `LLM calls: ${summary.callCount}`];
  ...
  const completeness =
    summary.reportedCallCount === summary.callCount ? "complete" : `partial (${summary.reportedCallCount}/${summary.callCount} calls reported usage)`;
  lines.push(`TOTAL input=${summary.inputTokens} output=${summary.outputTokens} ` + `total=${summary.totalTokens} usage=${completeness}`);
...
function extractTokenCounts(output: LLMResult): TokenCounts | undefined {
  for (const generations of output.generations) {
    for (const generation of generations) {
      ...
      const counts = readCounts(usage, "input_tokens", "output_tokens", "total_tokens");
      if (counts) return counts;
    }
  }

  const estimated = readRecord(output.llmOutput, "estimatedTokenUsage");
  return readCounts(estimated, "promptTokens", "completionTokens", "totalTokens");
}

function readCounts(value: Record<string, unknown> | undefined, inputKey: string, outputKey: string, totalKey: string): TokenCounts | undefined {
...
function readNestedString(value: Record<string, unknown> | undefined, recordKey: string, stringKey: string): string | undefined {
  return readString(readRecord(value, recordKey), stringKey);
}

function sum(calls: LlmTokenUsageCall[], key: "inputTokens" | "outputTokens" | "totalTokens"): number {
