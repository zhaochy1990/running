...
test("tracks every LLM call and aggregates provider-reported usage", () => {
  ...
  tracker.handleChatModelStart({}, [], "run-1", undefined, { invocation_params: { model: "gpt-orchestrator" } }, undefined, {});
  ...
  tracker.handleChatModelStart({}, [], "run-2", undefined, undefined, undefined, { lc_agent_name: "generate_master_plan", ls_model_name: "gpt-plan" });
  tracker.handleLLMEnd(
    {
      generations: [[{ text: "" }]],
      llmOutput: {
...
test("reports missing usage without silently treating it as zero", () => {
  ...
  tracker.handleLLMEnd({ generations: [[{ text: "no usage" }]] }, "run-missing");
...
test("retains failed LLM calls in the final report", () => {
  const tracker = new LlmTokenUsageTracker();
  tracker.handleChatModelStart({}, [], "run-failed", undefined, { invocation_params: { model: "gpt-plan" } }, undefined, {
  ...
  assert.match(formatTokenUsageReport(summary), /#1 agent=generate_master_plan model=gpt-plan .*status=failed/);
});
