...
export interface WeeklyFeedbackSource {
  getByDateRange(userId: string, startDay: string, endDay: string): Promise<WeeklyFeedbackRecord[]>;
}
...
export function createWeeklyFeedbackSource(store: MySqlFeedbackStore, options: WeeklyFeedbackSourceOptions = {}): WeeklyFeedbackSource {
  const cutoverComplete = options.cutoverComplete ?? weeklyFeedbackCutoverComplete();
  return cutoverComplete ? new MySqlWeeklyFeedbackSource(store) : new LegacyFileWeeklyFeedbackSource(options.legacyDataDir);
}

export class MySqlWeeklyFeedbackSource implements WeeklyFeedbackSource {
...
export class LegacyFileWeeklyFeedbackSource implements WeeklyFeedbackSource {
  ...
  async getByDateRange(userId: string, startDay: string, endDay: string): Promise<WeeklyFeedbackRecord[]> {
    assertUserId(userId);
    ...
    const rows = await Promise.all(
      entries
        ...
        .map(async (entry): Promise<WeeklyFeedbackRecord | null> => {
          ...
          if (!weekStart || weekStart < startDay || addDays(weekStart, 6) > endDay) {
            return null;
          }
    ...
    return rows.filter((row): row is WeeklyFeedbackRecord => row !== null).sort((left, right) => left.weekStart.localeCompare(right.weekStart));
  }
}

export function weeklyFeedbackCutoverComplete(value = process.env.STRIDE_WEEKLY_FEEDBACK_CUTOVER_COMPLETE): boolean {
  return value?.trim().toLowerCase() === "true";
}

function defaultDataDir(): string {
  return process.env.STRIDE_COACH_DATA_DIR ?? resolve(dirname(fileURLToPath(import.meta.url)), "../../../..", "data");
}
