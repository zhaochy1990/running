...
export class MySqlStore extends BaseStore {
  ...
  private async _put(op: { namespace: string[]; key: string; value: Record<string, unknown> | null }): Promise<void> {
    const ns = op.namespace.join(SEP);
    if (op.value === null) {
      await this.pool.execute(`DELETE FROM store WHERE ns=? AND key_name=?`, [ns, op.key]);
      return;
    }
  ...
  private async _get(op: { namespace: string[]; key: string }): Promise<Item | null> {
    const ns = op.namespace.join(SEP);
    const [rows] = await this.pool.query<RowDataPacket[]>(`SELECT * FROM store WHERE ns=? AND key_name=?`, [ns, op.key]);
    return rows[0] ? this.rowToItem(rows[0]) : null;
  }

  private async _search(op: { namespacePrefix: string[]; filter?: Record<string, unknown>; limit?: number; offset?: number }): Promise<SearchItem[]> {
    ...
    if (op.filter) {
      ...
      items = items.filter((it) => entries.every(([k, v]) => it.value[k] === v));
    }
    return items;
  }

  private async _listNamespaces(): Promise<string[][]> {
    const [rows] = await this.pool.query<RowDataPacket[]>(`SELECT DISTINCT ns FROM store`);
