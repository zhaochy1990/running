...
const result = await graph.invoke({ request }, { context: { userId: "local-testing-athlete", generationId } });
...
await Promise.all([writeJson("request.json", request), writeJson("outcome.json", outcome), writeJson("final-draft.json", outcome.artifact)]);
