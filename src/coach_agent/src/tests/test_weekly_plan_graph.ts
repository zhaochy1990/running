...
function requireUserId(): { userId: string; username: string } {
	...
	if (!username) {
		console.error(
			"Missing username. Usage: npm run test:weekly-plan-graph -- <user>",
		);
		process.exit(1);
	}
	...
	if (!userId) {
		console.error(
			`Unknown username: ${username}. Valid usernames: ${Object.keys(usernameMap).join(", ")}`,
		);
		process.exit(1);
	}
...
async function main() {
	...
	try {
		...
		if (
			result.outcome.decision !== "completed" ||
			!result.outcome.weekly_plan
		) {
			throw new Error(
				`Weekly plan generation failed: ${JSON.stringify(result.outcome)}`,
			);
		}
