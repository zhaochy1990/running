...
def _paths():
    ...
    These stay defined in ``stride_core.db`` — the canonical location that
    tests + callers already monkeypatch (``setattr(stride_core.db,
    "USER_DATA_DIR", tmp)``). Imported lazily, at call time, to avoid an
    import-time cycle with the ``stride_core.db`` shim that re-exports this
    module, and so that a monkeypatch on ``stride_core.db.USER_DATA_DIR`` is
    observed here.
    """
...
class Database:
    def __init__(self, db_path: Path | str | None = None, user: str | None = None):
        if db_path:
            ...
            self._path = _paths().USER_DATA_DIR / user / "coros.db"
        else:
    ...
    def get_running_week_summaries(
        ...
        for week_index, date_from, date_to in week_windows:
            km_expr = "CASE WHEN distance_m < 500 THEN distance_m ELSE distance_m / 1000.0 END"
            row = self._conn.execute(
                f"""SELECT count(*) AS run_count,
                          round(coalesce(sum({km_expr}), 0), 1) AS actual_distance_km,
    ...
    def upsert_planned_sessions(
        ...
        date_range = _paths()._parse_week_folder_dates(week_folder)
