...
def _intensity_seconds_in_zones_4_5(plan: WeeklyPlan) -> float:
    """Sum work-step seconds whose target HR or pace lands in Z4-Z5.

    Heuristic: any work step with target.kind in ('pace_s_km','hr_bpm') and
    HR ≥ 165 or pace ≤ 270 s/km (≈ 4:30/km) counts as Z4-Z5. The numbers
    are conservative defaults; tuning lives in plan §7.3 follow-up.
    """
    total = 0.0
        ...
        for block in spec.blocks:
            ...
            for step in getattr(block, "steps", []) or []:
                ...
                if tgt_kind == "hr_bpm" and hr_high and hr_high >= 165:
                    hot = True
                elif tgt_kind == "pace_s_km" and pace_low and pace_low <= 270:
                    hot = True
                if not hot:
                    continue
...
def check_intensity_distribution(plan: WeeklyPlan) -> list[RuleViolation]:
    """Z4-Z5 time ≤ 20% of weekly running time (80/20 polarization rule)."""
    total_s = _total_planned_seconds(plan)
    if total_s <= 0:
        return []
    hot_s = _intensity_seconds_in_zones_4_5(plan)
    share = hot_s / total_s
    if share > 0.20:
        return [
            RuleViolation(
...
def run_rule_filter(
    ...
    *,
    ...
    violations.extend(check_intensity_distribution(plan))
