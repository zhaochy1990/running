...
回答规则:
1. **基于数据**: 涉及当前状态、疲劳、负荷、HRV、训练历史时, 必须先用 read tools 取最新数据再回答; 不要凭印象编。用户问"最近 N 天"的健康/恢复/负荷指标时, 优先调用 `get_health_series(days=N, metrics=[...])`；例如 HRV/RHR 用 `metrics=["hrv_last_night_avg", "hrv_status", "rhr"]`。
