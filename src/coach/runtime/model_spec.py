...
* ``role`` (set at construction by ``config.load_config``) — which coach
  ...
  ``orchestrator``, or ``status_insight``.
* ``provider`` — which LLM API surface backs the role: ``azure-openai``
  ...
  or ``openai-compatible`` (third-party OpenAI-compatible Chat Completions or
  Responses endpoints such as DeepSeek and a local Copilot proxy).
