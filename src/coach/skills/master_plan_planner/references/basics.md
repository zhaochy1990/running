...
- If requested peak volume is far beyond history (e.g. peak 55km, ask 100km/week), push back in `training_principles[0]`: name history peak, cite ~10% progression + overuse/stress-fracture/overtraining risk, set this cycle's load budget at `60-70km` (absolute risk boundary `70-72km`, not `78-80km`), and give path `本周期60-70km，下周期80km，再后周期90+km，100km需更长期适应`. Realistic FM within that load budget still needs one protected `28km / 72km` rehearsal + recovery; do not stop at `25km`.
