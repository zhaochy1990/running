...
- Long_run + emphasis by distance, progressing only +1-3km at a time from recent longest run; use the load-estimator long-run share/dose signal to decide whether to spread load, hold, or reduce:
  - **FM (full marathon)**: long_run ~28-35km; emphasize MP long runs + tempo; peak phase 3-4 weeks.
  - **HM (half marathon)**: long_run commonly ~18-24km, adjusted by history/load concentration; threshold + HMP; peak 2-3 weeks.
  ...
  - **5K**: long_run commonly ~8-12km, longer only when history/load distribution supports it; vo2max/short intervals/speed dominate; peak 1-2 weeks. Do not create a 4-week 5K peak; put spare runway in speed/build.
