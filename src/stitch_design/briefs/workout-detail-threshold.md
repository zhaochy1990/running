...
- Coach bullet uses `热身段不追配速` rather than a fixed 15-minute restriction, so the first threshold repeat begins immediately after the prescribed warm-up without contradiction.
...
- Set `lang="zh-CN"`. Use Inter for Chinese labels such as `约`, `配速`, `心率`, `前`, and `分钟`; apply Geist Mono only to numeric/data spans.
...
- Every button and link has the exact Foundation dual blue `focus-visible` ring.
