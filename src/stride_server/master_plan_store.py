...
def get_master_plan_store() -> MasterPlanStore:
    """Return the appropriate backend based on env vars.

    - If ``STRIDE_MASTER_PLAN_TABLE_ACCOUNT_URL`` is set → Azure Table backend.
    - Otherwise → JSON file backend (dev / test).

    Result is cached (singleton per process). Call
    ``reset_master_plan_store_cache()`` in tests to get a fresh instance.
    """
    account_url = os.environ.get(ACCOUNT_URL_ENV, "").strip()
    table_name = (
        os.environ.get(TABLE_NAME_ENV, DEFAULT_TABLE_NAME).strip() or DEFAULT_TABLE_NAME
    )
    if account_url:
        logger.info(
