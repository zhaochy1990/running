...
- goal_realism 评估有条件 A 目标时，要看完整 gate 组合，不要只看单个 HM/10K gate。若 plan 明确默认执行 B、A 需要 HM/10K gate + 30-32km MP rehearsal + injury/HR/RPE gate 同时通过，则 HM 单项略宽只能算观察门，不应单独扣 goal_realism。若同一个 plan 的 race milestone 写了更严格 A gate（如 `HM≤1:24:30 + 32km MP + VO2/HR/RPE/跟腱全过`），而 7月 HM milestone 写较宽的 `≤1:25:30` 或 `≤1:26:30`，应把 7月 HM 当作 B/观察门，不要因它较宽而扣 goal_realism。zhaochaoyi fixture 中，若 7月 HM 明确写作 `观察/B+` 且 race milestone/principle 用 `31km/22km MP + VO2max + HR/RPE + 跟腱` 开 A，可给 goal_realism=5。若用户提到 VO2max/RHR/高原/跟腱等信号，plan 在原则、milestone 或 monitoring trigger 中把这些作为 A 通道观察/降级条件即可；不要求每个信号都出现在 race milestone 的同一句话里。
...
- MasterPlan schema 的 `goal.target_time` 只能容纳一个时间，常会填用户的 A 目标。若 plan 在 `training_principles`、race milestone 或 phase notes 中明确“默认执行 B=2:52-2:53，A=2:50 仅在多重 gate 全过时开放”，不要因为 `goal.target_time` 仍是 `2:50:00` 而判定 A 目标无条件化，也不要仅因此把 goal_realism 从 5 降到 4。
