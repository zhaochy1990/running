...
func runAPI() error {
	...
	verifier, err := api.NewJWTVerifier(cfg.API.Auth.PublicKeyPath, cfg.API.Auth.Issuer, cfg.API.Auth.Audience, cfg.API.Auth.AdminAudience)
	if err != nil {
		return err
	}
	...
	svc := api.NewService(api.Config{
		...
		Jobs:                    store.Jobs(),
		JobsIdem:                store,
		JobsCreate:              store.Jobs(),
		JobsTransition:          store,
		JobsStale:               store,
		JobsAdminList:           store,
		Pipelines:               orch,
