...
func TestGetWeeklyFeedbackDistinguishesMissingAndEmptyRowsByUser(t *testing.T) {
	store := openTestStore(t)
	ctx := context.Background()
	if err := store.AutoMigrateWeeklyFeedback(ctx); err != nil {
		t.Fatalf("automigrate: %v", err)
	...
	userID, otherUser := uuid.NewString(), uuid.NewString()
	if _, err := store.PutWeeklyFeedback(ctx, userID, "2026-07-27", ""); err != nil {
		t.Fatalf("put empty: %v", err)
	}
	if _, err := store.PutWeeklyFeedback(ctx, otherUser, "2026-07-27", "private"); err != nil {
		t.Fatalf("put other: %v", err)
	}
	row, err := store.GetWeeklyFeedback(ctx, userID, "2026-07-27")
	if err != nil || row == nil || row.ContentMD != "" || row.CreatedAt.IsZero() || row.UpdatedAt.IsZero() {
		t.Fatalf("empty row=%+v err=%v", row, err)
	}
	missing, err := store.GetWeeklyFeedback(ctx, userID, "2026-08-03")
	if err != nil || missing != nil {
		t.Fatalf("missing row=%+v err=%v", missing, err)
	}
}
