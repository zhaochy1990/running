...
// plus internal derived profile facts. Public profile writes still own only the
// onboarding core fields; they cannot set or clear derived fields.
//
...
type UserProfile struct {
	...
	// UsualActivityArea* stores an imprecise majority cluster of activity
	// starts. ComputedAt distinguishes "computed but unknown" (nil center) from
	// "not computed yet" and prevents repeated historical timeseries scans.
	UsualActivityAreaLatitude             *float64   `gorm:"column:usual_activity_area_latitude" json:"-"`
	UsualActivityAreaLongitude            *float64   `gorm:"column:usual_activity_area_longitude" json:"-"`
	UsualActivityAreaSupportingActivities int        `gorm:"column:usual_activity_area_supporting_activities;not null;default:0" json:"-"`
	UsualActivityAreaComputedAt           *time.Time `gorm:"column:usual_activity_area_computed_at;type:datetime(6)" json:"-"`
