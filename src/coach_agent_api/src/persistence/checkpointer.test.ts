...
test("checkpoint metadata makes a completed turn receipt recoverable", async () => {
	...
	await saver.put(
		config,
		checkpoint([{ type: "ai", content: "done" }]),
	...
	assert.deepEqual(
		(inserted[6] as Buffer).toString("utf8").includes("turn_fingerprint"),
		true,
	);
});
...
test("recovery does not mistake a prior turn assistant message for this turn", async () => {
			...
			return [
				[
					...
					},
					{
						...
						metadata: Buffer.from(
							JSON.stringify({ source: "loop", step: 1, parents: {} }),
						),
						type: "json",
					},
				],
	...
	assert.deepEqual(
		await new MySqlSaver(pool).recoverTurn("thread", "new-turn"),
		{ kind: "incomplete", fingerprint: "fingerprint" },
	);
});
					...
					[
						{
							...
							type: "json",
							blob_data: Buffer.from(
								JSON.stringify([{ value: { question: "Choose" } }]),
							),
						},
					],
				];
