...
export class MySqlSaver extends BaseCheckpointSaver {
	...
	async put(
		...
		if (threadId === undefined) {
			throw new Error(
				"MySqlSaver.put: missing thread_id in config.configurable",
			);
		}
		const ns = (config.configurable?.checkpoint_ns as string | undefined) ?? "";
		const parentId =
	...
	async putWrites(
		...
		if (threadId === undefined || checkpointId === undefined) {
			throw new Error(
				"MySqlSaver.putWrites: missing thread_id or checkpoint_id",
			);
		}
	...
	async deleteThread(threadId: string): Promise<void> {
		await this.pool.execute(`DELETE FROM checkpoints WHERE thread_id=?`, [
			threadId,
		]);
		await this.pool.execute(`DELETE FROM checkpoint_writes WHERE thread_id=?`, [
			threadId,
		]);
	}
