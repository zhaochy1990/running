...
- JWT: exactly one of `STRIDE_AUTH_PUBLIC_KEY_PEM`, `STRIDE_AUTH_PUBLIC_KEY_PATH`, or `STRIDE_AUTH_SERVICE_URL` (fetched from `{auth_service_url}/api/system/public-key` at startup), plus `STRIDE_AUTH_ISSUER` and optional `STRIDE_AUTH_AUDIENCE`
